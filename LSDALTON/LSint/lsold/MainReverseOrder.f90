...
CALL Build_PRECALCULATED_SPHMAT(lupri,sharedTUV,OD_LHS,OD_RHS)
...
real(realk)           :: maxLHSELM,maxRHSELM
...
ENDDO
DO ILHS=1,OD_LHS%nbatches
  IF (nPrimP(IRHS) .GT. 0) CALL FREE_OVERLAP(P(ILHS))
ENDDO
...
Character*(*)        :: SIDE
!
Integer              :: IA,Ja,KA,IB,JB,KB,sA,sB,CA,CB,AOA,AOB
     ...
     DO JB=1,INPUT%AO(AOB)%p%BATCH(IB)%nAngmom
      ...
      DO CA=1,INPUT%AO(AOA)%p%BATCH(IA)%nContracted(JA)
       DO CB=1,INPUT%AO(AOB)%p%BATCH(IB)%nContracted(JB)
        DO KA=1,INPUT%AO(AOA)%p%BATCH(IA)%nOrbComp(JA)
         DO KB=1,INPUT%AO(AOB)%p%BATCH(IB)%nOrbComp(JB)
          RED_GAB(IA,IB)=MAX(RED_GAB(IA,IB),GAB(sA+KA+(CA-1)*KA-1,sB+KB+(CB-1)*KB-1))
         ENDDO
        ENDDO
       ENDDO
      ENDDO
...
SUBROUTINE DistributePQin3(PQ,nContA,nContB,nContC,nContD,nAngA,nAngB,nAngC,nAngD,&
      &                    startA,startB,startC,startD,iderivP,NderivP,iderivQ,Output,INPUT)
implicit none
Real(realk)          :: PQ(nContA*nContB,nAngA,nAngB,nContC*nContD,nAngC,nAngD)
...
!   Calculation of screening integrals
    IF (Input%CS_int) THEN
      IF ((startA.EQ.startC).AND.(startB.EQ.startD)) THEN
        integral = sqrt(PQ(iContP,iAngA,iAngB,iContP,iAngA,iAngB))
     ...
     DO iContC=1,nContC
      DO iAngC=1,nAngC
       ...
       DO iContD=1,nContD
        ...
        DO iAngD=1,nAngD
         integral = PQ(iContP,iAngA,iAngB,iContQ,iAngC,iAngD)
               ...
               DO iContC=1,nContC
                  DO iAngC=1,nAngC
                     ...
                     DO iContD=1,nContD
                        ...
                        DO iAngD=1,nAngD
                           Output%ResultMat(iA,iB,iC,iD,ideriv) = &
                                PQ(iContP,iAngA,iAngB,iContQ,iAngC,iAngD)
                           IF(iC .NE. iD)THEN
                              Output%ResultMat(iA,iB,iD,iC,ideriv) = &
                                   PQ(iContP,iAngA,iAngB,iContQ,iAngC,iAngD)
                           ENDIF
 ...
 DO i1=1,P%orbital1%nPrimitives
   ...
   DO i2=start2,P%orbital2%nPrimitives
     ...
     IF(INPUT%PS_SCREEN)THEN
        ...
        ENDDO
     ENDIF
     IF(.NOT.INPUT%PS_SCREEN .OR. maxPrimGab .GT. INPUT%PS_THRESHOLD/maxPrimGabElm)THEN
       i12 = i12 + 1
...
DO iAngmom=1,P%nAngmom
  ...
  CALL BuildEcoeffTensor(TUV,P,P%ETUV(indexETUV:indexETUV+LENGTH),P%nPrimitives,ijk,nTUV,iAngmom,LUPRI,IPRINT)
ENDDO
...
DO iAngmom = 1,P%nAngmom
  ...
  ELSE
    ...
    nTUV = (l+1)*(l+2)*(l+3)/6
    NULLIFY(Ecoeffs)
    ALLOCATE(Ecoeffs(P%nPrimitives,ijk,nTUV))
    CALL BuildEcoeffTensor(Integral%TUV,P,Ecoeffs,P%nPrimitives,ijk,nTUV,iAngmom,LUPRI,IPRINT)
...
!    &                                     lm1,lm2,ijk1,ijk2,LUPRI,IPRINT)
     ...
     &                                     lm1,lm2,ijk1,ijk2,LUPRI,IPRINT)
      NULLIFY(SpherEcoeff)
      ALLOCATE(SpherEcoeff(P%nPrimitives,lm,nTUV))
      DO ituv=1,nTUV
        DO j=1,lm
          DO iPrim=1,P%nPrimitives
            SpherEcoeff(iPrim,j,ituv) = Ecoeffs(iPrim,1,ituv) * Spherical(1,j)
          ENDDO
        ENDDO
        DO i=2,ijk
          DO j=1,lm
            DO iPrim=1,P%nPrimitives
              SpherEcoeff(iPrim,j,ituv) = SpherEcoeff(iPrim,j,ituv) + &
     &           + Ecoeffs(iPrim,i,ituv) * Spherical(i,j)
            ENDDO
          ENDDO
        ENDDO
      ENDDO
...
!   Set up FTUV's
    DO idmat=1,Input%NDMAT_RHS
     ...
     &               + ContractedDmat(indlm,iPrim,idmat)*SpherEcoeff(iPrim,indlm,iTUV)
          ENDDO
        ENDDO
      ENDDO
    ENDDO
...
coulomb = PQ%Operator.EQ.'Coulomb'
nucrep = PQ%Operator(1:6).EQ.'Nucrep'
CALL SetIntegrand(PQ%iprimP,PQ%iprimQ,PQ%distance,PQ%squaredDistance,PQ%exponents,&
     &  PQ%reducedExponents,coulomb,nucrep,PQ%integralPrefactor,PQ%nPrimitives,&
!     &  PQ%reducedExponents,PQ%operator,PQ%integralPrefactor,PQ%nPrimitives,&
...
CASE ('Overlap')
  CALL GET_WTUV(INTEGRAL,PQ,LUPRI,IPRINT,'Overlap')
CASE ('Coulomb')
  CALL GET_WTUV(INTEGRAL,PQ,LUPRI,IPRINT,'Coulomb')
CASE ('Nucrep')
  CALL GET_WTUV(INTEGRAL,PQ,LUPRI,IPRINT,'Nucrep ')
CASE ('Kinetic')
  CALL GET_WTUV(INTEGRAL,PQ,LUPRI,IPRINT,'Overlap')
CASE ('Carmom')
...
SUBROUTINE GET_WTUV(INTEGRAL,PQ,LUPRI,IPRINT,OPERATORLABEL)
...
CASE ('Overlap')
   CALL BUILD_SJ000(SJ000,PQ,nPrim,JMAX,LUPRI,IPRINT)
CASE ('Coulomb')
   CALL BUILD_RJ000(SJ000,PQ,nPrim,JMAX,LUPRI,IPRINT,integral)
CASE ('Nucrep')
   CALL BUILD_NUCLEAR_RJ000(SJ000,PQ,nPrim,JMAX,LUPRI,IPRINT,integral)
CASE DEFAULT
...
!DO J=PQ%startAngmom,PQ%endAngmom
...
implicit none
Real(realk) :: PQ(nQ,nCont,nAng),AddPQ(nCont,nQ,nAng)
Integer     :: nQ,nAng,nCont,LUPRI,IPRINT
!
Integer     :: iAng
!
DO iAng=1,nAng
  call transposition(PQ(1,1,iAng),AddPQ(1,1,iAng),nQ,nCont)
ENDDO
...
IF(PQ%Operator(1:7) .EQ. 'Kinetic')THEN
   ...
   NULLIFY(Integral%IntegralIN)
   ALLOCATE(Integral%IntegralIN(nP,nOrbQ,ntuvP))
      ...
      DO jP = startP,endP
         DO tP=jP,0,-1
            DO uP=jP-tP,0,-1
               ...
               DO iPrimP=1,nP
                  Integral%IntegralIN(iPrimP,iOrbQ,ituvP) = &
      ...
      DO jP = startP,endP
         DO tP=jP,0,-1
            DO uP=jP-tP,0,-1
               ...
               DO iPrimP=1,nP
                  Integral%IntegralIN(iPrimP,iOrbQ,ituvP) = Integral%TUVQ(iPrimP,ifullP,iOrbQ)
               ENDDO
            ENDDO
         ENDDO
      ENDDO
...
implicit none
Real(realk) :: TUVQ(nPrimP,nTUVP,nContQ,nCompQ)
Real(realk) :: OldTUVQ(nContQ,nPrimP,nTUVP,nCompQ)
...
DO iCompQ=1,nCompQ
  DO iTUVP=1,nTUVP
    DO iContQ=1,nContQ
      DO iPrimP=1,nPrimP
        TUVQ(iPrimP,iTUVP,iContQ,iCompQ) = OldTUVQ(iContQ,iPrimP,iTUVP,iCompQ)
      ENDDO
    ENDDO
  ENDDO
ENDDO

CALL PrintTUVQ(TUVQ,nPrimP,nTUVP,nContQ,nCompQ,LUPRI,IPRINT)
END SUBROUTINE AddToTUVQ1

SUBROUTINE PrintTUVQ(TUVQ,nPrimP,nTUVP,nContQ,nCompQ,LUPRI,IPRINT)
implicit none
Real(realk) :: TUVQ(nPrimP,nTUVP,nContQ,nCompQ)
Integer     :: nTUVP,nPrimP,nCompQ,nContQ,LUPRI,IPRINT
!
...
IF (IPRINT.GT.10) THEN
  ...
  DO iContQ=1,nContQ
    DO iCompQ=1,nCompQ
      DO iTUVP=1,nTUVP
        WRITE(LUPRI,'(5X,A,I3,A,I3,A,I3)') 'iTUVP =',iTUVP,' iContQ =',iContQ,' iCompQ =',iCompQ
        WRITE(LUPRI,'(5X,5F10.4)')  (TUVQ(iPrimP,iTUVP,iContQ,iCompQ), iPrimP=1,nPrimP)
      ENDDO
    ENDDO
  ENDDO
ENDIF
...
IF (Sph1.OR.Sph2) THEN
  ...
  ALLOCATE(Integral%IntegralOUT(Integral%nPrim,Integral%nOrb,lm))
!
! CALL ContructSphericalTransformation(Spherical,Reshape(P%orbital1%SPHMAT(iS1:eS1),(/lm1,ijk1/)),&
!    &              Reshape(P%orbital2%SPHMAT(iS2:eS2),(/lm2,ijk2/)),lm1,lm2,ijk1,ijk2,LUPRI,IPRINT)
...
! Spherical transformation (with both centers beloning to one electron simultaneously)
  ...
  CALL DGEMM('N','N',dim1,lm,ijk,D1,Integral%IntegralIN,dim1,&
     &     Spherical,ijk,D0,Integral%IntegralOUT,dim1)
...
NULLIFY(Integral%IntegralOUT)
ALLOCATE(Integral%IntegralOUT(nCont,Integral%nOrb,Integral%nAng))
NULLIFY(CC)
ALLOCATE(CC(P%nPrimitives,nC1,nC2))
...
CALL DGEMM('T','N',nCont,dim1,Integral%nPrim,D1,CC,Integral%nPrim,&
     &     Integral%IntegralIN,Integral%nPrim,D0,Integral%IntegralOUT,nCont)


IF (IPRINT.GT.50) THEN
   CALL PrintTensor(Integral%IntegralOUT,'Contracted          ',&
        &nCont,Integral%nOrb,Integral%nAng,Lupri,'iCont ','iOrb  ','iAngP ',3)
ENDIF
...
!  CALL QUIT('Only Hermite implemented in ContractEcoeff')
  ...
  ijk  = ijk1*ijk2
  NULLIFY(Integral%IntegralOUT)
  ALLOCATE(Integral%IntegralOUT(Integral%nPrim,Integral%nOrb,ijk))

IF (P%ETUVisSet) THEN
  CALL ContractEcoeff1(Integral%IntegralIN,Integral%IntegralOUT,&
       &P%ETUV(P%ETUVindex(iAngmom):P%ETUVindex(iAngmom)+Integral%nPrim*Integral%nAng*ijk), &
     &                 Integral%nOrb,Integral%nAng,ijk,Integral%nPrim)
ELSE
  NULLIFY(Ecoeffs)
  ALLOCATE(Ecoeffs(Integral%nPrim,ijk,Integral%nAng))
  CALL BuildEcoeffTensor(integral%TUV,P,Ecoeffs,Integral%nPrim,ijk,Integral%nAng,&
...
Real(realk) :: IntegralIN(nPrim,nOrb,nAng)
Real(realk) :: IntegralOUT(nPrim,nOrb,ijk)
Real(realk) :: Ecoeffs(nPrim,ijk,nAng)
!
Integer :: iAng,i,iOrb,iPrim
Real(realk)         :: D1=1.0d0,D0=0.0d0
...
DO i=1,ijk
  DO iOrb=1,nOrb
    DO iPrim=1,nPrim
      IntegralOUT(iPrim,iOrb,i) = IntegralIN(iPrim,iOrb,1)*Ecoeffs(iPrim,i,1)
    ENDDO
  ENDDO
ENDDO
! iAng > 1
DO iAng = 2,nAng
  DO i=1,ijk
    DO iOrb=1,nOrb
      DO iPrim=1,nPrim
        IntegralOUT(iPrim,iOrb,i) = IntegralOUT(iPrim,iOrb,i) &
     &          + IntegralIN(iPrim,iOrb,iAng)*Ecoeffs(iPrim,i,iAng)
      ENDDO
    ENDDO
  ENDDO
ENDDO
...
Real(realk) :: D1=1.0d0,DM1=-1.0d0,signQ,oldfsum
Real(realk) :: fsum(nPrimP),ftemp
...
DO tuvP=1,ntuvP
  ...
  DO tuvQ=1,ntuvQ
    ...
    ftemp = FTUV(1,tuvQ,idmat)
    DO iPrimP=1,nPrimP
        fsum(iPrimP) = WTUV(iPrimP,tuvPQ)*ftemp
    ENDDO
    iPrimPQ = nPrimP + 1
    DO iPrimQ=2,nPrimQ
      ftemp = FTUV(iPrimQ,tuvQ,idmat)
      DO iPrimP=1,nPrimP
        fsum(iPrimP) = fsum(iPrimP) + WTUV(iPrimPQ,tuvPQ)*ftemp
        iPrimPQ = iPrimPQ+1
      ENDDO
    ENDDO
    DO iPrimP=1,nPrimP
      TUV(iPrimP,tuvP,idmat) = TUV(iPrimP,tuvP,idmat) + fsum(iPrimP)*signQ
    ENDDO
...
!    iPrimPQ = 1
!    DO iPrimP=1,nPrimP
!      oldfsum = WTUV(iPrimPQ,tuvPQ)*FTUV(1,tuvQ,idmat)
!      iPrimPQ = iPrimPQ+1
!      DO iPrimQ=2,nPrimQ
!        oldfsum = oldfsum + WTUV(iPrimPQ,tuvPQ)*FTUV(iPrimQ,tuvQ,idmat)
!        iPrimPQ = iPrimPQ+1
!      ENDDO
!      TUV(iPrimP,tuvP,idmat) = TUV(iPrimP,tuvP,idmat) + oldfsum*signQ
!    ENDDO
!    ENDIF
  ENDDO
ENDDO
...
Real(realk)         :: Ecoeffs(x,y,z) !nprim,ijk1*ijk2,nAng
...
implicit none
Real(realk)   :: HerInt(nPrim1,nPrim2,nAng*nOrb)
TYPE(Overlap) :: P
...
DO iPrim2=1,nPrim2
  ...
  DO iPrim1=1,nPrim1
    ...
    DO iAngOrb=1,nAng*nOrb
      HerInt(iPrim1,iPrim2,iAngOrb) = HerInt(iPrim1,iPrim2,iAngOrb) * pref12
    ENDDO
  ENDDO
ENDDO
...
Integer                 :: iPrimPQ,iP,iQ,iTUV,iPrimP,iPrimQ
...
ALLOCATE(Integral%tuvTUV(nQ,nP,ntuvP,ntuvQ))
         ...
         DO jP = startP,endP
            ...
            DO tP=jP,0,-1
               DO uP=jP-tP,0,-1
                  ...
                  DO iPrimQ=1,nQ
                    DO iPrimP=1,nP
                      Integral%tuvTUV(iPrimQ,iPrimP,ituvP,ituvQ) &
                          &  = signQ*Integral%WTUV(iPrimPQ,ituvPQ)
                      iPrimPQ = iPrimPQ + 1
                    ENDDO
                  ENDDO
!                  DO iPrimPQ=1,PQ%nPrimitives
!                     Integral%tuvTUV(PQ%iPrimQ(iPrimPQ),PQ%iPrimP(iPrimPQ),ituvP,ituvQ) &
!                          &  = signQ*Integral%WTUV(iPrimPQ,ituvPQ)
!                 ENDDO
               ENDDO
            ENDDO
...
NULLIFY(Integral%IntegralIN)
ALLOCATE(Integral%IntegralIN(nQ,nP*ntuvP,ntuvQ))
CALL DCOPY(nQ*nP*ntuvP*ntuvQ,Integral%tuvTUV,1,Integral%IntegralIN,1)
...
!       WRITE(LUPRI,'(3X,A,3I2)') 'tQ,uQ,vQ',tQ,uQ,vQ
      ...
      &            'W(',tP,',',uP,',',vP,'|',tQ,',',uQ,',',vQ,') ='
            DO iPrimP=1,np
               WRITE(LUPRI,'(17X,I3,5F10.4/,(20X,5F10.4))') iPrimP,&
      &           (Integral%tuvTUV(iPrimQ,iPrimP,ituvP,ituvQ),iPrimQ=1,nq)
            ENDDO
            ENDDO
          ENDDO
        ENDDO
...
SUBROUTINE Build_PRECALCULATED_SPHMAT(LUPRI,sharedTUV,OD_LHS,OD_RHS)
IMPLICIT NONE
TYPE(ODITEM)     :: OD_LHS,OD_RHS
...
CALL BUILD_RJ000_OP(RJ000,nPrim,Jmax,LUPRI,IPRINT,&
     ...
     &PQ%integralPrefactor,INTEGRAL%TUV%TABFJW)
...
SUBROUTINE BUILD_RJ000_OP(RJ000,nPrim,Jmax,LUPRI,IPRINT,alpha,R2,Prefactor,TABFJW)
IMPLICIT NONE
INTEGER         :: nPrim,Jmax,Lupri,Iprint
REAL(REALK)     :: RJ000(nPrim,0:Jmax)
REAL(REALK)     :: alpha(nPrim),R2(nprim),Prefactor(nprim),TABFJW(nprim)
...
REAL(REALK)     :: RJ000(nPrim,0:Jmax),prefac(Nprim),Prefactor(nprim)
...
IF (JMAX .GT. 0) THEN
   ...
   DO J = 0, JMAX-1
      DO I = 1, Nprim
         RJ000(I,J) = prefactor(I)*RJ000(I,J)
      ENDDO
      DO I = 1, Nprim
         prefactor(I) = prefac(I)*prefactor(I)
      ENDDO
   ENDDO
   DO I = 1, Nprim
      RJ000(I,JMAX) = prefactor(I)*RJ000(I,JMAX)
   ENDDO
ELSE
   DO I = 1, Nprim
...
Integer             :: LUPRI,IPRINT
!
Integer,parameter :: MAXAOJ=12,MAXDER=2,MAXJ=4*MAXAOJ+MAXDER
!Integer,parameter :: MAXAOJ=12,MAXDER=2
!Integer,parameter :: MAXJ=12
Integer,parameter :: NODES=121,ORDER=7,NTABFJ000=NODES*(MAXJ+ORDER)
Integer,parameter :: NTUVMAX = (MAXJ+1)*(MAXJ+2)*(MAXJ+3)/6
!
IF(INPUT%Operator(1:7) .EQ. 'Coulomb' .OR. INPUT%Operator(1:6).EQ.'Nucrep')THEN
...
TYPE(IntegralInput) :: Input
INTEGER             :: I
IF(INPUT%Operator(1:7) .EQ. 'Coulomb') DEALLOCATE(sharedTUV%TABFJW)
...
!Simen  Parameter was fixed empirically using ifort/SUN X2200 Opteron/2.2 Ghz 
!Simen  (titan.uio.no)
...
DO idmat = 1,ndmat
  DO tuv=1,F%nTUV
    DO ip=1,np
!simen
write(lupri,*) 'debug:',ip,tuv,idmat,offp,Q%FTUV(ip,tuv,idmat),F%FTUV(ip+offp,tuv,idmat)
...
!!$      ENDDO
 ...
 !     WRITE(LUPRI,*)'CARTORDER',CARTORDER,'CALL Multipole recurrence'
      CALL Multipole_Recurrence(INTEGRAL%Wtuv,M000,Jstart,JMAX,CARTORDER,&
           ...
           & Xorder,Yorder,Zorder,NPrim,nTUV,Integral%TUV%TUVindex,&
...
SUBROUTINE Multipole_Recurrence(OUTPUT,M000,Jstart,JMAX,CARTORDER,&
        & Xpq,Ypq,Zpq,Xorder,Yorder,Zorder,NPrim,nTUV,TUVindex,&
        & zeroX, zeroY, zeroZ,EXPONENTS,lupri)
IMPLICIT NONE
real(realk)             :: OUTPUT(nPrim,NTUV),M000(nPrim)
INTEGER                 :: J,K,T,U,V,TUV,JVAL,JMAX,NTUV
INTEGER                 :: nPrim
INTEGER,PARAMETER       :: MAXJ = 50
INTEGER                 :: TUVindex(0:MAXJ,0:MAXJ,0:MAXJ)
...
INTEGER                 :: Tm1,Tp1,jstart,CARTORDER,lupri,C,Nefg
