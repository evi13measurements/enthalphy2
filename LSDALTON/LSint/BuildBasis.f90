...
SUBROUTINE Build_BASIS(LUPRI,IPRINT,MOLECULE,BASINFO,BASISSETLIBRARY,&
     &BASISLABEL,UNCONTRACTED,SINGLESEGMENT,doprint,BASISSETNAME)
use files
use molecule_type
use molecule_module
implicit none
...
CALL GET_BASISSET_LIB(LUPRI,BASISDIR,LENBAS,doprint)
IF(IPRINT .GT. 0) WRITE(LUPRI,*)'BASISSET LIBRARY: ',BASISDIR(1:LENBAS)
...
DO I=1,J  
 ...
 IF(present(BASISSETNAME))THEN
  ...
  STRING = BASISDIR(1:LENBAS)//BASISSETNAME(1:NEND)
 ELSE
  ...
  STRING = BASISDIR(1:LENBAS)//BASISSETLIBRARY%BASISSETNAME(BINDEXES(I))(1:NEND)
 ENDIF
 ...
 ELSE
  ...
  CALL READ_BASISSET_FILE_AND_BUILD_BASISINFO(LUPRI,IPRINT,LUBAS,&
       ...
       &CHARGES,k,atomtype,SINGLESEGMENT)
  call mem_dealloc(CHARGES)
 ENDIF
...
CALL DETERMINE_NBAST(MOLECULE,BASINFO,.TRUE.,.FALSE.)
...
!   call mem_alloc(BASINFO%Chargeindex,MaxCharge)   
   NULLIFY(BASINFO%Chargeindex)   
   ALLOCATE(BASINFO%Chargeindex(MaxCharge))   
...
!  This is the case when ATOMBASIS is used in MOLECULE.INP
   ...
   DO I=1,MOLECULE%natoms
      ...
      DO J=1,BASINFO%natomtypes
         ...
         ENDIF
      ENDDO
      IF(MOLECULE%ATOM(I)%IDtype(R).EQ.0)CALL LSQUIT('ERROR IN IDtype assignment',-1)
   ENDDO
ENDIF
...
IF(K==0) CALL LSQUIT('Something wrong in Unique_basissets',lupri)
...
implicit none
INTEGER  :: LUPRI,k,CHARGES(:),I,IT,J
TYPE(BASISSETLIBRARYITEM) :: BASISSETLIBRARY
LOGICAL  :: NEWCHARGE
DO I=1,BASISSETLIBRARY%nbasissets
   DO J=1,BASISSETLIBRARY%nCharges(I)
      ...
      ELSE
         NEWCHARGE=.TRUE.
         DO IT=1,k
            IF(BASISSETLIBRARY%Charges(I,J)==CHARGES(IT)) NEWCHARGE=.FALSE.
         ENDDO
         IF(NEWCHARGE) THEN
            k=k+1
...
SUBROUTINE READ_BASISSET_FILE_AND_BUILD_BASISINFO(LUPRI,IPRINT,LUBAS,&
                    &CONTRACTED,BASISSETNAME,IAUG,POLFUN,IPOLST,&
                                  &BASINFO,CHARGE,L,atomtype,SINGLESEGMENT)
implicit none
!> the logical unit number for the output file
INTEGER                   :: LUPRI
!> the printlevel integer, determining how much output should be generated
...
INTEGER             :: ELEMENT,Totalnprim,TotalnOrbitals,AA,I
...
Call bubbleSort(CHARGE,L)

DO I=1,L !number of unique charges in basisset
   IF(IPRINT .GT. 5) WRITE(LUPRI,*)'FIND CHARGE',CHARGE(I)
  !SEARCH TO FIND ELEMENT WITH CHARGE(I) 
  CALL FIND_POSITION_ON_FILE(LUPRI,LUBAS,CHARGE(I),BASISSETNAME,ELEMENT) 
  BASINFO%ATOMTYPE(atomtype+I)%Charge=CHARGE(I)
  ...
  CALL TEST_IF_NEXT_ANGMOM_OR_NEW_ELEMENT(LUPRI,LUBAS, nprim, nOrbitals,&
                                                    &NewElements,ELEMENT)
  IF(NewElements) EXIT
  ...
  IF(nprim .NE. 0)THEN
    ...
    IF ((.NOT.CONTRACTED).OR.segmentedFormat) THEN 
      ...
      CALL INIT_BASISSETINFO_ContractionMatrix(BasInfo,atomtype+I,&
                                               &nAngmom,nprim+IAUG)
      DO K=1,nprim+IAUG
    ...
    ELSE
      ...
      IF(SINGLESEGMENT)THEN
         ...
         DO AA=1,nprim*nOrbitals
            BASINFO%ATOMTYPE(atomtype+I)%SHELL(nAngmom)%segment(1)&
                 &%elms(AA)=ContractionMatrix%elms(AA)
         ENDDO
         DO AA=1,nprim
      ...
      ELSE !DEFAULT
         CALL ANALYSE_CONTRACTIONMATRIX(LUPRI,IPRINT,BASINFO,atomtype+I,&
              &nAngmom,nprim,nOrbitals,ContractionMatrix,Exponents)   
      ENDIF
...
!> \author T. Kjaergaard
!> \date 2010
SUBROUTINE FIND_POSITION_ON_FILE(LUPRI,LUBAS,CHARGE,BASISSETNAME,ELEMENT) 
implicit none 
CHARACTER(len=140)  :: STRING
CHARACTER(len=1)    :: SIGN
INTEGER             :: CHARGE,NUCLEARCHARGE,ios,ELEMENT,LUBAS,LUPRI
LOGICAL             :: SEARCHING
CHARACTER(len=80)   :: BASISSETNAME
...
!> 
SUBROUTINE TEST_IF_NEXT_ANGMOM_OR_NEW_ELEMENT(LUPRI,LUBAS, nExponential, nOrbitals, NEWEL,ELEMENT)
implicit none
INTEGER             :: LUBAS,nExponential, nOrbitals,ELEMENT,ios,LUPRI
LOGICAL             :: BLANK,NEWEL
CHARACTER(len=140)  :: STRING
CHARACTER(len=1)    :: SIGN
NEWEL = .FALSE.
  ...
  ELSE
    ...
    IF (SIGN .EQ. ' ') THEN
      ...
      IF (BLANK) THEN
        cycle
      ELSE
        READ (STRING, '(BN,2I5)') nExponential, nOrbitals
        EXIT
      END IF
...
END SUBROUTINE TEST_IF_NEXT_ANGMOM_OR_NEW_ELEMENT
...
INTEGER               :: atype,nang,nprim,nOrbital,IAUG
INTEGER               :: J,NUMBER_OF_LINES,KNTORB,NUMNUM,KAUG
CHARACTER(len=140)    :: STRING
CHARACTER(len=1)      :: SIGN
...
IF(IAUG .GT. 0) THEN
 ...
 ELSE !THE NORMALE CASE OF A SEGMENTED OR GENERAL CONTRACTED
  ...
  DO KAUG = 1, IAUG
   ...
   Contractionmatrix%elms(nprim+kaug+(nOrbital+kaug-1)*(nprim+iaug))=1.0D0 
   EXMIN2=EXMIN1
   EXMIN1=Exp
  ENDDO
 ENDIF
...
! This finds out how many lines we have, and puts it into NUMBER_OF_LINES.
      IF ((nOrbital - 6) .LE. 0) THEN
         NUMBER_OF_LINES = 1
      ELSE IF (DMOD(C,B) .LT. 1.0D-30) THEN
...
SUBROUTINE ANALYSE_CONTRACTIONMATRIX(LUPRI,IPRINT,BASINFO,atomtype,&
          &nAngmom,nprim,nOrbital,Contractionmatrix,Exponents)
implicit none
TYPE(BASISSETINFO)  :: BASINFO
TYPE(lsmatrix) :: Contractionmatrix
TYPE(lsmatrix) :: Exponents
...
DO I=1,nprim*nOrbital
  IF(NEWBLOCK)THEN
    IF(ABS(Contractionmatrix%elms(I)) .LT. 1.0D-30) THEN 
      segments=segments+1      
      end(segments)=I-1        
      NEWBLOCK=.FALSE.
...
CALL INIT_BASISSETINFO_ContractionMatrix(BasInfo,atomtype,&
                                                  &nAngmom,Nsegments)
...
!ALLOCATE(SEGMENTROW(Nsegments))
!ALLOCATE(SEGMENTCOL(Nsegments))
...
!          WRITE(LUPRI,*)'CC(',Nstart(I)+K+(SEGMENTcol(I)+1-1)*nprim-1,')=',Contractionmatrix%elms(Nstart(I)+K+(SEGMENTcol(I)+1-1)*nprim-1)
          IF(Nstart(I)+K+(SEGMENTcol(I)+1-1)*nprim-1.LE.nprim*norbital)THEN
             IF(ABS(Contractionmatrix%elms(Nstart(I)+K+(SEGMENTcol(I)+1-1)*nprim-1)) .GT. 1.0D-30)&
...
!          WRITE(LUPRI,*)'CC(',Nstart(I)+SEGMENTrow(I)+(L-1)*nprim,')=',Contractionmatrix%elms(Nstart(I)+SEGMENTrow(I)+(L-1)*nprim)
          ...
          IF(ABS(Contractionmatrix%elms(Nstart(I)+SEGMENTrow(I)+(L-1)*nprim)) .GT. 1.0D-30)EXTRAROWS=EXTRAROWS+1          
       ENDDO
...
!      ENDDO
      ...
      !ALLOCATE(TEMPEXP1(SEGMENTrow(I)))
      DO J=1,SEGMENTrow(I)
...
!      ENDDO
      ...
      !ALLOCATE(TEMPEXP2(SEGMENTrow(I+1)))
...
!DEALLOCATE(SEGMENTROW)
!DEALLOCATE(SEGMENTCOL)
...
!> \author T. Kjaergaard
!> \date 2010
SUBROUTINE NORMALIZE_ORBITALS(LUPRI,IPRINT,angmom,nOrbitals,nExp,Exp,Cmatrix)
implicit none
real(realk) :: PI,PIPPI,D0,D1,D2,D4,DP25,DP5,DP75,THRMIN,EXPMIN
TYPE(lsmatrix):: Exp       !Vector of exponents
TYPE(lsmatrix):: Cmatrix   !INPUT : matrix of ContractionCoefficients
                         !OUTPUT: Normalised matrix of C Coefficients
...
IF(IPRINT .GT. 10) THEN
   ...
   CALL LSMAT_DENSE_PRINT(Cmatrix,1,Cmatrix%nrow,1,Cmatrix%ncol,lupri)
ENDIF
PI=3.14159265358979323846D0
D0 = 0.0D0
D1 = 1.0D0
...
THRMIN = 1.D-17
EXPMIN = 1.D-10
PIPPI = (0.5D0/PI)**(0.75D0) !=\sqrt{\frac{1}{(2\pi)^{3/2}}} !3 dimensions
  ...
  DO N = 1, nOrbitals
    ...
    DO L = 1, nExp
      DO M = 1, nExp
        ...
        SUM = SUM + Cmatrix%elms(L+(N-1)*nExp)*Cmatrix%elms(M+(N-1)*nExp)&
                                              &*(T**(angmom + DP5))
      END DO
    END DO
    ...
    SUM=D1/SQRT(SUM)
    DO L=1,nExp
      Cmatrix%elms(L+(N-1)*nExp)=Cmatrix%elms(L+(N-1)*nExp)*SUM&
