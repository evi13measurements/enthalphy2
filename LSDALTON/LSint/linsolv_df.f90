...
IF(ITERATIVE_MINUSHALF) THEN
    call io_get_filename(filename,'ALBEminushalf',basisType,'Empty',basisType,'Empty',0,0,&
     &                   oper,'Contracted',.FALSE.,LUPRI,LUERR)
...
!      Caclulate matric matrix A
       ...
       call ls_getIntegrals(basisType,'Empty',basisType,'Empty',1,1,oper,'Regular','Contracted',&
                             SETTING,LUPRI,LUERR)
       call retrieve_Output(lupri,setting,alphabeta)
...
! Solve linear-equation system using A^{-0.5} matrix twice to reduce errors
    ...
    CALL DGEMM('N','N',naux,ndmat,naux,1.d0,alphabeta,naux,rhs,naux,0.d0,work,naux)
    CALL DGEMM('N','N',naux,ndmat,naux,1.d0,alphabeta,naux,work,naux,0.d0,calpha,naux)
    call mem_dealloc(work)

! Solve linear equations using standard library routines based on Cholesky-factorization
ELSE

    call io_get_filename(filename,'ALBElu',basisType,'Empty',basisType,'Empty',0,0,&
     &                   oper,'Contracted',.FALSE.,LUPRI,LUERR)
    if(.not.io_file_exist(filename,SETTING%IO)) then
!       Caclulate matric matrix A
        call initIntegralOutputDims(setting%Output,naux,1,naux,1,1)
...
!       Perform Cholesky factors C from matric matrix A (A = C^T C)
        CALL DPOTRF('U',naux,alphabeta(:,1,:,1,1),naux,info)
        IF (info.ne.0) THEN
...
do iter=1, MAX_ITER
    ...
    if(iter.eq.1) then
       ...
       emax=1.d0
       emin=(1.d0/4.d0)*emin*(3.d0-emin)**2
    end if
