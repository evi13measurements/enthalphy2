...
!-------------------------------------------------------------------------------
   ...
   TYPE plane_type
      INTEGER(INTK) :: n     ! normal vector to the plane (along x,y, or z)
      INTEGER(INTK) :: d     ! box position along axis where plane is drawn
   END TYPE plane_type

   TYPE vol_type
      ! these are integers since we currently hard-wire for NN box search
      INTEGER(INTK) :: lo(3), hi(3)
   END TYPE vol_type

   TYPE item_type
      INTEGER(INTK) :: id       ! overlap ID to recall from main LHS/RHS arrays
      INTEGER(INTK) :: box(3)   ! box in which moment is held
   END TYPE item_type

   TYPE node_type
      TYPE(node_type), POINTER :: left, right   ! sub-trees
      INTEGER(INTK),   POINTER :: items(:)      ! id to main RHS array
      LOGICAL          :: leaf      ! end of tree (no sub-trees)
      INTEGER(INTK)    :: nitems    ! if a leaf, number of items in it
   ...
   INTEGER(INTK), PARAMETER :: LEAF_MAX = 10 
   ! Named constants for this module
   INTEGER(INTK), PARAMETER :: LEFT_OF_PLANE = 1
   INTEGER(INTK), PARAMETER :: RIGHT_OF_PLANE = 2
   INTEGER(INTK), PARAMETER :: CROSS_PLANE = 3
   ! Max. number of allowed tree recursions
   INTEGER(INTK), PARAMETER :: MAX_DIV = 100000000
...
! 
   SUBROUTINE mm_exe_overlap_tree(LHS,RHS,shape)
      ...
      INTEGER(INTK) :: WS_para, weight, NOFF
      INTEGER(INTK) :: i, common_level
      REAL(REALK) :: rawpairs
      ...
      LHS_branches: DO  ! until pointer disassociated
         ...
         RHS_branches: DO  ! until pointer disassociated
            ...
            END DO
            NOFF = MOD(NOFF+LHS_batch%id%hi-LHS_batch%id%lo+1_intk,INT(NNODES,INTK))
...
! so that separations can be expressed in terms of a common grid
   ...
   FUNCTION translated_box(box_paras,common_level)
      ...
      INTEGER(INTK),      INTENT(IN) :: common_level
      INTEGER(INTK) :: level, tmp_box(3), translated_box(3)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE build_tree(range,root,WS_para)
      ...
      INTEGER(INTK),   INTENT(IN) :: WS_para
      INTEGER(INTK) :: i,j, nitems
...
!-------------------------------------------------------------------------------
   ...
   RECURSIVE SUBROUTINE iterate_tree(node,WS_para)
      ...
      INTEGER(INTK),   INTENT(IN)    :: WS_para
      INTEGER(INTK) :: i, nitems

      TYPE(node_type), POINTER :: left, right  ! sub-trees  
      INTEGER(INTK) :: divisions = 1
      LOGICAL :: done_tree
...
! "done_tree" flags if the orginal node should in fact not be split afterall
   ...
   CONTAINS
      ...
      SUBROUTINE add_to_subtree(side,nitems,ip)
         ...
         INTEGER(INTK), INTENT(IN)    :: ip
         INTEGER(INTK), INTENT(INOUT) :: nitems
         nitems = nitems+1
...
! FIXME: could use only for the root and update on-the-fly for deeper nodes
   ...
   SUBROUTINE get_min_max_xyz(node_items,vol)
      ...
      IMPLICIT NONE
      INTEGER(INTK),  INTENT(IN)  :: node_items(:)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE search_leaf(ref_item,leaf_items,LHS,RHS,wt,tested)
      ...
      IMPLICIT NONE
      TYPE(item_type),    INTENT(IN)    :: ref_item
      INTEGER(INTK),      INTENT(IN)    :: leaf_items(:)
      TYPE(gen_mm_paras), INTENT(IN)    :: LHS, RHS
      INTEGER(INTK),      INTENT(IN)    :: wt
      LOGICAL,            INTENT(INOUT) :: tested(:)
