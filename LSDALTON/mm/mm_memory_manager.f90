...
! peak allocation.
   ...
   use memory_handling, only: mem_allocated_FMM, max_mem_used_FMM, mem_realsize,mem_intsize,&
        &mem_complexsize,max_mem_used_global,mem_allocated_global
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE allocate_3(name,array,a)
      ...
      !$OMP CRITICAL (memory)
      mem_allocated_FMM = mem_allocated_FMM + a*Fraw_mm_paras
      max_mem_used_FMM = MAX(max_mem_used_FMM, mem_allocated_FMM)
      mem_allocated_global = mem_allocated_global + a*Fraw_mm_paras
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE allocate_4(name,array,a)
      ...
      !$OMP CRITICAL (memory)
      mem_allocated_FMM = mem_allocated_FMM + a*Fbox_mm_data
      max_mem_used_FMM = MAX(max_mem_used_FMM, mem_allocated_FMM)
      mem_allocated_global = mem_allocated_global + a*Fbox_mm_data
      max_mem_used_global = MAX(max_mem_used_global,mem_allocated_global)
      !$OMP END CRITICAL (memory)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE allocate_5(name,array,a)
      ...
      !$OMP CRITICAL (memory)
      mem_allocated_FMM = mem_allocated_FMM + a*Fbox_mm_paras
      max_mem_used_FMM = MAX(max_mem_used_FMM, mem_allocated_FMM)
      mem_allocated_global = mem_allocated_global + a*Fbox_mm_paras
      max_mem_used_global = MAX(max_mem_used_global,mem_allocated_global)
      !$OMP END CRITICAL (memory)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE allocate_6(name,array,a,b,c)
      IMPLICIT NONE
      REAL(REALK),   POINTER    :: array(:,:,:)
      INTEGER(INTK), INTENT(IN) :: a,b,c, name
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE allocate_8(array,a)
      ...
      INTEGER(INTK), INTENT(IN) :: a
      INTEGER(INTK) :: error
      integer(KIND=selected_int_kind(10)) :: nsize

      ALLOCATE(array(a),STAT=error)
      IF (error /= 0) CALL LSQUIT('allocation error in allocate_8',-1)
      !size of T_pair_single = 7int,4realk,1Character
      nsize = mem_realsize*4+7*mem_intsize+1*mem_complexsize
      !$OMP CRITICAL (memory)
      mem_allocated_FMM = mem_allocated_FMM + size(array)*nsize
      max_mem_used_FMM = MAX(max_mem_used_FMM, mem_allocated_FMM)
      mem_allocated_global = mem_allocated_global + size(array)*nsize
      max_mem_used_global = MAX(max_mem_used_global,mem_allocated_global)
      !$OMP END CRITICAL (memory)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE deallocate_3(array)
      ...
      INTEGER(INTK) :: error
      !$OMP CRITICAL (memory)
      mem_allocated_FMM = mem_allocated_FMM - size(array)*Fraw_mm_paras
      mem_allocated_global = mem_allocated_global - size(array)*Fraw_mm_paras
      !$OMP END CRITICAL (memory)
      deallocate(array)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE deallocate_4(array)
      ...
      TYPE(box_mm_data), POINTER    :: array(:) 
      !$OMP CRITICAL (memory)
      mem_allocated_FMM = mem_allocated_FMM  - size(array)*Fbox_mm_data
      mem_allocated_global = mem_allocated_global - size(array)*Fbox_mm_data
      !$OMP END CRITICAL (memory)
      deallocate(array)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE deallocate_5(array)
      ...
      TYPE(box_mm_paras), POINTER    :: array(:) 
      !$OMP CRITICAL (memory)
      mem_allocated_FMM = mem_allocated_FMM - size(array)*Fbox_mm_paras
      mem_allocated_global = mem_allocated_global - size(array)*Fbox_mm_paras
      !$OMP END CRITICAL (memory)
      deallocate(array)
