...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_get_BQ_Vff(scheme,LHS_raw_paras,Vff)
      ...
      INTEGER(INTK)            :: level, LMAX, lm_dim
      ...
      CALL mm_allocate(MEM_BOX_VFF,boxed_Vff,lm_dim,INT(SIZE(LHS%box_paras),INTK))
      boxed_Vff = zero
...
!        ! get packed RHS MM paras and translated moments
         ...
         IF (use_Vff_tmp /= 2) THEN
            CALL mm_allocate(MEM_VFF_TMP,Vff_tmp1,lm_dim,INT(SIZE(LHS%box_paras),INTK))
            Vff_tmp1 = zero
            Vff_ptr => Vff_tmp1(:,:) 
         ELSE
            CALL mm_allocate(MEM_VFF_TMP,Vff_tmp2,lm_dim,INT(SIZE(LHS%box_paras),INTK))
            Vff_tmp2 = zero
            Vff_ptr => Vff_tmp2(:,:) 
         END IF
         ! set T_contractor pointers
