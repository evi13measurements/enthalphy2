...
MODULE mm_grid_types
   ...
   TYPE occ_list
      INTEGER(INTK) :: id   ! ID of box member (position in raw input array)
   ...
   TYPE grid_type
      INTEGER(INTK) :: bra      ! branch (one grid for each branch)
      INTEGER(INTK) :: level    ! level FIXME (should be constant for now)
...
!      CALL init_new_grid(overs(1)%bra,overs(1)%level,grid_head,xtbox)
      CALL init_new_grid(overs(1)%bra,0_intk,grid_head,xtbox)
...
!            CALL init_new_grid(over%bra,over%level,new_grid,xtbox)
            CALL init_new_grid(over%bra,0_intk,new_grid,xtbox)
            grid%next => new_grid
         END IF
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE init_new_grid(branch,level,grid,xtbox)
      ...
      INTEGER(INTK),   INTENT(IN)  :: branch, level
      TYPE(hi_lo),     INTENT(IN)  :: xtbox
      INTEGER(INTK) :: i,j,k
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE add_to_box(indx,box,grid)
      ...
      IMPLICIT NONE
      TYPE(grid_type), INTENT(INOUT) :: grid
      INTEGER(INTK),   INTENT(IN)    :: indx, box(3) 
