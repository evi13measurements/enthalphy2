...
!FIXME: would be nice to allow possibility of building rectangular-based too
   ...
   SUBROUTINE mm_get_SPLTSQ_T_matrix(LMAX,r_ab,T_matrix)
      ...
      INTEGER(INTK), EXTERNAL :: get_JMAX
...
!------------------------------------------------------------------------------
   ...
   ! triangular, we need only half the elements there
      ...
      INTEGER(INTK) :: L,m,J,k, p,q, pp,qq, u
      INTEGER(INTK) :: sign_k, sign_m, km
      ...
      L_loop: DO L = 0, LMAX 
         ...
         J_loop: DO J = L, JMAX(L,LMAX)
            ...
            positive_m: DO m = 0, L 
               ...
               DO k = (-J), MIN((-m-1),-1_intk)
                  ! (m+k)<0 and (m-k)>0
                  T_matrix(pp+k,q) = I_sh(u+(m+k)) + sign_m*I_sh(u+(-(m-k)))
               END DO
 
               DO k = MAX(-J,-m), MIN(-m,-1_intk)
                  ! (m+k)=0 and (m-k)>0
...
!
!      INTEGER(INTK) :: l,m,j,u,v
!      INTEGER(INTK) :: sign, hi
...
!------------------------------------------------------------------------------
   ...
   FUNCTION mm_contract_Tq(LMAX,vect,T_matrix)
     ...
     INTEGER(INTK) :: L,m, p,q,s, u, qmin,qmax
...
!------------------------------------------------------------------------------
   ...
   ! triangular, we need only half the elements there
      ...
      INTEGER(INTK) :: L,m,J,k, q, pp,qq, u, Jlast
      INTEGER(INTK) :: sign_m
...
!------------------------------------------------------------------------------
   ...
   FUNCTION mm_contract_multi_Tq(LMAX,vect,T_mats,ndim)
     ...
     INTEGER(INTK) :: L,m, p,q,r,s ,u, qmin,qmax
     REAL(REALK) :: fac 
...
subroutine pbc_contract_Tq(loc_q,siz_q,Tmatrix,siz_T,mm_q,lmax_q)
  ...
  implicit none
  integer(intk), intent(in) :: lmax_q, siz_q, siz_T
...
SUBROUTINE pbc_get_FLTSQ_T_matrix(LMAX,r_ab,T_matrix)
  ...
  integer(intk) :: l, j, qmin, qmax, siz
     ...
     rows_m: do m = l,-l,-1
        ...
        cols_j: do j = lmax,0,-1
           cols_k: do k = j,-j,-1
              ...
              if (mod(j,2_intk) .eq. 1) fac = -fac

              if (jk .le. lm) then
                 T_matrix(lm,jk) = T_matrix(lm,jk) * fac
              else if (mod(j-l,2_intk) .eq. 1) then
