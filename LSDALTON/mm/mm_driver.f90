...
!> \param lwork  Remaining length of work
   SUBROUTINE mm_allocate_Vff(scheme,WORK,LWORK)
      ...
      INTEGER(INTK) :: i, lm_dim, mms_dim, max_Lmin
...
!      CALL mm_allocate(MEM_RAW_VFF,Vff,lm_dim,mms_dim)
   ...
   CONTAINS
      ...
      SUBROUTINE set_v(v,NROWS,NCOLS,W,NWORK)
         ...
         IMPLICIT NONE
         INTEGER(INTK), INTENT(IN) :: NROWS, NCOLS, NWORK
...
!> \param lwork  Remaining length of work
   SUBROUTINE mm_get_grad_via_raw_potentials(scheme,grad,energy,numnuc,WORK,LWORK)
      ...
      INTEGER(INTK),      INTENT(IN)    :: LWORK, numnuc
...
SUBROUTINE mm_get_energy(ERI_energy,n_el,WORK,LWRK)
   ...
   CHARACTER(36) :: E_text
   LOGICAL       :: A, B
   INTEGER(INTK) :: INUM, KWRK, integral_type
   ...
   integral_type = mm_get_int_type(n_el)
...
SUBROUTINE mm_get_grad(n_el,grad,numnuc,WORK,LWRK)
   ...
   REAL(REALK)                  :: energy
   LOGICAL                      :: A, B
   INTEGER(INTK)                :: INUM, KWRK, integral_type, numnuc

   ! determine which type of calculation
   integral_type = mm_get_int_type(n_el)
   IF( (integral_type .NE. INTTYPE_TWO_EL ).AND. &
     & (integral_type .NE. INTTYPE_NUC_AT ).AND. &
     & (integral_type .NE. INTTYPE_NUC_EL )) THEN
   ...
   IF ( integral_type .EQ. INTTYPE_NUC_AT ) THEN
      !
   ...
   ELSEIF ( integral_type .EQ. INTTYPE_NUC_EL) THEN
      ! 
   ...
   ELSEIF ( integral_type .EQ. INTTYPE_TWO_EL ) THEN
      !
...
!
      if(fit%AUX) THEN
         ...
         CALL mm_disconnect_interface
         !
         !     get df-gradient contribution (rho_fit'|rho_fit)
         !
         !CALL mm_connect_interface(scheme,WORK,KWRK,LWRK)
         !CALL mm_get_grad_via_raw_potentials(scheme,grad,energy,numnuc,3,WORK(KWRK:LWRK),LWRK-KWRK+1)
         !CALL mm_disconnect_interface
      endif
   END IF
...
SUBROUTINE mm_getj_sync_slaves
   ...
   CALL MPI_Bcast(integral_type,1,MPI_INTEGER,master,MPI_COMM_WORLD,IERR)
...
FUNCTION SECOND()
   ...
   IMPLICIT NONE
   REAL(REALK) :: SECOND
   INTEGER(INTK) :: count, count_rate, count_max, tmp
