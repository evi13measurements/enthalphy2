SUBROUTINE init_standalone(LMAX,NBAST,FILE)
   ...
   IMPLICIT NONE
   INTEGER(INTK), INTENT(IN) :: LMAX, NBAST
   INTEGER(INTK) :: IDUMMY
   CHARACTER(*) FILE
...
SUBROUTINE mm_get_external_parameters(LMAX,NBAST)
   ...
   USE mm_global_paras_mod
   IMPLICIT NONE
   INTEGER(INTK), INTENT(OUT) :: LMAX, NBAST
   INTEGER(INTK) :: ID, LUINTM
   REAL(REALK)   :: RD
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_standalone_driver
      ...
      CHARACTER(6)  :: n_el
      INTEGER(INTK) :: LMAX, NBAST
      INTEGER(INTK) :: NLEVEL, TLMAX, ALGORITHM, LEXTRA, T_CONTRACTOR
