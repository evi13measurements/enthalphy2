...
!-------------------------------------------------------------------------------
   ...
   FUNCTION mm_box(centre,grain_inv,nbox_max)
      ...
      INTEGER(INTK), INTENT(IN) :: nbox_max 
      INTEGER(INTK) :: mm_box(3)
...
!  nothing is treated via boxes inside the classical radius
   ...
   FUNCTION mm_branch(extent,grain_inv,scheme)
      ...
      INTEGER(INTK) :: mm_branch 

      IF (scheme%branch_free) THEN 
         mm_branch = MAX(WS_MIN,INT(CEILING(scheme%cls_radius*grain_inv),INTK))
         ! we ensure branch is EVEN (for consistency with normal code)
         mm_branch = 2*((mm_branch+1)/2)
         stat_common_branch = mm_branch
      ELSE
         mm_branch = MAX(WS_MIN,INT(2*CEILING(extent*grain_inv),INTK))
      END IF
...
!      TYPE(box_mm_paras) :: tmp_paras(SIZE(raw_paras))
      INTEGER(INTK)      :: i, l_up, box(3)
      INTEGER(INTK),pointer :: tmp_map(:)
...
!      ALLOCATE(tmp_map(SIZE(raw_paras)))
      ...
      IF (.FALSE.) THEN
         ...
         CALL mm_allocate(MEM_BOX_QLM,pkd_paras,INT(SIZE(raw_paras),INTK))
         pkd_paras(:) = tmp_paras(:)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_shift_and_pack_paras(level,scheme,paras_in,paras_out)
      ...
      INTEGER(INTK)      :: i, l_down, l_up, box(3)
      ...
      IF (.FALSE.) THEN
         ! just build unpacked paras
         CALL mm_allocate(MEM_BOX_QLM,paras_out,INT(SIZE(paras_in),INTK))
         paras_out(:) = tmp_paras(:)
...
!-------------------------------------------------------------------------------
   ...
   FUNCTION test_new_parameters(test_item,ref_items,lo,hi)
      ...
      INTEGER(INTK) :: i, test_new_parameters
