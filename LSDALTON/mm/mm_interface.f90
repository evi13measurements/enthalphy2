...
!
#if defined (SYS_AIX) || defined (VAR_PGF77) || defined (VAR_LINSCA)
   ! for some strange reason, this fails on AIX.
...
MODULE mm_interface_mod
   ...
   INTEGER(INTK),      POINTER, SAVE :: mom2atom(:,:)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_connect_interface(scheme,WORK,KWRK,LWORK)
      ...
      INTEGER(INTK),      INTENT(IN)    :: LWORK
      INTEGER(INTK),      INTENT(INOUT) :: KWRK
      ...
      if(mynum==0) CALL mm_read_in_raw_data
      CALL mm_raw_data_sync
#else
      IF ( stat_do_grad) THEN 
         CALL mm_read_in_raw_data_der
      ELSE
         CALL mm_read_in_raw_data
      END IF
#endif
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_init_moments(LMAX,WORK,KWRK,LWORK)
      ...
      IMPLICIT NONE
      INTEGER(INTK), INTENT(IN)    :: LMAX, LWORK
      INTEGER(INTK), INTENT(INOUT) :: KWRK
      REAL(REALK),   INTENT(INOUT) :: WORK(LWORK)
...
#endif
      IF(stat_do_grad) THEN 
        CALL mm_allocate_mms_arrays(LMAX,WORK,LWORK/(LMAX+1)**2/(6+1),6_INTK)
        KWRK = KWRK + ((LMAX+1)**2)*n_mms%tot*(6 + 1)
      ELSE
         CALL mm_allocate_mms_arrays(LMAX,WORK,LWORK/(LMAX+1)**2,0_INTK)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_get_n_mms_from_file(LMAX_in)
     USE density_fitting
      ...
      INTEGER(INTK) :: LMAX, NBAST, ID
      INTEGER       :: LUINTM
      REAL(REALK)   :: RD   
      LOGICAL       :: ldumm1, ldumm2

      ldumm1=.false.
      ldumm2=.false.
      IF ( .NOT. stat_do_grad) THEN
        ! first we need the fitting information
         LUINTM = 0
      ...
      REWIND (LUINTM)
      IF ( .NOT. stat_do_grad) THEN
         READ (LUINTM) LMAX, NBAST, n_mms%elec, n_mms%nuc, ID, &
     &                 fit%NAUXMOM, ldumm1, ldumm2
      ELSE
         READ (LUINTM) LMAX, NBAST, n_mms%elec, n_mms%nuc, ID, &
     &                 fit%NAUXMOM, fit%LHS_AUX, fit%RHS_AUX
      END IF
      ...
      IF (LMAX /= LMAX_in) CALL lsQUIT('LMAX inconsistency in MM interface!',-1)
      CALL LSCLOSE(LUINTM,'KEEP')
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_allocate_mms_arrays(LMAX,WORK,maxmom,nucder)
      ...
      IMPLICIT NONE
      INTEGER(INTK), INTENT(IN) :: LMAX, maxmom, nucder
      ...
      IF ( stat_do_grad ) THEN
         CALL mm_allocate(MEM_RAW_QLM,mom2atom,n_mms%tot,2_INTK)
      END IF
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE mm_get_raw_data(scheme,LHS_mms,RHS_mms)
     ...
     USE density_fitting
      ...
      INTEGER(INTK) :: lo,hi, startreg, startaux, startnuc, endreg, endaux, endnuc
      ...
      ELSEIF (scheme%RHS_mm_range == REG_MIN_AUX .AND. fit%RHS_AUX) THEN
          ...
          lo = startaux
          hi = endaux
          raw_qlm(:,lo:hi) = -1.0D0*raw_qlm(:,lo:hi)
          lo = startreg
          hi = endaux
      ELSE
...
!   (0),(-1,0,1),(-2,-1,0,1,2)...
   ...
   SUBROUTINE mm_read_in_raw_data

     USE density_fitting

      IMPLICIT NONE

      REAL(REALK)   :: EXTP,PX,PY,PZ, SPH,DNS,THRES
      INTEGER(INTK) :: L,M, A,B, LMINA,LMINB, IBTCH, I, p,J, JFAC
      INTEGER(INTK) :: ID, IBUFFER, IEND, ISTART
      INTEGER       :: LUINTM, LUINTMI, LUINTMR
      INTEGER(INTK) :: II, IDONE, MMBUFLEN,MAXBUFI, MAXBUFR, MAXBUFN, MXBUFI,MXBUFR
      INTEGER(INTK) :: ALLOC_ERR, NREDI, NREDR, INI, INR
      REAL(REALK),pointer :: RBUF(:)
      INTEGER(INTK),pointer :: IBUF(:)
      LOGICAL :: USEMMBUF
      LOGICAL :: do_save_mm
      ...
      IF (USEMMBUF) THEN
         ...
         IF (MAXBUFI .NE. 8) CALL lsQUIT('wrong MAXBUFI in mm_read_in_raw_data',-1)
         IF (MAXBUFR .NE. 6) CALL lsQUIT('wrong MAXBUFR in mm_read_in_raw_data',-1)
...
!         IF (alloc_err .NE. 0) CALL lsQUIT('Memory allocation failed in mm_read_in_raw_data(II).',-1)
         ...
         ! first read electronic multipole moments into buffer
         readloop1: DO
            READ (LUINTMI) NREDI
            READ (LUINTMR) NREDR
            IF (NREDI .EQ. -1) EXIT readloop1
            IF (NREDR/6 .NE. NREDI/8 ) CALL lsQUIT ("ERROR while reading the MM from file",-1)
         ...
         readloop2: DO
            READ (LUINTMR) NREDR
            IF(NREDR.LE.0) EXIT readloop2
            READ (LUINTMR) (RBUF(II),II=1,NREDR)
            DO I = 1, NREDR/4
               p = n_mms%elec + I + idone
               II = (I-1)*4+1
               raw_qlm(1,p) = RBUF(II)
               raw_paras(p)%cntr(1)= RBUF(II+1)
               raw_paras(p)%cntr(2)= RBUF(II+2)
               raw_paras(p)%cntr(3)= RBUF(II+3)
               raw_paras(p)%batch = raw_paras(n_mms%elec)%batch + I + idone
            END DO
            idone = idone + NREDR/4
         END DO readloop2
      ...
      ELSE
       !
       ! original routine, without use of buffer
       !
       LUINTM = 0
       CALL LSOPEN(LUINTM,INPUT_FILE1,'UNKNOWN','UNFORMATTED')
       ! first read electronic multipole moments into core
       REWIND (LUINTM)

       readloop3: DO
          READ (LUINTM) L,M, A,B, LMINA,LMINB,IBTCH,I,EXTP,PX,PY,PZ,SPH,DNS
          IF (L .EQ. -1) EXIT readloop3
          IF (I > SIZE(raw_qlm,2)) then 
              write(lupri,*)'I',I,'qlm,2',SIZE(raw_qlm,2)
              CALL lsQUIT ('interface file error 1',lupri)
          ENDIF
          IF (((L+1)**2) > SIZE(raw_qlm,1)) CALL lsQUIT ('interface file error 2',lupri)
           ! indices to map moments to orbitals in J-matrix
          J_indices(I)%i_indx = A
          J_indices(I)%j_indx = B
           ! index to batches of unique centre and extent
       ...
       DO I = 1, n_mms%nuc 
          ...
          raw_paras(p)%batch = raw_paras(n_mms%elec)%batch + I
       END DO

       CALL LSCLOSE(LUINTM,'KEEP')

      END IF ! end of read-in with or without buffer

      raw_paras((n_mms%elec+1):)%Lmin = 0       ! spherical charge
      raw_paras((n_mms%elec+1):)%ext = zero     ! point charges
...
!Simen     Matched by an exual extent set up in Create_ODbatches (ODbatches.f90)
      raw_paras((n_mms%elec+1):)%ext = 1.d0     ! point charges
      J_indices((n_mms%elec+1):)%i_indx = 0     ! not relevant
      J_indices((n_mms%elec+1):)%j_indx = 0     ! not relevant
 
      dens((n_mms%elec+1):) = one

      !
      ! eventually the density was saved in an extra file, so we read it in
      !
!     CALL MMGETINFO(do_save_mm)
!     IF (do_save_mm) THEN  
      IF ( .NOT. stat_do_grad) THEN
         !
         ! read the density in
...
!         ELSE IF (fit%RHS_AUX .AND. .NOT. fit%LHS_AUX) THEN
...
!        write(*,*)'the end of the ordinary density',IEND
...
!           write(*,*)'dens for fitting',ISTART,n_mms%elec
            densfloop: DO I = ISTART, n_mms%elec
!              IF(fit%LHS_AUX .AND. .NOT. fit%RHS_AUX) THEN
!                 IBUFFER = J_indices(I)%j_indx
!              ELSE IF(.NOT. fit%LHS_AUX .AND. fit%RHS_AUX) THEN
!                 IBUFFER = J_indices(I)%j_indx - n_mms%nbast
!              ENDIF
               IF(fit%RHS_AUX) THEN
                  IBUFFER = J_indices(I)%j_indx
...
!-------------------------------------------------------------------------------
! Read in multipole moment data and the derivatives  from interface file.
! This routine is based on mm_read_in_raw_data and is only extended to also read in
! the derivatives. /Andreas Krapp
!
!  this is the new routine using a buffer to read the data
   SUBROUTINE mm_read_in_raw_data_der
 
     USE density_fitting

      IMPLICIT NONE

      REAL(REALK)   :: EXTP, PX, PY, PZ, SPH, DNS
      REAL(REALK)   :: SPHDER1, SPHDER2, SPHDER3, SPHDER4, SPHDER5, SPHDER6
      INTEGER(INTK) :: L,M, A,B, LMINA, LMINB, IBTCH, I, p
      INTEGER(INTK) :: ID, MMBUFLEN, MAXBUFI,MAXBUFR,MAXBUFN, MXBUFI, MXBUFR
      INTEGER       :: LUINTM, LUINTMI, LUINTMR
      INTEGER(INTK) :: ISCOOR1, ISCOOR2, ISCOOR3, ISCOOR4, ISCOOR5, ISCOOR6
      INTEGER(INTK) :: ICENTA, ICENTB,II,IDONE
      INTEGER(INTK) :: ALLOC_ERR, NREDI, NREDR, INI, INR
      INTEGER(INTK) :: nreg, naux, nnuc, nele, n
      REAL(REALK),   POINTER :: RBUF(:)
      INTEGER(INTK), POINTER :: IBUF(:)
      LOGICAL :: USEMMBUF
      ...
      IF (USEMMBUF) THEN
         IF (MAXBUFI .NE. 16) CALL lsQUIT('wrong MAXBUFI in mm_read_in_raw_data_der',-1)
         IF (MAXBUFR .NE. 12) CALL lsQUIT('wrong MAXBUFR in mm_read_in_raw_data_der',-1)
         IF (MAXBUFN .NE.  4) CALL lsQUIT('wrong MAXBUFN in mm_read_in_raw_data_der',-1)
         MXBUFI = MMBUFLEN*MAXBUFI
         MXBUFR = MMBUFLEN*MAXBUFR
         ! allocate memory for the buffer
!         ALLOCATE (IBUF(MXBUFI),STAT=ALLOC_ERR)
!         IF (alloc_err .NE. 0) CALL lsQUIT('Memory allocation failed in mm_read_in_raw_data_der(I).',-1)
!         ALLOCATE (RBUF(MXBUFR),STAT=ALLOC_ERR)
!         IF (alloc_err .NE. 0) CALL lsQUIT('Memory allocation failed in mm_read_in_raw_data_der(II).',-1)
         call mem_alloc_fmm(IBUF,MXBUFI)
         call mem_alloc_fmm(RBUF,MXBUFR)
         ...
         readloop: DO
            READ (LUINTMI) NREDI
            READ (LUINTMR) NREDR
            IF (NREDI .EQ. -1) EXIT readloop
            IF (NREDR/12 .NE. NREDI/16 ) CALL lsQUIT ("ERROR while reading the MM derivatives from file",-1)
            READ (LUINTMI) (IBUF(II),II=1,NREDI)
            READ (LUINTMR) (RBUF(II),II=1,NREDR)

            ! put buffer content to the corresponding places
            INR = 1
            DO INI = 1, NREDI-15, 16
               ! integer buffer
               L       = IBUF(INI)
               M       = IBUF(INI+1)
               A       = IBUF(INI+2)
               ...
               mom2atom(I,1) = ICENTA
               mom2atom(I,2) = ICENTB
               if (ISCOOR1.ne.0) raw_qlm_der(p,I,ISCOOR1) = SPHDER1
               if (ISCOOR2.ne.0) raw_qlm_der(p,I,ISCOOR2) = SPHDER2
               if (ISCOOR3.ne.0) raw_qlm_der(p,I,ISCOOR3) = SPHDER3
               if (ISCOOR4.ne.0) raw_qlm_der(p,I,ISCOOR4) = SPHDER4
               if (ISCOOR5.ne.0) raw_qlm_der(p,I,ISCOOR5) = SPHDER5
               if (ISCOOR6.ne.0) raw_qlm_der(p,I,ISCOOR6) = SPHDER6
            END DO
         END DO readloop

         IF(NREDI .NE. -1) CALL lsQUIT("ERROR2 while reading MM der from file",-1)
         ...
         readloop2: DO
            ...
            DO I = 1, NREDR/4
               ...
               II = (I-1)*4+1
               raw_qlm(1,p)         =  RBUF(II)
               raw_qlm_der(1,p,1)   = -RBUF(II)
               raw_qlm_der(1,p,2)   = -RBUF(II)
               raw_qlm_der(1,p,3)   = -RBUF(II)
               mom2atom(p,1)        =  idone+I
               mom2atom(p,2)        =  idone+I
               raw_paras(p)%cntr(1) =  RBUF(II+1)
               raw_paras(p)%cntr(2) =  RBUF(II+2)
               raw_paras(p)%cntr(3) =  RBUF(II+3)
               raw_paras(p)%batch   =  raw_paras(n)%batch + I + idone
               raw_paras(p)%Lmin    =  0       ! spherical charge
               raw_paras(p)%ext     =  zero    ! point charges
!Simen         Set up an artificial extent of 1 because the error in the electron-nuclear attraction
!Simen         is larger than the electronic repulsion contribution. Should be analyzed!
!Simen         Matched by an exual extent set up in Create_ODbatches (ODbatches.f90)
         ...
         IF (idone .NE. n_mms%nuc) CALL lsQUIT("ERROR3 while reading MM der from file",-1)

         IF (NREDR.NE.-2) CALL lsQUIT("ERROR4 while reading MM der from file",-1)
...
!         DEALLOCATE (IBUF)
      ...
      ELSE
         ...
         ! first read electronic multipole moments and derivatives into core
         REWIND (LUINTM)
         readloop3: DO
            READ (LUINTM) L,M, A,B, LMINA,LMINB,IBTCH,I,EXTP,PX,PY,PZ,SPH,DNS
            READ (LUINTM) ICENTA, ICENTB
            READ (LUINTM) ISCOOR1,ISCOOR2,SPHDER1,SPHDER2
            READ (LUINTM) ISCOOR3,ISCOOR4,SPHDER3,SPHDER4
            READ (LUINTM) ISCOOR5,ISCOOR6,SPHDER5,SPHDER6
            IF (L .EQ. -1) EXIT readloop3
            IF (I > SIZE(raw_qlm,2))          CALL lsQUIT ('interface file error 1',-1)
            IF (((L+1)**2) > SIZE(raw_qlm,1)) CALL lsQUIT ('interface file error 2',-1)
         ...
         DO I = 1, n_mms%nuc
            ...
            raw_qlm_der(1,p,1) = -raw_qlm(1,p)
            raw_qlm_der(1,p,2) = -raw_qlm(1,p)
            raw_qlm_der(1,p,3) = -raw_qlm(1,p)
            mom2atom(p,1)       = I
            mom2atom(p,2)       = I
            raw_paras(p)%Lmin   = 0     ! spherical charge
            raw_paras(p)%ext    = zero  ! point charges
...
!Simen      Matched by an exual extent set up in Create_ODbatches (ODbatches.f90)
  ...
  END SUBROUTINE mm_read_in_raw_data_der
...
! note that no record is kept of the packing for later "unpacking".
   ...
   SUBROUTINE mm_pack_raw_moments(raw_mms,dens,dens_thr,pkd_paras,pkd_qlm,ndim)
      ...
      CALL mm_allocate(MEM_RAW_QLM,pkd_qlm,INT(SIZE(raw_mms%qlm,1),INTK),nbatch)
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE get_normalised_qlm_der(scheme,LHS_qlm,RHS_qlm,LHS_qlm_der,RHS_qlm_der)
      ...
      REAL(REALK), INTENT(INOUT) :: LHS_qlm(:,:), RHS_qlm(:,:)
      INTEGER(INTK) :: LMAX, i, qlm_dim, iscoor
