...
!> 
   ...
   do i = 1, ndim
      if (abs(eival_i(i)/eival_denom(i)) > 1.0d-5) then
   ...
   !> \author L. Thogersen
   !> \date 2003
   SUBROUTINE my_DSYGV(N,A,B,eigval,wrk,lwrk,DESC)
   use precision
     implicit none
   ! eigenvectors are in A.
     ...
     !> The length of the array wrk. lwrk should be >= max(1,3*N-1)
     integer,          intent(in)    :: lwrk
     !> Work space
     real(realk)     , intent(inout) :: wrk(lwrk)
     !> Should contain name of routine from which my_DSYGV is called
     character(20),     INTENT(IN)    :: desc
     integer    :: ierr
   !#if 0 && defined(SYS_AIX)
   !  !aix/ESSL is different than anything else :-/
   !  real(realk) :: hes(N,N) 
   !  hes = A
   !  call DSYGV(1,hes,N,B,N,eigval,A,N,N,wrk,lwrk)
   !#else
...
!          wall0 = darr(1)*1.d0 + darr(2)/1.d6
!      end if
!      TIMCPU = etime(tarray) - TCPU0
!      call gettimeofday(darr,err)
!      TIMWAL = darr(1)*1.d0 + darr(2)/1.d6 - wall0
!#else
!      real(realk),PARAMETER :: D0 = 0.0D0
!      logical    :: first = .true.
...
! We don't count milliseconds.
   ...
   walltime = dble(dateandtime(7))                      !Seconds counted
...
!> 
!> THE PROGRAM IS SET UP TO HANDLE 5 COLUMNS/PAGE WITH A 1P,5D24.15 FORMAT
...
implicit none 
      ...
      real(realk) :: amax
      real(realk), PARAMETER :: ZERO=0.D00
      integer, parameter     :: KCOLP=4, KCOLN=6
      real(realk), PARAMETER :: FFMIN=1.D-3, FFMAX = 1.D3
