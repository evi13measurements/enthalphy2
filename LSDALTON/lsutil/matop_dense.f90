...
!> \brief See mat_init in mat-operations.f90
  subroutine mat_dense_init(A,nrow,ncol)
     ...
     allocate(A%elms(A%nrow * A%ncol &
                   & * merge(2,1,A%complex)))
     nsize = size(A%elms)*mem_realsize
...
!    enddo
     ...
     if (alpha.ne.1.0d0) call mat_dense_scal(alpha,a)
...
!> \brief See mat_mpicopy in mat-operations.f90
  subroutine mat_dense_mpicopy(a,slave,master)
    use lsmpi_mod
     ...
     IF(SLAVE)call mat_dense_init(a,a%nrow,a%ncol)
     i = a%nrow*a%ncol
     call LS_MPIBCAST(a%elms,i,Master)
...
!    enddo
     ...
     if (alpha.ne.1.0d0) call mat_dense_scal(alpha,b)
...
!> \brief See mat_dmul in mat-operations.f90
  subroutine mat_dense_dmul(a,b,transb,alpha,beta,c) 
     ...
     elseif (transb == 't' .or. transb == 'T') then
       ...
       STOP 'unknown format in mat_dense_dmul'
     endif
...
!> \brief See mat_outdia_sqnorm2 in mat-operations.f90
  function mat_dense_outdia_sqnorm2(a)
     ...
     mat_dense_outdia_sqnorm2 = 0.0d0
     n = 1
     do i = 1,a%nrow*a%ncol
...
!> \brief See mat_diag_f in mat-operations.f90
  subroutine mat_dense_diag_f(F,S,eival,Cmo)
    ...
    ndim = s%nrow
    ALLOCATE(wrk((8+2)*Ndim),tmp(Ndim*Ndim))
    do i = 1,ndim*ndim
       tmp(i) = S%elms(i)
    enddo
    call  mat_dense_assign(Cmo, F)
    call my_DSYGV(ndim,Cmo%elms,tmp,eival,wrk,size(wrk),"DENSE_GET_DENS_SOL  ")
    DEALLOCATE(wrk,tmp)
...
!> \brief See mat_max_elm in mat-operations.f90
  subroutine mat_dense_max_elm(a, val)
  implicit none
     type(matrix),intent(in)  :: a
     real(realk), intent(out) :: val
     integer                  :: i
...
!> \brief See mat_dE_dmu in mat-operations.f90
  FUNCTION mat_dense_dE_dmu(Fnew,Fdamp,SDS,Cmo,nocc)
   ...
   ![dE/dC]ij = 4[FC]ij  
     ...
     call mat_dense_mul(Fnew,Cmo_occ,'n','n',1.d0,0.d0,X)
     call mat_dense_mul(Cmo,X,'t','n',4.0d0,0.d0,CTFC)
   ![dC/dmu]ij = -[dC/dmu]ji => [dC/dmu]ij = 0 for i=j
   ![dC/dmu]ij = [C^TSDSC]ij/(C^TFdampC_ii - C^TFdampC_jj))
     call mat_dense_init(tSDS,ndim,nocc)
     call mat_dense_mul(SDS,Cmo_occ,'n','n',1.d0,0.d0,X)
     call mat_dense_mul(cmo,X,'t','n',1.d0,0.d0,tSDS)
...
!> \brief See mat_section in mat-operations.f90
  subroutine mat_dense_section(A,from_row,to_row,from_col,to_col,Asec)
     ...
     if (SIZE(Asec%elms) < Asec%nrow*Asec%ncol) STOP 'too little memory allocated in mat_dense_section'
...
!> \brief See mat_precond in mat-operations.f90
  subroutine mat_dense_precond(M,x,xprec)
    ...
    do i = 1, x%nrow
       if (abs(M%elms((i-1)*M%nrow+i)) > 1.0d-9) then
...
!> \brief See mat_mo_precond in mat-operations.f90
  subroutine mat_dense_mo_precond(nocc,omega,Eorb_final,X_MO)
    ...
    do j = 1,nocc  !columns
      do i = nocc+1,X_MO%nrow  !rows
        ...
        !NOTE TO SELF: WHY the 2.d0s and not 1.d0????
  ...
  subroutine mat_dense_new_mo_precond(nocc,omega,Eorb_final,Xp_MO,Xm_mo)
    ...
    do j = nocc+1,Xp_MO%ncol  !columns
      do i = 1,nocc          !rows
        !dia = E[2]_dia + omega*S[2]_dia
        aa=2d0*(Eorb_final(j) - Eorb_final(i))*(Eorb_final(j)- Eorb_final(i))-2d0*omega*omega
        !dia = 2.0d0*(Eorb_final(j) - Eorb_final(i)) - omega * 2.d0
        dia=Xp_mo%elms((j-1)*Xp_mo%nrow+i)
  ...
  subroutine mat_dense_new_complex_precond(nocc,omega,gammma,Eorb_final,Xp_MO,Xm_mo,Xpi_mo,xmi_mo)
    ...
    type(matrix)           :: xp,xm,xpi,xmi,Ii
    real(realk) :: dia,aa,a,b,c,d,ab
    integer :: i,j,nvirt,ndim
    ...
    if (symmetry == 1 .or. symmetry == 2) then !Symmetric or antisymmetric X_AO
       ...
       do j = 1,ndim   !columns
         do i = 1,ndim  !rows
           ...
           if (abs(denom) < 1.0d-1) then !Do not divide by too small elements
              denom = denom*1.0d-1/(abs(denom)) !Keep the sign on denominator
           endif
           X_AO%elms((j-1)*ndim+i) = X_AO%elms((j-1)*ndim+i)/(denom)
         enddo
       enddo
...
!!    real(realk), parameter :: alpha = 4.20d0
...
!    
!    !Convert framework:
!    call mat_dense_to_full(hes, 1.0d0, hes_full)
...
!
!    call mat_dense_to_full(X_AO, 1.0d0, res)
...
!!STOP
!    !Convert framework:
!    call mat_dense_set_from_full(x_full,1.0d0,X_AO)
!STOP
...
!> \brief See mat_identity in mat-operations.f90
  subroutine mat_dense_identity(Id)
    ...
    do j = 1,Id%ncol
      do i = 1,Id%nrow
        if (i == j) then
          Id%elms((j-1)*Id%nrow+i) = 1.0d0
        else
          Id%elms((j-1)*Id%nrow+i) = 0.0d0
        endif
      enddo
    enddo
...
!> \brief See mat_write_to_disk in mat-operations.f90
  ...
  !                  A (matrix) - The matrix that should be written to disk
    ...
    if (.not.ASSOCIATED(A%elms)) STOP 'A in mat_dense_WRITE_TO_DISK non&
                                    &-existant'
 ...
 !      enddo
    ...
    if (symmetry == 'a' .OR. symmetry == 'A') then
       do n = 1, MAT%nrow
          do m = 1, n
             if (n == m) then
                MAT%elms((n-1)*MAT%nrow + m) = 0.0d0 !Diagonal element are 0 (antisym. matrix)
             else
                k = k + 1
...
!> \brief See stat_allocated_memory in mat-operations.f90
  subroutine dens_stat_allocated_memory(size)
     implicit none
     integer, intent(in) :: size
    
     mem_allocated_type_matrix = mem_allocated_type_matrix + size
     max_mem_used_type_matrix = MAX(max_mem_used_type_matrix,mem_allocated_type_matrix)
     !Count also the total memory:
     mem_allocated_global = mem_allocated_global  + size
     max_mem_used_global = MAX(max_mem_used_global,mem_allocated_global)
  
  end subroutine dens_stat_allocated_memory

!> \brief See stat_deallocated_memory in mat-operations.f90
  subroutine dens_stat_deallocated_memory(size)
     implicit none
     integer, intent(in) :: size
  
     mem_allocated_type_matrix = mem_allocated_type_matrix - size
     if (mem_allocated_type_matrix < 0) then
        call LSQUIT('Error concerning allocation - probably integer overflow!', mat_lu)
     endif
     !Count also the total memory:
     mem_allocated_global = mem_allocated_global - size
  ...
  end subroutine dens_stat_deallocated_memory
...
!    integer :: info,i,j
!
!!   call mat_dense_copy(1d0,A,B)
!    call mat_dense_assign(B,A)
!    call dpotrf('L',B%nrow,B%elms,B%nrow,info)
!    if(info.ne.0) stop 'exit in mat_dense_cholesky!'
...
!    if(info.ne.0) stop 'exit in mat_dense_inverse_triang!'
!
!    return
...
!       call mat_dense_mul(B,A,'T','N',1d0,0d0,T)
!       call mat_dense_mul(T,B,'N','N',1d0,0d0,C)
!    else
!       call mat_dense_mul(B,A,'N','N',1d0,0d0,T)
!       call mat_dense_mul(T,B,'N','T',1d0,0d0,C)
!    endif
!
...
!    if(info.ne.0) stop 'exit in mat_dense_to_minus_one_half!'
...
!> \brief See mat_report_sparsity in mat-operations.f90
  ...
  integer      :: i
       ...
       do i=1, A%ncol*A%nrow
         if (abs(A%elms(i)).gt.1d-9) nnz = nnz+1d0
       enddo
