...
!mem_realsize is size of real - specified in memory_handling
    call lsdens_stat_allocated_memory(nsize)
...
!> \param a type(lsmatrix) that should be freed
  subroutine lsmat_dense_free(a)
    ...
    nsize = SIZE(a%elms)*mem_realsize
!mem_realsize is size of real - specified in memory_handling
    call lsdens_stat_deallocated_memory(nsize)
    DEALLOCATE(a%elms)
    NULLIFY(a%elms)
...
!> \param b The scaled output type(lsmatrix).
  subroutine lsmat_dense_copy(alpha,a,b)
    ...
    if (alpha.ne.1.0d0) call lsmat_dense_scal(alpha,b)
...
!> \param A Matrix which should be written on disk
  subroutine lsmat_dense_WRITE_TO_DISK(iunit,A)
    ...
    if (.not.ASSOCIATED(A%elms)) STOP 'A in lsmat_dense_WRITE_TO_DISK non&
         &-existant'
...
!> \brief Count allocated memory for type(lsmatrix) counted as type(matrix)
!> \author T. Kjaergaard
!> \date 2010
!> \param size Number of real(realk) elements that have been allocated
  subroutine lsdens_stat_allocated_memory(size)
  use memory_handling
    implicit none
    integer, intent(in) :: size

    !mem_allocated_type_matrix is a variable in memory_handling
    !max_mem_used_type_matrix is a variable in memory_handling
    !mem_allocated_global  is a variable in memory_handling
  ...
  end subroutine lsdens_stat_allocated_memory

!> \brief Count deallocated memory for type(lsmatrix) counted as type(matrix)
!> \author T. Kjaergaard
!> \date 2010
!> \param nsize Number of real(realk) elements that have been deallocated
  subroutine lsdens_stat_deallocated_memory(size)
  use memory_handling
    implicit none
    integer, intent(in) :: size
    !mem_allocated_type_matrix is a variable in memory_handling
    ...
    if (mem_allocated_type_matrix < 0) then
       call LSQUIT('Error concerning memory allocation, probably integer overflow!',-1)
    endif
    !Count also the total memory:
    mem_allocated_global = mem_allocated_global - size
    if (mem_allocated_global < 0) then
       call LSQUIT('Error concerning allocation - probably integer overflow!',-1)
    endif

  end subroutine lsdens_stat_deallocated_memory
