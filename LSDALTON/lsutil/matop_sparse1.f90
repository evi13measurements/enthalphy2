...
module matrix_operations_sparse1
  ...
  real(realk) :: zero = 1.d-9   
  !> Threshold for elements to neglect, squared
  real(realk) :: zero2 = 1.d-18
...
!> \param afull a on full matrix form
  subroutine mat_sparse1_to_full(a, alpha, afull)
     ...
     if (.not.ASSOCIATED(a%selm1)) STOP 'a in mat_sparse_to_full&
                                     & non-existant'
...
! NOT TESTET
    ...
    call LS_MPIBCAST(a%idata(nelm),Master)
    IF(SLAVE)call sp_allocate(a,a%idata(nelm))
    call LS_MPIBCAST(a%idata(pack),Master)
    call LS_MPIBCAST(a%idata(symm),Master)
    do i=1,A%idata(Nelm)
      call LS_MPIBCAST(A%selm1(i)%i,Master)
      call LS_MPIBCAST(A%selm1(i)%j,Master)
      call LS_MPIBCAST(A%selm1(i)%val,Master)
    enddo
...
!>
  function mat_sparse1_TrAB(a,b)
     ...
     if(.not.ASSOCIATED(a%selm1).or..not.ASSOCIATED(b%selm1)) &
...
!> \param C The product of A and B scaled by alpha and added with beta*C(in)
  subroutine mat_sparse1_mul(A,B,transa, transb,alpha,beta,C)
     ...
     if (ABS(beta) > zero2 .and. .not. ASSOCIATED(c%selm1)) &
...
!***********************************************
     if (alpha > 1.0d0 + zero .or. alpha < 1.d0 - zero) then
...
!> \return The square norm of the off-diagonal elements of A
  function mat_sparse1_outdia_sqnorm2(A)
     ...
     mat_sparse1_outdia_sqnorm2 = 0.0D0
     do i=1,A%idata(Nelm)
       if (A%selm1(i)%i /= A%selm1(i)%j) then
...
!> \brief See mat_diag_f in mat-operations.f90
  ...
  ! diagonalizaton carried out in full basis
     call mat_sparse1_to_full(S, 1.d0, tmp)
     call mat_sparse1_to_full(F,1.d0,Cmo_full)
  !
     call my_DSYGV(ndim,Cmo_full,tmp,eival,wrk,size(wrk),"SPARSE1_GET_DENS_SOL")
  !
     call mat_sparse1_set_from_full(Cmo_full,1.d0,Cmo)
  !
     DEALLOCATE(wrk,tmp,Cmo_full)
  ...
  !   call mat_sparse1_to_full(S, 1.d0, tmp)
  !   call mat_sparse1_to_full(X,1.d0,Cmo_full)
  ...
  !   call mat_sparse1_set_from_full(Cmo_full,1.d0,Cmo)
  !   call mat_sparse1_set_from_full(Dnew_full,0.5d0,Dnew)
  !   call mat_sparse1_set_from_full(eig,1.0d0,eival)  
...
call mat_sparse1_print(Cmo_occ, 1, ndim, 1, nocc, mat_lu)
     call mat_sparse1_mul(Fnew,Cmo_occ,'n','n',1.d0,0.d0,X)
     call mat_sparse1_mul(Cmo,X,'t','n',4.0d0,0.d0,CTFC)
   ...
   ![dC/dmu]ij = [C^TSDSC]ij/(C^TFdampC_ii - C^TFdampC_jj))
     ...
     call mat_sparse1_mul(SDS,Cmo_occ,'n','n',1.d0,0.d0,X)
     call mat_sparse1_mul(Cmo,X,'t','n',1.d0,0.d0,tSDS)
     ...
     call mat_sparse1_mul(Fdamp,Cmo,'n','n',1.d0,0.d0,X)
  ...
  subroutine mat_sparse1_mo_precond(nocc,omega,Eorb_final,X_MO)
    ...
    do i = 1,X_MO%idata(nelm)
      if (X_MO%selm1(i)%i > nocc .and. X_MO%selm1(i)%j <= nocc) then
        ...
        dia = 2.0d0*(Eorb_final(X_MO%selm1(i)%i) - Eorb_final(X_MO%selm1(i)%j)) &
            & + omega * 2.d0
        X_MO%selm1(i)%val = X_MO%selm1(i)%val/dia
      elseif (X_MO%selm1(i)%j > nocc .and. X_MO%selm1(i)%i <= nocc) then
        !upper right block (ia block)
        !dia = E[2]_dia + omega*S[2]_dia
        dia = 2.0d0*(Eorb_final(X_MO%selm1(i)%j) - Eorb_final(X_MO%selm1(i)%i)) &
            & - omega * 2.d0
        X_MO%selm1(i)%val = X_MO%selm1(i)%val/dia
      endif
    enddo
  ...
  subroutine mat_sparse1_retrieve_block(A,fullmat,fullrow,fullcol,insertrow,insertcol)
    ...
    do i = insertrow, insertrow+fullrow-1
       do j = insertcol, insertcol+fullcol-1
          ...
          enddo
          if(.NOT.found)THEN
                fullmat(i-insertrow+1,j-insertcol+1) = 0.D0
          endif
       enddo
    enddo
...
!> \brief See mat_scal_dia in mat-operations.f90
  subroutine mat_sparse1_scal_dia(alpha,A)
    ...
    type(matrix), intent(inout) :: A
    integer :: i
    if (.not.ASSOCIATED(A%selm1)) STOP 'input in mat_sparse1_SCAL_DIA &
                                     & non-existant'
    do i=1,A%idata(Nelm)
      if (A%selm1(i)%i == A%selm1(i)%j) then
        A%selm1(i)%val = alpha*A%selm1(i)%val
...
!> \param A The sparse matrix that should be written to disk
  subroutine mat_sparse1_write_to_disk(iunit,A)
    ...
    if (.not.ASSOCIATED(A%selm1)) STOP 'A in mat_sparse1_WRITE_TO_DISK non&
                                    &-existant'
...
!
!          if(i.eq.j) then
!             if(sum.le.0d0) stop 'choldc failed'
!             j_and_val%j=i
...
!          if(j.eq.i) then
!             j_and_val%j=i
!             j_and_val%value=1d0/current%row%value
!             call row_insert(C%col(i),j_and_val)
!             exit
!          endif
!          current=>current%next
...
!       call mat_sparse1_mul(B,A,'T','N',1d0,0d0,D)
!       call mat_sparse1_mul(D,B,'N','N',1d0,0d0,C)
!    else
!       call mat_sparse1_mul(B,A,'N','N',1d0,0d0,D)
!       call mat_sparse1_mul(D,B,'N','T',1d0,0d0,C)
!    endif
!
