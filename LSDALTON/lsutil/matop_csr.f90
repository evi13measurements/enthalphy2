...
! HACK
  real(realk),parameter :: zeroCSR = 1.d-20
...
!
    ...
    !calculate nnz
#ifdef VAR_MKL
    call mkl_dcsradd('n', request, sort, m, n, a_temp%val, a_temp%col, a_temp%row, beta, b%val, b%col, b%row, ddummy, idummy, c%row, idummy, info)
#endif
    ...
    request = 2
#ifdef VAR_MKL
    call mkl_dcsradd('n', request, sort, m, n, a_temp%val, a_temp%col, a_temp%row, beta, b%val, b%col, b%row, c%val, c%col, c%row, c%nnz, info)
#endif
!assert(info!=0)
...
!
    ...
    !calculate nnz
#ifdef VAR_MKL
    call mkl_dcsrmultcsr(transa, request, sort, m, n, k, a%val, a%col, a%row, b_temp%val, b_temp%col, b_temp%row, ddummy, idummy, c%row, idummy, info)
#endif
!RA: assert(info!=0)
    !nnz can now be found as the last element in c's row - 1
    nnz = c%row(n+1)-1
...
#ifdef VAR_MKL
    call mkl_dcsrmultcsr(transa, request, sort, m, n, k, a%val, a%col, a%row, b_temp%val, b_temp%col, b_temp%row, c%val, c%col, c%row, c%nnz, info)
#endif
    !Stinne fix: multiply by alpha if different from one!
    if (abs(1.0d0-alpha) > 1.0d-7) then
       call mat_csr_scal(alpha,c)
    endif

!RA: assert(info!=0)
    !print *, "Will clean result matrix, nnz is ", c%nnz
    call mat_csr_cleanup(c%val, c%col, c%row, n, c%nnz)
...
!> \brief See mat_to_full in mat-operations.f90
  subroutine mat_csr_to_full(a, alpha, afull)
     ...
     afull(1) = 0.0d0 !Stinne fix - some compilers complain about intent(out)
                      !without MKL present
     job(1) = 1
     job(2) = 1
     job(3) = 1
  ...
  subroutine mat_csr_retrieve_block_full(A,fullmat,fullrow,fullcol,insertrow,insertcol)
    ...
    type(Matrix), intent(in) :: A
    integer                     :: i, current_row, j, current_col, k
...
!          fullmat(i-insertrow+1,j-insertcol+1) = A%elms((j-1)*A%nrow+i)
  ...
  subroutine mat_csr_report_sparsity(A,sparsity)
    ...
    nnz = a%nnz
    call mat_csr_cleanup(A%val, A%col, A%row, A%nrow, A%nnz)
    !if (nnz /= a%nnz) then
