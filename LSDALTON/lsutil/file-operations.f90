...
implicit none
        ...
        integer       :: LOUTFL,IERR,LENOUT,LWRKDR,LENWRK,IOS,i
        character(len=80) :: filename, filestatus, fileformat 
        character(len=20) :: outfil
        character(len=60) :: wrkdir
        logical :: fileexists, lunexists
...
!
     ...
     !    CALL FLSHFO(LUPRI)
      ...
      IF (DISP == 'KEEP') THEN
         ...
         if (ios /= 0) call lsquit('Something wrong when closing/deleting file',-1)
         IUNTAB(LUNIT) = 0
         LUNIT = -20000-LUNIT
      else
