...
!>
  subroutine mat_unres_dense_set_from_full3(afull,alpha,a)
    ...
    if (alpha.ne.1.0d0) call mat_unres_dense_scal(alpha,a)
...
!> \brief See mat_mpicopy in mat-operations.f90
  subroutine mat_unres_dense_mpicopy(a,slave,master)
    use lsmpi_mod
     ...
     IF(Slave)call mat_unres_dense_init(a,a%nrow,a%ncol)
     i = a%nrow*a%ncol
     call LS_MPIBCAST(a%elms,i,Master)
     call LS_MPIBCAST(a%elmsb,i,Master)
...
!> \brief See mat_copy in mat-operations.f90
  subroutine mat_unres_dense_copy(alpha,a,b)
     ...
     if (alpha.ne.1.0d0) call mat_unres_dense_scal(alpha,b)
...
!> \brief See mat_dotproduct in mat-operations.f90
  function mat_unres_dense_dotproduct(a,b)
     ...
     mat_unres_dense_dotproduct = 0.0d0
     !do i = 1,a%nrow*a%ncol
...
!> \brief See mat_outdia_sqnorm2 in mat-operations.f90
  function mat_unres_dense_outdia_sqnorm2(a)
     ...
     mat_unres_dense_outdia_sqnorm2 = 0.0d0
     n = 1
     do i = 1,a%nrow*a%ncol
...
!> \brief See mat_diag_f in mat-operations.f90
  subroutine mat_unres_dense_diag_f(F,S,eival,Cmo)
    ...
    call my_DSYGV(ndim,Cmo%elms,tmp,eival(1:ndim),wrk,size(wrk), &
    ...
    call my_DSYGV(ndim,Cmo%elmsb,tmp,eival(ndim+1:2*ndim),wrk,size(wrk), &
...
!> \brief See mat_column_norm in mat-operations.f90
  FUNCTION mat_unres_dense_column_norm(Mat,ncol,from_row,to_row)
    !Returns the sum of the elements A(from_row:to_row,col_num) squared
     ...
     mat_unres_dense_column_norm = 0.d0
...
!> \brief See mat_precond in mat-operations.f90
  subroutine mat_unres_dense_precond(M,x,xprec)
    ...
    do i = 1, x%nrow
       ...
       if (abs(M%elmsb((i-1)*M%nrow+i)) > 1.0d-9) then
    ...
    do j = 1,Id%ncol
      do i = 1,Id%nrow
        if (i == j) then
          ...
          Id%elmsb((j-1)*Id%nrow+i) = 1.0d0
        else
          Id%elms((j-1)*Id%nrow+i) = 0.0d0
          Id%elmsb((j-1)*Id%nrow+i) = 0.0d0 
        endif
      enddo
    enddo
...
!> \brief Create only element in alpha part.
  subroutine mat_unres_dense_create_elm_alpha(i,j,val,A)
    implicit none
    integer, intent(in) :: i,j
    real(realk), intent(in) :: val
    type(Matrix), intent(inout) :: A
  ...
  end subroutine mat_unres_dense_create_elm_alpha
...
!=======================================================================
  ...
  if (size(elm) /= 2) stop 'wrong dimension of elm in&
                           & mat_unres_dense_get_elm'
...
!> \brief See mat_retrieve_block in mat-operations.f90
  subroutine mat_unres_dense_retrieve_block(A,fullmat,fullrow,fullcol,insertrow,insertcol)
    ...
    do j = insertcol, insertcol+fullcol-1
       do i = insertrow, insertrow+fullrow-1
           fullmat(i-insertrow+1,j-insertcol+1) = 0.5d0*(A%elms((j-1)*A%nrow+i)+A%elmsb((j-1)*A%nrow+i))
       enddo
    enddo
       ...
       do n = 1, MAT%nrow
          do m = 1, n
             if (n == m) then
                ...
                MAT%elmsb((n-1)*MAT%nrow + m) = 0.0d0 !Diagonal element are 0 (antisym. matrix)
             else
                k = k + 1
...
!> \brief See stat_allocated_memory in mat-operations.f90
  subroutine unres_dens_stat_allocated_memory(size)
     implicit none
  ...
  end subroutine unres_dens_stat_allocated_memory

!> \brief See stat_deallocated_memory in mat-operations.f90
  subroutine unres_dens_stat_deallocated_memory(size)
     implicit none
     integer, intent(in) :: size
  ...
  end subroutine unres_dens_stat_deallocated_memory
