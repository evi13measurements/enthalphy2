...
type AvItem
   ...
   !==============
      ...
      TYPE(CFG_set) :: cfg_settings(2) = (/ &
           & CFG_set( 0.98D0, 10, 0.08d0),  &  ! Hartree-Fock thresholds
           & CFG_set( 0.975D0, 7, 0.03d0) /)   ! DFT Thresholds 
...
implicit none
   ...
   SUBROUTINE queue_init(av,queue)
      ...
      queue%metric = 0.0d0 !Stinne 24/10-06
...
!FIXME: should this be done for EDIIS???
   ...
   SUBROUTINE queue_on_disk(queue,queue_lu,ndim)
      ...
      if (queue%used_entries > 0) then
         ...
         do i = 1, queue%used_entries
            call mat_write_to_disk(queue_lu,queue%D(i))
            CALL mat_free(queue%D(i))
         enddo
         do i = 1, queue%used_entries
            call mat_write_to_disk(queue_lu,queue%F(i))
            CALL mat_free(queue%F(i))
         enddo
      endif
   ...
   SUBROUTINE queue_from_disk(queue,queue_lu,ndim)
      ...
      if (queue%used_entries > 0) then
         ...
         do i = 1, queue%used_entries
            ...
            call mat_read_from_disk(queue_lu,queue%D(i))
         enddo
         do i = 1, queue%used_entries
            CALL mat_init(queue%F(i), ndim, ndim)
            call mat_read_from_disk(queue_lu,queue%F(i))
         enddo
   ...
   SUBROUTINE add_to_queue(av, F, D, S, E, grad, queue)
      ...
      IF(av%save_gradient) THEN
        ...
        queue%DIIS_Amat(av%diis_pos,av%diis_pos) = 2.0d0*gradsqnorm   
        i = av%diis_pos - 1
        ...
        do
          ...
          queue%DIIS_Amat(i,av%diis_pos) = 2.0d0*gdotprod
   ...
   subroutine queue_update_TrDSDS(av,S,queue)
      ...
      call mat_mul(S,queue%D(pos),'n','n',1.d0,0.d0,SDpos)
      call mat_mul(SDpos,S,'n','n',1.d0,0.d0,SDS)
...
! Reallocate stuff in queue
     if (nentries > 0) then
        ...
        do i = nentries,1,-1
          queue%F(i) = save(i)
          if (av%save_gradient) save(i) = queue%gd(pos)
          pos = pos - 1
          if (pos == 0) pos = queue%used_entries
        enddo
        do i = 1,nentries
          if (av%save_gradient) queue%gd(i) = save(i)
...
!     N = queue%used_entries    !number of stored fock and density matrices
     ...
     !** SDmuS
     call mat_mul(Din,S,'n','n',1.d0,0.d0,tmp)
     call mat_mul(S,tmp,'n','n',1.d0,0.d0,SDinS)
