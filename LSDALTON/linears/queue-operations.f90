...
!> \paramndim Size of matrices to be stored (ndim x ndim)
!> \param disk True if vectors in queue should be kept on disk instead of in core
   SUBROUTINE modFIFO_init(queue,qsize,ndim,disk)
      implicit none
      TYPE(modFIFO) :: queue
      ...
      integer :: i, j
         ...
         if (queue%disk) then
            ...
            call mat_init(queue%tempD,ndim,ndim)
            call mat_init(queue%tempF,ndim,ndim)
         else
...
!> \param exppoint True if the added pair of matrices should be used as expansion point
   ...
   implicit none
      ...
      call mat_init(queue%Fmatrices(queue%qcounter),ndim,ndim)
      call mat_init(queue%Dmatrices(queue%qcounter),ndim,ndim)
      queue%Fmatrices(queue%qcounter) = F
      queue%Dmatrices(queue%qcounter) = D
...
!> \param Dpointer Pointer to retrieved D matrix
   ...
   implicit none
      ...
      if (i == 0) then !Get the expansion point
         rewind(queue%iF_exp)
         rewind(queue%iD_exp)
         call mat_read_from_disk(queue%iF_exp,queue%tempF)
         call mat_read_from_disk(queue%iD_exp,queue%tempD)
         ...
         call mat_read_from_disk(queue%iFarray(i)%p,queue%tempF)
         call mat_read_from_disk(queue%iDarray(i)%p,queue%tempD)
         Fpointer => queue%tempF
         Dpointer => queue%tempD
...
!> \param ndim Number of basis functions/size of matrices
   SUBROUTINE fifoqueue_on_disk(queue,lun,ndim)
      ...
      do i = 1, queue%offset
         ...
         call mat_write_to_disk(lun,queue%Farray(i)%p)
         call mat_write_to_disk(lun,queue%Darray(i)%p)
      enddo
...
!> \param ndim Number of basis functions/size of matrices
   SUBROUTINE fifoqueue_from_disk(queue,lun,ndim)
      ...
      do i = 1, offset
         call mat_read_from_disk(lun,F)
         call mat_read_from_disk(lun,D)
         call add_to_modFIFO(queue, F, D, 0.0d0, .false.)
      enddo
