...
!> \param weights The weights for averaging Fock/KS and density matrices
  subroutine DIIS_WEIGHTS(av,msize,queue,weights)
    ...
    real(realk) :: piv(msize+1),bvec(msize+1)
    integer :: i,ierr
    ...
    bvec(1:msize) = 0.0d0
    bvec(msize+1) = -1.0d0
    call DGESV(msize+1,1,Amat,msize+1,piv,bvec,msize+1,ierr)
    if (ierr == 0) then
...
!> \param Fav The averaged Fock/KS matrix
  subroutine simple_averaging(av,iteration,Dav,Fav)
    ...
    if (iteration == 1) then
       ...
       call mat_scal(0.5d0, Fav)
       call mat_daxpy(0.5d0, av%Fprev, Fav) 
       call mat_scal(0.5d0, Dav)
       call mat_daxpy(0.5d0, av%Dprev, Dav)
    endif
...
!> \param weights The weights for averaging Fock/KS and density matrices
  subroutine EDIIS_WEIGHTS(av,msize,queue,weights)
    ...
    type(Matrix) :: Dav, Fav
    integer :: piv(msize-1), error, i, j, his_start
...
!** Construct bvec
    ...
    do k = 1,msize-1
      ...
      bvec(i) = 2.0d0*(TrDoFo - mat_dotproduct(queue%D(j),queue%F(his_start)))
    enddo
!** Construct Amat
    !   A_ij = Tr(Di-Do)(Fj-Fo) + Tr(Dj-Do)(Fi-Fo) = 2TrDoFo + TrDiFj + TrDjFi - TrDiFo - TrDjFo - TrDoFj - TrDoFi
    j = queue%current_position
    i = av%ediis_pos
    Amat = 2.d0*TrDoFo  ! 2TrDoFo
    do k = 1,msize-1  
      j = j-1; if(j==0) j=av%cfg_settings(av%cfg_set_type)%max_history_size
      i = i-1; if(i==0) i=av%ediis_history_size-1
