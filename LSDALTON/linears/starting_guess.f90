...
subroutine get_initial_dens(H1,S,D,ls,config)
   ...
   !> Overlap matrix
   type(Matrix),intent(in),target :: S
   ...
   type(Matrix) :: C
   double precision trace, Nelectrons
   integer :: restart_lun, idum, ldum, i, j
   logical :: restart_from_dens, restart_from_cmo, purify_failed, gcbasis,dens_exsist,cmo_exsist
   real(realk),parameter :: THRNEL = 1.d-3
  ...
  !ENDIF
   ...
   if (restart_from_dens) then
     ...
     rewind restart_lun
     call mat_read_from_disk(restart_lun,D)
     ...
     IF (config%diag%CFG_restart .AND. config%optinfo%optimize) THEN
        ...
        if (purify_failed) then
         ...
         call starting_guess_entry(H1,D,ls,config)
        else
   ...
   elseif (restart_from_cmo) then
     ...
     call mat_read_from_disk(restart_lun,C)
     call density_from_orbs(config%opt%cfg_unres,config%decomp%nocc,config%decomp%nocca,config%decomp%noccb,C,D)
     call mat_free(C)
     call lsclose(restart_lun,'KEEP')
     WRITE(config%lupri,*)
...
!> \author L. Thogersen
!> \date 2003
  subroutine starting_guess_entry(H1,D,ls,config)
    use typedef
    implicit none
    !> One-electron Hamiltonian
    type(matrix),intent(inout)     :: H1
    ...
    !   call starting_guess_atden(D)
       ...
       call atoms_start(config,D,H1,ls)
...
#endif
    allocate(eival(config%decomp%S%nrow*2)) ! allow for unrestricted.
...
!> \date 2003
  ...
  use decompMod
    ...
    type(matrix)             :: D
    type(matrix)             :: Cmo
    real(realk), allocatable :: eival(:), V(:)
    integer                  :: n, n2, info
    ...
    !transform H1 to OAO
    call res_to_oao_basis(decomp,H1,H1)

    !diagonalize. on output V  contain Cmo orbitals
    allocate(eival(n))
    allocate(V(n2))

    call mat_to_full(H1,1.0d0,V)
    call my_dsyev('v','u',n,V,eival)
    call mat_init(Cmo,n,n)
    call mat_set_from_full(V,1.0d0,Cmo)

    deallocate(eival,V)

    !get OAO density
    call density_from_orbs(decomp%cfg_unres,decomp%nocc,decomp%nocca,decomp%noccb,Cmo,D)
  ...
  subroutine starting_guess_fit_density(decomp,D)
    ...
    real(realk)              :: Etotal
    logical :: getd, getdv
    real(realk), allocatable :: orb(:), eival(:)
    ...
    allocate(eival(ndim*2)) ! allow for unrestricted
  ...
  implicit none
      ...
      call mat_read_from_disk(D1lun,D1)
      call mat_read_from_disk(D2lun,D2)
      call LSCLOSE(D1lun,'KEEP')
      call LSCLOSE(D2lun,'KEEP')

      call mat_add(opt%cfg_weight_param,D1,1.0d0-opt%cfg_weight_param,D2,D)
