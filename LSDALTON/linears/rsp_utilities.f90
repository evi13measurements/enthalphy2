...
! circular dependency by eg. creation of a header file.
   ...
   private :: Cmo_final,  MOinfo, MOprecond, orbE_final
   public
   LOGICAL, SAVE :: MOinfo = .false., flushed = .false.
...
!************************************
    ...
    subroutine get_rsp_trials_from_MO(nroots,bvec_ao,nocc)
      ...
      do i = 1,nroots
        ...
        !call mat_create_elm(Mindex(i,1),Mindex(i,2),1.0d0,Bvec_ao(i))
        iniguess_full = 0.0d0
        iniguess_full(Mindex(i,1),Mindex(i,2)) = 1.0d0
        call mat_set_from_full(iniguess_full,1.0d0, Bvec_ao(i), 'MO iniguess')
        !transform to AO-basis
        call mat_mul(Cmo_final,Bvec_ao(i),'n','n',1.d0,0.d0,tmp_mat)
        call mat_mul(tmp_mat,Cmo_final,'n','t',1.d0,0.d0,Bvec_ao(i))
      enddo
...
!**** Transform Gn_MO back to the AO-basis
    ...
    subroutine util_AO_to_MO(S,X_ao,X_mo,cov)
      ...
      if (cov) then
        ...
        call mat_mul(S,Cmo_final,'n','n',1.d0,0.d0,SC)
      endif
...
!scriptP(X) = P X Q^T + Q X P^T in orbitalbase
    subroutine util_scriptPx(trans,D,S,X)
      ...
      call mat_mul(D,S,'n','n',1.d0,0.d0,P)    
      call mat_add_identity(1.0d0,-1.0d0,P,Q)  !Q = I-P
      if (trans == 'T' .or. trans == 't') then
        !** scr2 = P^TXQ
        call mat_mul(P,x,'t','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,Q,'n','n',1.d0,0.d0,scr2)
        !** scr3 = Q^TXP
        call mat_mul(Q,X,'t','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,P,'n','n',1.d0,0.d0,scr3)
        !** X = P^TXQ + Q^TXP
        call mat_add(1.d0,scr2,1.d0,scr3,X)
      elseif (trans == 'N' .or. trans == 'n') then
        !** scr2 = PXQ^T
        call mat_mul(P,x,'n','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,Q,'n','t',1.d0,0.d0,scr2)
        !** scr3 = QXP^T
        call mat_mul(Q,X,'n','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,P,'n','t',1.d0,0.d0,scr3)
        !** X = PXQ^T + QXP^T
        call mat_add(1.d0,scr2,1.d0,scr3,X)
      else
    ...
    subroutine util_scriptPxOAO(trans,D,X)
      ...
      call mat_init(scr2,D%nrow,D%ncol)
      call mat_init(scr3,D%nrow,D%ncol)
      call mat_add_identity(1.0d0,-1.0d0,D,Q)  !Q = I-P
      if (trans == 'T' .or. trans == 't') then
        !** scr2 = P^TXQ
        call mat_mul(D,x,'t','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,Q,'n','n',1.d0,0.d0,scr2)
        !** scr3 = Q^TXP
        call mat_mul(Q,X,'t','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,D,'n','n',1.d0,0.d0,scr3)
        !** X = P^TXQ + Q^TXP
        call mat_add(1.d0,scr2,1.d0,scr3,X)
      elseif (trans == 'N' .or. trans == 'n') then
        !** scr2 = PXQ^T
        call mat_mul(D,x,'n','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,Q,'n','t',1.d0,0.d0,scr2)
        !** scr3 = QXP^T
        call mat_mul(Q,X,'n','n',1.d0,0.d0,scr1)
        call mat_mul(scr1,D,'n','t',1.d0,0.d0,scr3)
        !** X = PXQ^T + QXP^T
        call mat_add(1.d0,scr2,1.d0,scr3,X)
      else
        WRITE(6,*) 'unknown transformation in scriptPx',trans
