...
!1. Find eigenvalues of alfa part of FUP
   ...
   fullmat1 = 0.0d0 ; fullmat2 = 0.0d0
   call mat_to_full(decomp%FUP,1.0d0,fullmat1)
   call mat_to_full(decomp%DU,1.0d0,fullmat2)
   ...
   if (DD%nocca == 0) then
      ...
      call DD_Fock_eigenvalue_unres('a',DD,decomp,halfmat,proj,'h',0.0d0)
...
!2. Find eigenvalues of beta part of FUP
   ...
   if (DD%noccb == 0) then
      ...
      call DD_Fock_eigenvalue_unres('b',DD,decomp,halfmat,proj,'h',0.0d0)
      if (decomp%cfg_hlgap_converged) then
         betaocc_conv = .true.
         write (decomp%lupri,*)
...
!3. Find eigenvalues of alfa part of FUQ
   ...
   fullmat1 = 0.0d0 ; fullmat2 = 0.0d0
   call mat_to_full(decomp%FUQ,1.0d0,fullmat1)
   call mat_to_full(decomp%QU,1.0d0,fullmat2)
   ...
   if (DD%nvirta == 0) then
      ...
      call DD_Fock_eigenvalue_unres('a',DD,decomp,halfmat,proj,'l',0.0d0)
      if (decomp%cfg_hlgap_converged) then
         alfavirt_conv = .true.
         write (decomp%lupri,*)
...
!4. Find eigenvalues of beta part of FUQ
   ...
   if (DD%nvirtb == 0) then
      ...
      call DD_Fock_eigenvalue_unres('b',DD,decomp,halfmat,proj,'l',0.0d0)
      if (decomp%cfg_hlgap_converged) then
         betavirt_conv = .true.
         write (decomp%lupri,*)
   ...
   !   if (abs(DD_FUPalfa_eival(i)) < 1.0d-12) then
   !      DD_FUPalfa_eival(i) = -1.0d3
   !   else if (abs(DD_FUPbeta_eival(i)) < 1.0d-12) then
   !      DD_FUPbeta_eival(i) = -1.0d3
   !   endif
   !enddo
      ...
      do i = 1, nguesses
         ...
         if (alfa) then
            ...
            do mu = 1, matdim
               do nu = 1, matdim
                  if (do_rsp_iniguess) then
                     iniguess_full(mu,nu) = DD%FUQalfa_eivecs(mu,alfaFUQFUPindex(posmin(1),1))* &
                                         &  DD%FUPalfa_eivecs(nu,alfaFUQFUPindex(posmin(1),2))
                  else
                     iniguess_full(mu,nu) = DD%FUQalfa_eivecs(mu,alfaFUQFUPindex(posmin(1),1))* &
                                         &  DD%FUPalfa_eivecs(nu,alfaFUQFUPindex(posmin(1),2))- &
                                         &  DD%FUPalfa_eivecs(mu,alfaFUQFUPindex(posmin(1),1))* &
                                         &  DD%FUQalfa_eivecs(nu,alfaFUQFUPindex(posmin(1),2))
                  endif
               enddo
            enddo
            ...
            !call mat_set_from_full(iniguess_full,1.0d0, iniguess(i))
         else
            ...
            do mu = 1, matdim
               do nu = 1, matdim
                  if (do_rsp_iniguess) then
                     iniguess_full(mu,nu) = DD%FUQbeta_eivecs(mu,betaFUQFUPindex(posmin(1),1))* &
                                         &  DD%FUPbeta_eivecs(nu,betaFUQFUPindex(posmin(1),2))
                  else
                     iniguess_full(mu,nu) = DD%FUQbeta_eivecs(mu,betaFUQFUPindex(posmin(1),1))* &
                                         &  DD%FUPbeta_eivecs(nu,betaFUQFUPindex(posmin(1),2))- &
                                         &  DD%FUPbeta_eivecs(mu,betaFUQFUPindex(posmin(1),1))* &
                                         &  DD%FUQbeta_eivecs(nu,betaFUQFUPindex(posmin(1),2))
                  endif
               enddo
            enddo
            !write(lupri,*) 'full Hes starting guess no', i
