...
type lshiftItem
   ...
   logical     :: arh_newdamp
   !> Do not allow level shift to go below this value (0.0d0 => standard scheme)
...
contains
   ...
   !> \author S. Host
   !> \date 2007
   subroutine main_levelshift(lshift,Ared,Sred,Gred,redsize,lub,red_space_dim,matdim,mu,xF,sigmaF,vectorsubspace)
   ...
   implicit none
        ...
        logical                       :: extradim
        real(realk), allocatable      :: A(:,:), S(:,:), RHS(:)
   ...
   if (lshift%fixed_shift) then
      ...
      do i = 1, 5
         call mat_init(x5(i),matdim,matdim)
      enddo
      ...
      allocate(A(fullsize,fullsize), S(fullsize,fullsize), RHS(fullsize))
   ...
   implicit none
        ...
        real(realk), allocatable    :: eignum(:), eignum_im(:),eigenvec(:,:), &
                                     & eigdenom(:), Atemp(:,:), Stemp(:,:) 

   !Determine initial intervals:
   !if (abs(mu) < 1.0d-2) then
   !   intervals(1) = -16.0d0 ; intervals(2) = -8.0d0 ; intervals(3) = -4.0d0 
   !   intervals(4) =  -2.0d0 ; intervals(5) =  0.0d0 
   !else
   !   intervals(1) = 2.0d0*mu  ; intervals(2) = 1.5d0*mu ; intervals(3) = mu
   !   intervals(4) = 0.75d0*mu ; intervals(5) = 0.5d0*mu  
   !endif
   ...
   allocate(eignum(reddim),eignum_im(reddim),eigdenom(reddim),eigenvec(reddim,reddim))
   allocate(Atemp(reddim,reddim),Stemp(reddim,reddim))
   ...
   do i = 1, reddim
      if (abs(eignum_im(i)/eigdenom(i)) > 1.0d-6) &
   ...
   do i = 1, reddim
      ...
      if (abs(eigdenom(i))>10d-12 .and. abs(eignum(i))>10d-12) then
   ...
   if (min_eival >= 0.0d0 .or. abs(min_eival) < 1.0d-2)  then
      intervals(1) = -16.0d0 ; intervals(2) = -8.0d0 ; intervals(3) = -4.0d0 
      intervals(4) =  -2.0d0 ; intervals(5) =  0.0d0 
   else if (reddim > 1) then
      intervals(1) = min_eival*10.0d0  ; intervals(2) = min_eival*7.5d0 ; intervals(3) = min_eival*5.0d0
      intervals(4) = min_eival*2.5d0 ; intervals(5) = min_eival
   else !quickfix: If reddim = 1, we cannot set mu=eigenval, because then H = A - mu*S = 0 
      intervals(1) = min_eival*10.0d0  ; intervals(2) = min_eival*7.5d0 ; intervals(3) = min_eival*5.0d0
      intervals(4) = min_eival*2.5d0 ; intervals(5) = intervals(4)
   endif
   deallocate(eignum,eignum_im,eigdenom,eigenvec)
   deallocate(Atemp,Stemp)
   ...
   do 
      ...
      if ((acceptedsteps >= 3 .and. dev < 0.05d0) .or. get_out) exit  
   enddo
   !if (debug_arh_iter_hlgap < 0.0d0 .and. mu > debug_arh_iter_hlgap) then
   !   !Don't accept smaller damping than HOMO-LUMO gap
   !   mu = debug_arh_iter_hlgap
   !   if (info_levelshift) write(lupri,*) 'Damping determined from HOMO-LUMO gap: mu =', mu
   !endif
   ...
   implicit none
        ...
        real(realk), allocatable    :: H(:,:), xtrial(:,:), RHS(:)
   ...
   allocate(H(nsize,nsize), IPIV(nsize), xtrial(nsize,5), RHS(nsize))
   ...
   !write (lupri,*) 'A in levelshift:'
   !call OUTPUT(A, 1, nsize, 1, nsize, reddim, reddim, 1, lupri)
   !write (lupri,*) 'S in levelshift:'
   !call OUTPUT(S, 1, nsize, 1, nsize, reddim, reddim, 1, lupri)
   ...
   do i = 1, 5
      ...
      H(1:nsize,1:nsize) = A(1:nsize,1:nsize) - mu*S(1:nsize,1:nsize)
         !write (lupri,*) 'H in levelshift, i =', i, ' and mu =', mu
         !call OUTPUT(H, 1, nsize, 1, nsize, nsize, nsize, 1, lupri)
         !write (lupri,*) 'RHS in levelshift:'
         !call OUTPUT(RHS, 1, nsize, 1, 1, nsize, 1, 1, lupri)
   ...
   do i = 1, nvecs
      if (lshift%arh_truncate) then
         ...
         call mat_read_from_disk(lub,bn)
         do j = 1, 5
   ...
   do j = 1,5
      ...
      if (j > 1 .and. xsize_old > xsize) then
         ...
         best_mu = intervals(j)
         get_out = .true.
         !if (abs(intervals(j)) < 1.0d-2) then
         !   best_mu = 0.0d0
         !   get_out = .true.
         !else
