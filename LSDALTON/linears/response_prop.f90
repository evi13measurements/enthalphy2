...
module response_wrapper_module
  ...
  public MCDresponse_driver, ALPHAresponse_driver, BETAresponse_driver, &
       ...
       & ESGresponse_driver, Get_dipole_moment, ESDresponse_driver, DTPAresponse_driver
  private
...
Contains
  ...
  subroutine Get_dipole_moment(molcfg,F,D,S,doPrint,DipoleMoment)
    ...
    complex(8) :: DipoleMoment_complex(3)
    integer :: i
    ...
    ! Zero dipole
    DipoleMoment(:) = 0.d0
    DipoleMoment_complex(:) = (0.d0,0.d0)
  ...
  subroutine ALPHAresponse_driver(molcfg,F,D,S,alphainput)
    ...
    complex(8), allocatable :: rsp_results_vec(:,:)
    type(RspFunc), allocatable :: MyRspFunc(:)
    complex(8), allocatable :: alpha(:,:,:), isotropic(:)
    ...
    SetFrequencies: do i=1,nfreq
       ...
       if(alphainput%real_frequencies_in_input) then
          MyRspFunc(i)%freq(2)=(1.d0,0.d0)*alphainput%bfreq(i)
       else
          MyRspFunc(i)%freq(2)=(0.d0,0.d0)
       end if

       ! Imaginary frequencies
       if(alphainput%imag_frequencies_in_input) then
          MyRspFunc(i)%freq(2)=MyRspFunc(i)%freq(2) + (0.d0,1.d0)*alphainput%imbfreq(i)
       end if
    ...
    CalculateLinearResponseFunction: do i=1,nfreq
       ...
       Isotropic(i) = ( alpha(i,1,1) + alpha(i,2,2) + alpha(i,3,3) )/3.d0

    end do CalculateLinearResponseFunction
  ...
  subroutine BETAresponse_driver(molcfg,F,D,S,betainput,DipoleMoment)
    ...
    complex(8), allocatable :: BetaPara(:), BetaPerp(:)
    ...
    SetFrequencies: do i=1,nfreq
       ...
       if(betainput%real_bfrequencies_in_input) then
          MyRspFunc(i)%freq(2)=(1.d0,0.d0)*betainput%bfreq(i)
       else
          MyRspFunc(i)%freq(2)=(0.d0,0.d0)
       end if
       ...
       if(betainput%imag_bfrequencies_in_input) then
          MyRspFunc(i)%freq(2)=MyRspFunc(i)%freq(2) + (0.d0,1.d0)*betainput%imbfreq(i)
       end if


       ! Real frequencies operator C
       if(betainput%real_cfrequencies_in_input) then
          MyRspFunc(i)%freq(3)=(1.d0,0.d0)*betainput%cfreq(i)
       else
          MyRspFunc(i)%freq(3)=(0.d0,0.d0)
       end if

       ! Imaginary frequencies operator C
       if(betainput%imag_cfrequencies_in_input) then
          MyRspFunc(i)%freq(3)=MyRspFunc(i)%freq(3) + (0.d0,1.d0)*betainput%imcfreq(i)
       end if
    ...
    CalculateQuadResponseFunction: do i=1,nfreq
    ...
    end do CalculateQuadResponseFunction
  ...
  subroutine Get_averaged_betas(BetaPara,BetaPerp,beta,DipoleUnit)
    ...
    complex(8), intent(inout) :: BetaPara
    !> Perpendicular avaraged beta
    complex(8), intent(inout) :: BetaPerp
    !> Full beta tensor (3,3,3)
    complex(8), intent(in) :: beta(3,3,3)
    ...
    do i=1,3
       do j=1,3
          BetaPerp = BetaPerp + &
               & ( 2.d0*beta(j,i,i) - 3.d0*beta(i,j,i) + 2.d0*beta(i,i,j) )*DipoleUnit(j)
       end do
    end do
    BetaPerp = (1.d0/5.d0)*BetaPerp
  ...
  subroutine GAMMAresponse_driver(molcfg,F,D,S,gammainput)
    ...
    character(len=1) :: comp(3)
    complex(8), allocatable :: GammaPara(:), GammaPerp(:)
    integer, pointer :: lupri
    logical :: ImagFreq
    ...
    SetFrequencies: do i=1,nfreq
       ...
       if(gammainput%real_bfrequencies_in_input) then
          MyRspFunc(i)%freq(2)=(1.d0,0.d0)*gammainput%bfreq(i)
       else
          MyRspFunc(i)%freq(2)=(0.d0,0.d0)
       end if
       ...
       if(gammainput%imag_bfrequencies_in_input) then
          MyRspFunc(i)%freq(2)=MyRspFunc(i)%freq(2) + (0.d0,1.d0)*gammainput%imbfreq(i)
       end if


       ! Real frequencies operator C
       if(gammainput%real_cfrequencies_in_input) then
          MyRspFunc(i)%freq(3)=(1.d0,0.d0)*gammainput%cfreq(i)
       else
          MyRspFunc(i)%freq(3)=(0.d0,0.d0)
       end if
       ...
       if(gammainput%imag_cfrequencies_in_input) then
          MyRspFunc(i)%freq(3)=MyRspFunc(i)%freq(3) + (0.d0,1.d0)*gammainput%imcfreq(i)
       end if

       ! Real frequencies operator D
       if(gammainput%real_dfrequencies_in_input) then
          MyRspFunc(i)%freq(4)=(1.d0,0.d0)*gammainput%dfreq(i)
       else
          MyRspFunc(i)%freq(4)=(0.d0,0.d0)
       end if

       ! Imaginary frequencies operator D
       if(gammainput%imag_dfrequencies_in_input) then
          MyRspFunc(i)%freq(4)=MyRspFunc(i)%freq(3) + (0.d0,1.d0)*gammainput%imdfreq(i)
       end if
    ...
    CalculateCubicResponseFunction: do i=1,nfreq
    ...
    end do CalculateCubicResponseFunction
  ...
  subroutine Get_averaged_gammas(GammaPara,GammaPerp,gamma)
    ...
    complex(8), intent(inout) :: GammaPara
    !> Perpendicular avaraged gamma
    complex(8), intent(inout) :: GammaPerp
    !> Full gamma tensor (3,3,3,3)
    complex(8), intent(in) :: gamma(3,3,3,3)
    integer :: i,j
    ...
    GammaPara = (1.d0/15.d0)*GammaPara
    ...
    do i=1,3
       do j=1,3
          GammaPerp = GammaPerp + &
               & 2.d0*gamma(j,i,i,j) - gamma(j,j,i,i)
       end do
    end do
    GammaPerp = (1.d0/15.d0)*GammaPerp
  ...
  subroutine OPAresponse_driver(molcfg,F,D,S)
    ...
    CalculateTransMoment: do i=1,nexci
       ...
       OscillatorStrength(i) = (2.d0/3.d0)*ExEnergies(i)*OscillatorStrength(i)
  ...
  subroutine TPAresponse_driver(molcfg,F,D,S,TPAinput)
    ...
    j = 0
    SetupTPAResponseFunction: do i=1,nexci
  ...
  subroutine Get_averaged_tpa(tpa,tpa_lin,tpa_circ)
    ...
    F=2; G=2; H=2
    tpa_lin = (1.d0/30.d0) * (F*tpaF + G*tpaG + H*tpaH)
    ...
    F=-2; G=3; H=3
    tpa_circ = (1.d0/30.d0) * (F*tpaF + G*tpaG + H*tpaH)
  ...
  subroutine DTPAresponse_driver(molcfg,F,D,S,dtpainput)
    ...
    type(matrix)               :: RHS(6), Xreal(1), Ximag(1),RHSR(1),RHSI(1), D1(3)
    integer                    :: I,J,K,nfreq
    ...
    if (molcfg%solver%rsp_new_solver) then
       call rsp_sym_complex_init(1, 1, 1, 1, 1)
    else 
       call rsp_complex_init(1, 1, 1, 1, 1)
    endif
    ! Initialize solution matrices
    call init_mat(Xreal(1),D)
    call init_mat(Ximag(1),D)
    ...
    FrequencyLoop: do I = 1,nfreq
       ...
       do K = 1,3
          ...
          if (normRHS > 1e-9) then
             ...
             RHSR(1) = re(RHS(K))   ! Real part of RHS
             RHSI(1) = im(RHS(K))   ! Imaginary part of RHS


             ! Complex second order RHS is constructed.
             ! Solve 2nd order equations for Kth component
             if (molcfg%solver%rsp_new_solver) then
                call rsp_sym_complex_solver(molcfg, F, D, S, 1, RHSR(1:1), w, 1, &
                     & Xreal(1:1), Ximag(1:1), .true., gdi=RHSI(1:1))
             else
                call rsp_complex_solver(molcfg, F, D, S, 1, RHSR(1:1), w, 1, &
                     & Xreal(1:1), Ximag(1:1), .true., gdi=RHSI(1:1))
             endif
             ...
             do J =1,3
                ...
                ! Set RHSR(1) and RHSI(1) equal to the appropriate RHS components
                RHSR(1) = re(RHS(J))
                RHSI(1) = im(RHS(J))
                ...
                tmp1 = -2.d0*dot(RHSI(1),Xreal(1))
                tmp2 = 2.d0*dot(RHSR(1),Ximag(1))
                tpa(I,J,K) = tmp1 + tmp2
             enddo
       ...
       do K=4,6
          ...
          if (normRHS > 1e-9) then
             ...
             if (molcfg%solver%rsp_new_solver) then
                call rsp_sym_complex_solver(molcfg, F, D, S, 1, RHSR(1), w(1), 1, &
                     & Xreal(1), Ximag(1), .true., gdi=RHSI(1))
             else
                call rsp_complex_solver(molcfg, F, D, S, 1, RHSR(1), w(1), 1, &
                     & Xreal(1), Ximag(1), .true., gdi=RHSI(1))
             endif
    ...
    D1(:)=0
    RHS(:)=0
    Xreal(1)=0; Ximag(1)=0; RHSR(1)=0; RHSI(1)=0
  ...
  subroutine calculate_one_photon_absorption(molcfg,D,D1,one_photon)
    ...
    complex(realk)             :: linear_response_function(1)
    integer                    :: J
    ...
    do J=1,3
       ...
       linear_response_function(1) = (0.d0,0.d0)
       call prop_oneave(molcfg, D, (/'EL'/), (/D1(J)/), (/1/), &
            & linear_response_function(1:1), comp=(/J/))
       one_photon(J) = -imag(linear_response_function(1))
    enddo
    ...
    do I = 1,nfreq
       ...
       do J=1,6
          if(J<4) then ! XXXX, YYYY, ZZZZ
             ...
             tpaGH = tpaGH + 2.d0*tpa(I,J,J)
          endif
       enddo

       tpa_av(I) = (1.d0/30.d0) * (F*tpaF + GH*tpaGH)
  ...
  !> \author Kasper Kristensen                                                                
  !> \date 2010-08
  subroutine ESGresponse_driver(molcfg,F,D,S,ESGinput)
    use decompMod, only: decompItem
    ...
    logical :: AllStates
    logical, allocatable :: doState(:)
    complex(8), allocatable :: rsp_results_vec(:,:), GSG(:)
    real(realk), allocatable :: ExEnergies(:), ESG(:,:,:)
    type(RspFunc), allocatable :: MyRspFunc(:) 
    ...
    totdim=3*natoms
    allocate(ESG(size_rsp,natoms,3))
    ...
    ! Use j as counter in case not all excited states are requested
    j = 0
    SetupESGResponseFunction: do i=1,nexci
    ...
    end do SetupESGResponseFunction
    ...
    deallocate(rsp_results_vec)
    deallocate(GSG)
    if(esginput%specific_states_in_input) deallocate(esginput%ExStates)
  ...
  subroutine ESDresponse_driver(molcfg,F,D,S,ESDinput,DipoleMoment)
    ...
    ! Use j as counter in case not all excited states are requested
    j = 0
    SetupESDResponseFunction: do i=1,nexci
    ...
    end do SetupESDResponseFunction
...
!    use scf_config, only : cfg_rsp_gamma
!    implicit none
!    !> response function structure to be built
!    type(RspFunc) :: MyRspFunc
!    !> magnetic component
...
!    MyRspFunc%freq(1) =  0.d0    
!    cfg_rsp_gamma = gamma
!    MyRspFunc%freq(2) = -freq*(1.d0,0.d0)-gamma*(0.d0,1.d0)  
!    MyRspFunc%freq(3) =  freq*(1.d0,0.d0)+gamma*(0.d0,1.d0)  
...
!  !> <br>
!  !> (lineshape parameter) (default is 0.005D0 for lorentz, 0.0070851079363103793d0 for gaussian)
!  !> <br>
...
!   complex(8), allocatable           :: rsp_results(:,:)
!   type(prop_molcfg) :: molcfg
!   integer, target   :: natoms
!   !    complex(8), allocatable, target, save           :: rsp_results(:,:)
!   complex(8) :: freqold,freq
!   integer,pointer :: MCDexci1(:),MCDexci2(:)
!   complex(8),pointer :: MCDfreq(:)
!   logical :: allow
...
!   molcfg = prop_molcfg(0d0*S,setting%MOLECULE(1)%p%Natoms, &
...
!          IF(ABS(rsp_results(j,i)) .GT. 1.0D-5)THEN
!             allowed(j,i) = .TRUE.
!          ELSE
!             allowed(j,i) = .FALSE.
!          ENDIF
...
!            &rsp_results(3,i)*rsp_results(3,i)) .GT. 1.0D-5)THEN
!          STRING4 = ' YES' 
!       ENDIF
...
!       enddo
!       if(allow)then
!          if(abs(freq-freqold)<3.d-6) then
!             !degenerate
...
!!       call print_response_function(RspFunc_stack, n_RspFunc, .false.,lupri)
!
!       !********************************************************************************
...
!!          call print_response_function(RspFunc_stack, n_RspFunc, .true.,lupri) 
!       ENDIF
!       !********************************************************************************
!       !*              Print Results
!       !********************************************************************************
...
!          do j=1,3
!             SIGNL =  1.d0
!             IF(AtermmomentL(i,j).LT. 0.D0) SIGNL = -1.d0
!             eigenvalueL(j) = ABS(AtermmomentL(i,j))
!             SIGN =  1.d0
!             IF(Atermmoment(i,j).LT. 0.D0) SIGN = -1.d0
...
!               Estart = MAX(MIN(freqA(1),freqB(1))-0.1*MAX(freqA(nAterms),freqB(nBterms)),0.d0)
!            ELSE
!               Estart = MAX(freqA(1)-0.1*MAX(freqA(nAterms),freqB(nBterms)),0.d0)
!            ENDIF
!         ELSE
!            IF(nBterms.NE.0)THEN
!               Estart = MAX(freqB(1)-0.1*freqB(nBterms),0.d0)
!            ELSE
!               Estart = 0
!            ENDIF
!         ENDIF
!         IF(Eend.NE.-1)THEN
!          Nstep = MCDinput%Nsteps
!          Range = Eend-Estart
!          step = Range/Nstep
!          x=Estart
!          lengthX=0
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREB(lengthX)=Y(i)*GAMMA/((x-freqB(i))**2+GAMMA**2) 
!              PUREBL(lengthX)=YL(i)*GAMMA/((x-freqB(i))**2+GAMMA**2) 
...
!             DO WHILE(x<Eend)
!              x=x+step
!              FUN=1/(gamma*sqrt(2*PI))*EXP(-((x-freqB(i))**2)/(2*gamma*gamma)) 
!              INT=INT+(FUN/x)*step
!             ENDDO
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREB(lengthX)=Y(i)/(gamma*sqrt(2*PI))*EXP(-((x-freqB(i))**2)/(2*gamma*gamma)) 
!              PUREBL(lengthX)=YL(i)/(gamma*sqrt(2*PI))*EXP(-((x-freqB(i))**2)/(2*gamma*gamma)) 
!              COMBINED(lengthX) = COMBINED(lengthX) + PUREB(lengthX)  
!              COMBINEDL(lengthX) = COMBINEDL(lengthX) + PUREBL(lengthX)  
!             ENDDO
!            ENDDO
...
!             DO WHILE(x<Eend)
!              x=x+step
!              FUN=(2.D0*GAMMA*(x-freqA(i)))/((x-freqA(i))**2+GAMMA**2)**2
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREA(lengthX)  =Y(i)*(2.D0*GAMMA*(x-freqA(i)))/((x-freqA(i))**2+GAMMA**2)**2
!              PUREAL(lengthX)=YL(i)*(2.D0*GAMMA*(x-freqA(i)))/((x-freqA(i))**2+GAMMA**2)**2
...
!             DO WHILE(x<Eend)
!              x=x+step
!              FUN=((x-freqA(i))/(gamma*gamma))/(gamma*sqrt(2*PI))*&
!                   &EXP(-((x-freqA(i))**2)/(2*gamma*gamma)) 
!              INT=INT+(x-freqA(i))*(FUN/x)*step
!             ENDDO
!             Y(i)=MCDAterm(i)/INT
!             YL(i)=MCDAtermL(i)/INT
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREA(lengthX)=Y(i)*((x-freqA(i))/(gamma*gamma))/(gamma*sqrt(2*PI))*&
!                   &EXP(-((x-freqA(i))**2)/(2*gamma*gamma)) 
!              PUREAL(lengthX)=YL(i)*((x-freqA(i))/(gamma*gamma))/(gamma*sqrt(2*PI))*&
!                   &EXP(-((x-freqA(i))**2)/(2*gamma*gamma)) 
!              COMBINED(lengthX) = COMBINED(lengthX) + PUREA(lengthX)  
!              COMBINEDL(lengthX)= COMBINEDL(lengthX)+ PUREAL(lengthX)  
!             ENDDO
...
!         IF(nAterms.NE.0)THEN
!            IF(nBterms.NE.0)THEN
!               Estart = MAX(MIN(freqA(1)*auTOcm,freqB(1)*auTOcm)-0.1*MAX(freqA(nAterms)*auTOcm,freqB(nBterms)*auTOcm),0.d0)
!            ELSE
!               Estart = MAX(freqA(1)*auTOcm-0.1*MAX(freqA(nAterms)*auTOcm,freqB(nBterms)*auTOcm),0.d0)
!            ENDIF
!         ELSE
!            IF(nBterms.NE.0)THEN
!               Estart = MAX(freqB(1)*auTOcm-0.1*freqB(nBterms)*auTOcm,0.d0)
!            ELSE
!               Estart = 0
!            ENDIF
!         ENDIF
...
!             DO WHILE(x<Eend)
!              x=x+step
!              FUN=GAMMA*auTOcm/((x-freqB(i)*auTOcm)**2+(GAMMA*auTOcm)**2)
!              INT=INT+(FUN/x)*step
!             ENDDO
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREB(lengthX)=Y(i)*GAMMA*auTOcm/((x-freqB(i)*auTOcm)**2+(GAMMA*auTOcm)**2) 
!              PUREBL(lengthX)=YL(i)*GAMMA*auTOcm/((x-freqB(i)*auTOcm)**2+(GAMMA*auTOcm)**2) 
!              COMBINED(lengthX) = COMBINED(lengthX) + PUREB(lengthX)  
!              COMBINEDL(lengthX) = COMBINEDL(lengthX) + PUREBL(lengthX)  
!             ENDDO
!            ENDDO
...
!             DO WHILE(x<Eend)
!              x=x+step
!              FUN=1/(gamma*auTOcm*sqrt(2*PI))*EXP(-((x-freqB(i)*auTOcm)**2)/(2*(gamma*gamma*auTOcm*auTOcm))) 
!              INT=INT+(FUN/x)*step
!             ENDDO
!             !PLOT IN ATOMIC UNIT THEN ELSE
!             Y(i)=-BtermConstant*MCDBterm(i)*33.53/INT 
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREB(lengthX)=Y(i)/(gamma*auTOcm*sqrt(2*PI))*EXP(-((x-freqB(i)*auTOcm)**2)/(2*gamma*auTOcm*gamma*auTOcm)) 
!              PUREBL(lengthX)=YL(i)/(gamma*auTOcm*sqrt(2*PI))*EXP(-((x-freqB(i)*auTOcm)**2)/(2*gamma*auTOcm*gamma*auTOcm)) 
!              COMBINED(lengthX) = COMBINED(lengthX) + PUREB(lengthX)  
!              COMBINEDL(lengthX) = COMBINEDL(lengthX) + PUREBL(lengthX)  
!             ENDDO
!            ENDDO
...
!             DO WHILE(x<Eend)
!              x=x+step
!              FUN=(2.D0*GAMMA*auTOcm*(x-freqA(i)*auTOcm))/((x-freqA(i)*auTOcm)**2+(GAMMA*auTOcm)**2)**2
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREA(lengthX)  = Y(i)*(2.D0*GAMMA*auTOcm*(x-freqA(i)*auTOcm))/((x-freqA(i)*auTOcm)**2+(GAMMA*auTOcm)**2)**2
!              PUREAL(lengthX) =YL(i)*(2.D0*GAMMA*auTOcm*(x-freqA(i)*auTOcm))/((x-freqA(i)*auTOcm)**2+(GAMMA*auTOcm)**2)**2
!              COMBINED(lengthX) = COMBINED(lengthX) + PUREA(lengthX)  
!              COMBINEDL(lengthX)= COMBINEDL(lengthX) + PUREAL(lengthX)  
!             ENDDO
!            ENDDO
...
!             DO WHILE(x<Eend)
!              x=x+step
!              FUN=((x-freqA(i)*auTOcm)/(gamma*auTOcm*gamma*auTOcm))/(gamma*auTOcm*sqrt(2*PI))*&
!                   &EXP(-((x-freqA(i)*auTOcm)**2)/(2*gamma*auTOcm*gamma*auTOcm)) 
!              INT=INT+(x-freqA(i)*auTOcm)*(FUN/x)*step
!             ENDDO
!             Y(i)=AtermConstant*MCDAterm(i)*33.53/INT
!             YL(i)=AtermConstant*MCDAtermL(i)*33.53/INT
...
!              x=x+step
!              lengthX=lengthX+1
!              PUREA(lengthX) = Y(i)*((x-freqA(i)*auTOcm)/(gamma*auTOcm*gamma*auTOcm))/(gamma*auTOcm*sqrt(2*PI))*&
!                   &EXP(-((x-freqA(i)*auTOcm)**2)/(2*gamma*auTOcm*gamma*auTOcm)) 
!              PUREAL(lengthX)=YL(i)*((x-freqA(i)*auTOcm)/(gamma*auTOcm*gamma*auTOcm))/(gamma*auTOcm*sqrt(2*PI))*&
!                   &EXP(-((x-freqA(i)*auTOcm)**2)/(2*gamma*auTOcm*gamma*auTOcm)) 
!              COMBINED(lengthX) = COMBINED(lengthX) + PUREA(lengthX)  
!              COMBINEDL(lengthX)= COMBINEDL(lengthX)+ PUREAL(lengthX)  
!             ENDDO
...
!             Deltavec(i) = i*(1.d0/MCDinput%nVecForPeak)  
!          enddo
!          Deltavec(MCDinput%nVecForPeak) = 0.998d0 
...
!                   nXcoor2 = nXcoor2+1
!                   Xcoor2(nXcoor2) = -sqrt(GAMMA*GAMMA*(1.d0-DELTAVEC(j))/(DELTAVEC(j)))+freqB(i)
!                enddo
...
!             unique=.TRUE.
!             do j=1,nXcoor3
!                IF(ABS(Xcoor3(j)-Xcoor2(i)).LT.1.0D-7)unique=.FALSE.
!             enddo
!             if(unique)then
!                nXcoor3 = nXcoor3+1
...
!!          call print_response_function(RspFunc_stack2, n_RspFunc,.true.,molcfg%lupri)
!
...
!       enddo
!       
!       write(lupri,'(A)')'The A terms of Magnetic Circular Dicroism'
!       do i=1,nAterms
