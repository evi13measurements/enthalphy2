module RSPsolver
  ...
  use TYPEDEF, only: LSSETTING !due to prop_molcfg type def.
  ...
  integer, save :: rsp_number_of_current_trial, rsp_number_of_rhs, rsp_number_of_sols, &
                 ...
                 & rsp_bvec_dim, lub_mcdvec, nmcdvec
...
!==========================================================
! The linear scaling response solver:
! LINEQ.EQ.FALSE  DIRECT THE SOLUTION THE GENERALIZED RSP
!                 EIGENVALUE PROBLEM
!                  ( E(2)-w(I)S(2))X(I) = 0
! LINEQ.EQ.TRUE  DIRECT THE SOLUTION THE GENERALIZED RSP
!                LINEAR EQUATIONS
!                  ( E(2)-w(I)S(2))X(I) - GD = 0
! gd : Right hand sides = property gradients
! Sonia, Poul, Lea 2004-2006
! Revised by Stinne and Sonia, October 2006 (disk-based version)
!==========================================================
    use lstiming
    implicit none
...
!    type(decompItem),intent(inout) :: decomp
    ...
    integer, intent(in)         :: n_gd_or_exci  !# of gradients or excitations
    ...
    real(realk), intent(inout)  :: eival(rsp_number_of_omegas)  !if LINEQ, eival = laser freqs  => input
                                                                 !else      eival = excitation E => output
    type(Matrix), intent(inout) :: eivecs(rsp_number_of_sols) !output solution vectors
!local
...
!for restart
  ...
  rsp_thresh= molcfg%solver%rsp_thresh
    ...
    red_E = 0.0d0 ; red_S = 0.0d0 ; red_X = 0.0d0
  ...
  &     ' AND TRANSITION MOMENT CALCULATION (LSTDHF) >>>'
    ...
    if (nmcdvec /= 0) then
       ...
       if (ndim>2) then
         ...
         IF(conv)then
            ...
            do i = 1,n_gd_or_exci
               ...
               call expand_on_basis(molcfg,ndim_red,ndim,RED_X(1:ndim_red*2,i),lub_rsp,eivecs(i))
            enddo
            do i = 1,rsp_bvec_dim
       ...
       call get_1st_orth_trials(molcfg,F,D,S,n_gd_or_exci,gd,EIVAL,Nb_new,bvecs)
       Ndim_red = 0
    endif
    ...
    do itmic = 1,max_it
       ...
       call get_residuals_and_new_b(molcfg,F,D,S,itmic,ndim_red,n_gd_or_exci, &
           & gd,RED_X,eival,Nb_new,bvecs,conv,exit_loop,molcfg%lupri) 
       ...
       if (conv) then
          ...
          enddo
          exit
      else !Stinne: Dump solutions to disk for restart
         lurestart = -1
         ...
         do i = 1,n_gd_or_exci
            ...
            !Construct solution vector by expanding red space solution onto trial vector space
            if (molcfg%solver%info_rsp) WRITE(molcfg%LUPRI,*)'RSP_SOLVER -> expand_on_basis (dump to disk), ITMIC: ', itmic
            call expand_on_basis(molcfg,ndim_red,ndim,RED_X(1:ndim_red*2,i),lub_rsp,eivecs(i))

            write(lurestart) i, eival(i)
            call mat_write_to_disk(lurestart,eivecs(i))
         enddo
...
!========================================================
  subroutine get_1st_orth_trials(molcfg,F,D,S,n_gd_or_exci,gd,eival,Nb_new,bvecs)
!--------------------------------------
! Purpose: set up initial trial vectors 
!--------------------------------------
    implicit none
    type(prop_molcfg),intent(inout) :: molcfg
    type(Matrix), intent(in)   :: F,D,S,gd(rsp_number_of_rhs)
    real(realk), intent(inout) :: eival(rsp_number_of_omegas)
    integer, intent(in)        :: n_gd_or_exci
    type(Matrix),intent(inout) :: bvecs(rsp_bvec_dim)  !output   
    integer, intent(out)       :: Nb_new

    IF (LINEQ) THEN              
       if (molcfg%solver%info_rsp) WRITE(molcfg%LUPRI,*) 'Now enter 1st guess for linear equations '
       CALL get_1st_orth_trial_lineq(molcfg,F,D,S,n_gd_or_exci,gd,eival,bvecs,Nb_new)
    ELSE
...
!============================================================================
  subroutine get_1st_orth_trial_lineq(molcfg,F,D,S,ngd,GD,eival,bvecs,Nb_new)
!*******************************************************
! Purpose: 
...
!
    ...
    do ibx = 1, ngd
       grad_x = gd(ibx)  !STINNE - we do now not divide in symm and antisymm parts
       ...
       if (norm > molcfg%solver%rsp_tolerance) then          
          ...
          call rsp_AB_precond(molcfg,grad_x,F,D,S,EIVAL(ibx),Pgrad) !FIXME: needs to be changed to work for more than one frequency at a time
          Bvec_tmp(ibx) = Pgrad
       !Sonia_mag
       !WRITE(molcfg%LUPRI,*) '-----------------------------------------------'
...
!
    if (molcfg%solver%rsp_restart_exci) then
       ...
       do i = 1, molcfg%solver%rsp_restart_nexci
          ...
          call mat_read_from_disk(lurestart,Bvec_tmp(i))          
       enddo
...
!local 
 ...
 max_red= molcfg%solver%rsp_maxred
   ...
   !
      ...
      rewind(lub_rsp)
      do k = 1,Nb_prev
         call mat_read_from_disk(lub_rsp,b_k)
         if (nmcdvec /= 0 .and. k <= nmcdvec) then
            call mat_read_from_disk(lub_mcdvec,Xf)
            call mat_read_from_disk(lub_mcdvec,rho_k)
         endif
         do irx = 1,Nb_new
...
!---------------------------------------------------------------------
    ...
    do irx = 1,Nb_new    
       ...
       if (sym_or_antisym) then
          ...
          call mat_scal_dia(0.5d0,Bvec_tmp(irx))
          sym_or_antisym = .false.
       endif
    enddo
...
!
! INPUT
! Bvec_temp_i,  the i'th bvector to be symmetrically orthonormalized
! i          ,  index of Bvec_temp_i among the new bvectors (for printout)

! Output
! Bvec_temp_i, the i'th bvector which is now symmetrically orthonormalized
! lin_depend_i, linear dependence indicator: 1 if non-dep, 0 if lin dep 


    implicit none
    type(prop_molcfg),intent(inout) :: molcfg
    integer, intent(inout) :: lin_depend_i
    integer, intent(in) :: i !Only used for printout if linearly dependent
    ...
    do j = 1,2
       ...
       IF (ABS(1.d0-X2) < molcfg%solver%rsp_OVLMIN) THEN
...
! of RPA - Sonia, June 2003
!******************************************************
    implicit none
    type(prop_molcfg),intent(inout) :: molcfg
!    type(decompItem),intent(in) :: decomp
    type(Matrix), intent(in) :: D,S,F,Bvecs(:)
    integer, intent(in) :: nnew
    logical, intent(in) :: make_rhos
    type(Matrix), intent(inout) :: sigmas(:),rhos(:)
    type(Matrix) :: scr
    integer :: i,ndim,l 
    ndim = S%nrow
...
!==============================================================
  subroutine make_lintran_vecs(molcfg,D,S,F,Bvecs_i,sigma_i,rho_i,make_rhos)
    ...
    use dal_interface
!*******************************************************
!  Carry out linear transformation: sigma(i) = E[2]*b(i)
...
!    type(decompItem),intent(in) :: decomp
    type(prop_molcfg),intent(inout) :: molcfg
    type(Matrix), intent(in) :: D,S,F,Bvecs_i
    type(Matrix), intent(inout) :: sigma_i,rho_i  !output
    logical, intent(in) :: make_rhos
    type(matrix) :: prod,prod1,prod2,GbDs
    integer :: ndim
    logical :: cov
...
!Test: declarations used in testing
    ...
    if (make_rhos) then
       call mat_mul(prod2,S,'n','n',1.d0,0.d0,prod)
       call mat_mul(S,prod,'n','n',2.d0,0.d0,rho_i) !Sign changed 15/7-09!
    endif
    call ABCcommutator(ndim,F,S,prod2,sigma_i)
    
    call di_GET_GbDs_LSDALTON(molcfg%lupri,molcfg%luerr,& 
         &molcfg%setting,prod2,GbDs)
    if (molcfg%setting%do_dft) THEN 
       ...
       call mat_daxpy(1.d0,prod,GbDs) 
    endif
    call ABCcommutator(ndim,GbDs,S,D,prod)
    call mat_DAXPY(1.d0,prod,sigma_i)
    call mat_scal(2.0d0,sigma_i)
  ...
  subroutine make_lintran_vecsArray(molcfg,D,S,F,Bvecs,sigma,rho,make_rhos,nnew)
    ...
    use dal_interface
!****************************************************************
!  see make_lintran_vecs this one is just for many at a time
!****************************************************************
    implicit none
    integer :: ndim,i,nnew
!    type(decompItem),intent(in) :: decomp
    type(prop_molcfg),intent(inout) :: molcfg
    type(Matrix), intent(in) :: D,S,F,Bvecs(nnew)
    type(Matrix), intent(inout) :: sigma(nnew),rho(nnew)  !output
    logical, intent(in) :: make_rhos
    type(matrix) :: prod1
...
!Test: declarations used in testing
    ...
    do i=1,nnew
       ...
       if (make_rhos) then
          call mat_mul(prod2(i),S,'n','n',1.d0,0.d0,prod1)
          call mat_mul(S,prod1,'n','n',2.d0,0.d0,rho(i)) !Sign changed 15/7-09!
       endif
    enddo
    call di_GET_GbDs_LSDALTON(molcfg%lupri,molcfg%luerr,& 
         &molcfg%setting,prod2,GbDs,nnew) 
    if(molcfg%setting%do_dft)then
       call II_get_xc_linrsp(molcfg%lupri,molcfg%luerr,& 
            &molcfg%setting,ndim,prod2,D,sigma,nnew)   !sigma used as temp mat 
       do i=1,nnew
          call mat_daxpy(1.d0,sigma(i),GbDs(i))
       enddo
    endif
    ...
    do i=1,nnew
       ...
       call mat_DAXPY(1.d0,prod1,sigma(i))
       call mat_scal(2.0d0,sigma(i))
       ! Project out redundancies
...
!
! Input:
!  ndim_red, number of vectors on disk and current half size of reduced space
!  nb_NEW, number of new vectors
!  BVEC, the Nb_new new vectors (old ones on disk, lub_rsp)
!  SIGMAS, E[2] transformed Nb_new b-vectors (old ones on disk, lusigma_rsp)
!  RHOS, S[2] transformed Nb_new b-vectors (old ones on disk, lurho_rsp)
!
! Output:
!  Ndim_red, updated number of vecs on disk and new half size of reduced space (updated in extend_red_matrices2)
!  RED_EIVEC, eigenvectors/solutions of the new reduced rpa eq. dimension(ndim_RED,ngrad)
!  EIVAL, reduced space excitation energies, or input frequencies. dim(n_gd_or_exci)
!
    implicit none 
    type(prop_molcfg),intent(inout) :: molcfg
    integer, intent(in) :: nb_new,n_gd_or_exci
    integer, intent(inout) :: ndim_red !updated in extend_red_matrices2
    type(Matrix), intent(in) :: gd(:),sigmas(:),rhos(:),bvecs(:)
    real(realk), intent(inout) :: eival(:) !if LINEQ, it is input, otherwise it is output
    real(realk), intent(out) :: red_eivec(:,:) !org dim:(2*molcfg%solver%rsp_maxred,2*molcfg%solver%rsp_maxred)
...
! ndim_red, number of vectors on disk and current half size of reduced space
! nb_NEW, number of new vectors
! ngd, the number of response vectors or of excitations/eigenvectors to be found
! gd(ngd), if(lineq): matrix array containing ngd right hand sides (gradients)
!                     not referenced if lineq=false
! sigmas,rhos,bvecs:  matrix arrays of the nb_new new vectors
!
! OUTPUT:
! Ndim_red, updated number of vecs on disk and new half size of reduced space
!
    implicit none
    type(prop_molcfg),intent(inout) :: molcfg
    integer, intent(in) :: nb_new,ngd
    ...
    do j = 1, 2*ndim_red, 2
       call mat_read_from_disk(lusigma_rsp,sigma_j)
       call mat_read_from_disk(lurho_rsp,rho_j)
    ...
    do i = 1, 2*ndim_red, 2
       call mat_read_from_disk(lub_rsp,b_j)
    ...
    do i = 1,Nb_new
       call mat_write_to_disk(lub_rsp,bvecs(i))
       call mat_write_to_disk(lusigma_rsp,sigmas(i))
       call mat_write_to_disk(lurho_rsp,rhos(i))
...
! Purpose: extend reduced gradient with new vectors
...
! bvecs:    matrix arrays of the nb_new new vectors
!
! OUTPUT:
! No output in argument list, but global variable red_GD is updated
    implicit none
type(prop_molcfg), intent(inout)    :: molcfg
    integer,intent(in) :: ndim_red, nb_new, ngd
    type(Matrix), intent(in) :: gd(:),bvecs(:)
    type(Matrix) :: bT
...
!============================================================================
  subroutine get_residuals_and_new_b(molcfg,F,D,S,itmic,ndim_red,ngd,gd,&
          &red_eivec,eival,Nb_new,bvecs,conv,exit_loop,lupri)
    implicit none
!******************************************************************
! Purpose (~RSPNEX): 
...
!  gd(ngd):  matrix array containing ngd right hand sides (gradients)
!  freq:     the ngd frequencies
!  red_eivec:    the ngd reduced space solution vectors (same as RED_X)
!  eival:    the ngd frequencies if lineq=true, otherwise the ngd excitation energies

!  OUTPUT:
!  nb_NEW:   number of new vectors
!  bvecs:    matrix arrays of the nb_new new vectors
!  conv:     set true if converged, and microiterations should be stopped
!  exit_loop:set true if we have no new vectors due to linear dependencies,
!                and microiterations should be stopped  

    type(prop_molcfg),intent(inout) :: molcfg
    integer,intent(in) :: itmic,ndim_red,ngd,lupri
    type(Matrix),intent(in) :: F,D,S,gd(:)
    real(realk), intent(in) :: red_eivec(:,:)
    real(realk), intent(inout) :: eival(:)
    integer, intent(inout)  :: Nb_new
    type(Matrix),intent(inout) :: bvecs(:) !output
    logical, intent(out)    :: conv,exit_loop
!local
...
!    rewind(lurestart)
    do ibx = 1,ngd
       ...
       if (ABS(wibx) > 1.d-20) then
          !for nozero frequencies fill in with -wi*S[2]X(ibx) contrib
          call expand_on_basis_minus(molcfg,ndim_red,ndim,red_eivec(1:ndim_red*2,ibx),lurho_rsp,residuals(ibx))
       ...
       call expand_on_basis(molcfg,ndim_red,ndim,red_eivec(1:ndim_red*2,ibx),lusigma_rsp,residuals(ibx))
       ...
       if (LINEQ) then
          ...
          call mat_daxpy(-1.d0,gd(ibx),residuals(ibx))
       endif
    enddo
!    call LSCLOSE(lurestart,'KEEP')
...
! New trial(s) is equal to preconditioned residual(s)
    ...
    !and set molcfg%solver%rsp_conv_thr to this value times the factor requested in input (1.0d-1, 1.0d-2, or 1.0d-3).
       if (molcfg%solver%rsp_convdyn) then
           if (ITMIC == 1) then
             molcfg%solver%rsp_dyn_thresh=0d0
             av_norm = 0.0d0
             do i = 1, ngd
    ...
    do ibx = 1,ngd
       ...
       if (conv_test <= rsp_thresh) then
          ...
          call precond_residual(molcfg,F,D,S,EIVAL(ibx),residuals(ibx))
       endif
    enddo
    ...
    do ibx = 1,ngd
       if (index_conv(ibx) == 0) then !vec not converged
          nb_new = nb_new + 1
          if (nb_new /= ibx) residuals(nb_new) = residuals(ibx)
          all_conv = .false.
       endif
    enddo
!
...
!
    ...
    call verify_conv_prop(molcfg,ngd,n_not_conv,Nb_new,itmic,ndim_red,conv,exit_loop,molcfg%lupri)
!
...
!=================================================
  subroutine precond_residual(molcfg,F,D,S,omega,Pres_jr)
    !form new trial from preconditioned residual
    implicit none
    type(prop_molcfg),intent(inout) :: molcfg
    ...
    if (norm > molcfg%solver%rsp_tolerance) then                 
       call rsp_AB_precond(molcfg,scr,F,D,S,omega,Pres_jr)
    endif
    call mat_free(scr)
  end subroutine precond_residual

  subroutine verify_conv_prop(molcfg,ngd,n_not_conv,Nb_new,itmic,ndim_red,conv,exit_loop,lupri)
!Preeetty ugly routine, evaluate convergence status and prints.
...
!we instead hit some boundry/threshold
    ...
    integer, intent(in) :: ngd,n_not_conv,Nb_new,itmic,ndim_red,lupri
    logical, intent(out) :: conv,exit_loop
    integer :: kconv
!
! ---
...
!====================================================================
  subroutine expand_on_basis(molcfg,ndim_red,ndim,red_eivec_i,lu_basis,expanded_vec)
!********************************************************************
! PURPOSE:
...
!******************************************************
    ...
    do k = 1,ndim_red !length of red. solution for each grad
                      !and # of bf's (no pair)
      call mat_read_from_disk(lu_basis,basisvec_k) 
...
!========================================================================
  subroutine expand_on_basis_minus(molcfg,ndim_red,ndim,red_eivec_i,lu_basis,expanded_vec)
!************************************************************************
! PURPOSE:
!   Expand reduced space solution rho on full basis 
!
!   INPUT:
!   ndim:     size of matrices/number of basis functions
!   ndim_red: number of vectors on disk
...
!
!  Solve reduced eigenvalue equation in subspace
!
!  INPUT:
!  ndim_red:  half size of reduced space
!  nexci:     requested number of excitation energies
!
!  OUTPUT:
!  eival:     array containing the nexci excitation energies
!  red_eivec: the nexci reduced space eigenvectors
!
    implicit none
type(prop_molcfg), intent(inout)    :: molcfg
    integer, intent(in) :: ndim_red, nexci
    real(realk),intent(inout) :: eival(:)
    ...
    real(realk), parameter :: drtest = 1.0d-5
    ndim_red_mat = 2*ndim_red
...
!
  ...
  real(realk), PARAMETER :: ZEROT=1.0D-9, COMPLX = 1.0D7
...
!
  ...
  CALL DGEMM('N','N',NDIM,NDIM,NDIM,1.D0,SRED,NDIM, &
     &           EIVEC,NDIM,0.D0,WRK1,NDIM)
  if (molcfg%solver%info_rsp) then
    WRITE (MOLCFG%LUPRI,*) ' REDS[2]*X'
    CALL OUTPUT(WRK1,1,NDIM,1,NDIM,NDIM,NDIM,1,MOLCFG%LUPRI)
  endif
  CALL DGEMM('T','N',NDIM,NDIM,NDIM,1.D0,EIVEC,NDIM, &
     &           WRK1,NDIM,0.D0,WRK2,NDIM)
  if (molcfg%solver%info_rsp) then
...
!    type(decompItem),intent(in) :: decomp
    ...
    type(Matrix),intent(inout) :: bvecs(rsp_bvec_dim),gd(rsp_number_of_rhs) 
    integer, intent(out)       :: Nb_new
...
! thomas
    ...
    do i=1,nBvec_tmp
       call mat_write_to_disk(lub_mcdvec,Bvec_tmp(i))
       call make_lintran_vecs(molcfg,D,S,F,Bvec_tmp(i),sigma_i,rho_i,.true.)
       call mat_write_to_disk(lub_mcdvec,rho_i)
    enddo
...
!
    ...
    do ibx = 1, ngd
       ...
       if (norm > molcfg%solver%rsp_tolerance) then          
          !Precondition if the matrix exists  by
          !solving A Pgrad = grad
          call rsp_AB_precond(molcfg,grad_x,F,D,S,freq(ibx),Pgrad)
          Bvec_tmp(ibx) = Pgrad
       else
          Bvec_tmp(ibx) = grad_x
       endif
...
!    
    ...
    do irx = 1,Nb_new
       if (lin_depend(irx) /= 0) then
         no_of_new = no_of_new + 1
         if(.NOT. molcfg%solver%rsp_quiet)write(molcfg%lupri,*)'assigning'
         bvecs(no_of_new) = Bvec_tmp(irx)
       endif
    enddo
...
!  ajt Generalization to treat negative frequencies too
    ...
    integer,intent(in)         :: ngd   !number of gradients
    real(realk), intent(in)    :: freq(rsp_number_of_omegas)
    type(Matrix),intent(in)    :: D,S,bvecs(rsp_bvec_dim)
    ...
    do i=1,NrP
       ...
       call mat_mul(tempm1,S,'n','n',1.d0,0.d0,tempm2)
       call mat_mul(S,tempm2,'n','n',2.d0,0.d0,tempm1)
...
! Generalization to equations with negative frequency AndreasJT
    ...
    do igd=1, ngd
       ...
       do j = 1, rdim
          l = min(j+nump,merge(2*j-1,2*j,freq(igd)<0.0d0))
          do i = 1, rdim
             k = min(i+nump,merge(2*i-1,2*i,freq(igd)<0.0d0))
             E2(i,j) = red_E(k,l)
             S2(i,j) = red_S(k,l)
          enddo
       ...
       i = merge(2, 1, freq(igd) < 0.0d0)
       red_X(i:i+2*(nump-1):2,igd) = 0.0d0
       i = merge(1, 2, freq(igd) < 0.0d0)
...
!************************************************************
    ...
    call ABCcommutator(ndim,bvecs_i,D,S,prod2)
    call mat_mul(prod2,S,'n','n',1.d0,0.d0,prod)
    call mat_mul(S,prod,'n','n',-2.d0,0.d0,rho_i)
    call util_scriptPx('T',D,S,rho_i)
...
call mat_add(1.d0,gd,-1.d0,E2XR,res_real)
...
subroutine rsp_AB_precond(molcfg,Gn,F,D,S,omega,Gnt)
  use decompMod
  use rspPrecond
  !Preconditioning of Gn by solving A Gnt = Gn
  implicit none
    type(prop_molcfg),intent(inout) :: molcfg
  type(Matrix), intent(in) :: Gn, F,D,S
  integer                  :: ndim
  real(realk), intent(inout)  :: omega
  ...
  call mat_init(GntU,ndim,ndim)
     ...
     if (molcfg%solver%rsp_MO_precond) then 
        ...
        !if(.not.molcfg%solver%rsp_quiet)write (molcfg%lupri,*) 'OAO preconditioning'
        !Convert residual to Cholesky (U) basis:
        call res_to_oao_basis(molcfg%decomp,Gn, GnU)
        call oao_rsp_solver(molcfg%decomp, GnU, omega, GntU)
        !call PCG_precond(GnU, GntU, omega, symm) STINNE comment
...
call mat_free(GntU)
    ...
    subroutine get_1st_rsp_trials(molcfg,nroots,bvec_ao,option)
    ...
    type(prop_molcfg),intent(inout) :: molcfg
      ...
      character, intent(in), optional :: option*3
      type(matrix), intent(inout) :: bvec_ao(nroots)
      integer :: ndim, mu, nu
      type(matrix) :: iniguess, scr, scr2
      real(realk), allocatable :: iniguess_full(:,:)
      real(realk), allocatable :: orb_energy_diff(:)
      integer, allocatable     :: FUQFUPindex(:,:)
      integer                  :: i, j, k, posmin(1), posmax(1)
      real(realk)              :: fac 
      type(debugItem)          :: debug
      type(DDitem)             :: DD

      !ndim = bvec_ao(1)%nrow
      !call mat_init(iniguess,ndim,ndim)
      !call mat_init(scr,ndim,ndim)
      !call mat_init(scr2,bvec_ao(1)%nrow,bvec_ao(1)%nrow)
    ...
    !call mat_free(iniguess)
    !call mat_free(scr)
    !call mat_free(scr2)
    end subroutine get_1st_rsp_trials

    subroutine get_1st_rsp_trials_unres(molcfg,nroots,bvec_ao,option)
    ...
    type(prop_molcfg),intent(inout) :: molcfg
      ...
      type(matrix) :: iniguess, scr, scr2
      real(realk), allocatable :: iniguess_full(:,:)
      real(realk), allocatable :: alfa_orb_energy_diff(:), beta_orb_energy_diff(:)
      integer, allocatable     :: alfaFUQFUPindex(:,:), betaFUQFUPindex(:,:)
      integer                  :: i, j, k, posmin(1), posmax(1), fulldim
      real(realk)              :: fac, alfamin, betamin 
      logical                  :: alfa
      type(debugItem)          :: debug
      type(DDitem)             :: DD
      ...
      if (molcfg%solver%rsp_mostart) then
         ...
         call dd_shutdown(molcfg%decomp,DD)
         !!### JEPPE'S SUGGESTION: Use eigenvectors corresponding to lowest eigenvalues of FUP and FUQ
         !allocate(iniguess_full(ndim,ndim))
         !allocate(alfa_orb_energy_diff(DD_nvirta*DD_nocca))
         !allocate(beta_orb_energy_diff(DD_nvirtb*DD_noccb))
         !allocate(alfaFUQFUPindex(DD_nvirta*DD_nocca,2))
         !allocate(betaFUQFUPindex(DD_nvirtb*DD_noccb,2))

         !k = 0
         !!Calculate alfa orbital energy differences obtained from FUP/FUP eigenvalues
         !do i = 1, DD_nvirta !FUQeival
         !   do j = 1, DD_nocca !FUPeival
         !      k = k + 1
         !      alfa_orb_energy_diff(k) = DD_FUQalfa_eival(i) - DD_FUPalfa_eival(j)
         ...
         !do i = 1, DD_nvirtb !FUQeival
         !   do j = 1, DD_noccb !FUPeival
         !      k = k + 1
         !      beta_orb_energy_diff(k) = DD_FUQbeta_eival(i) - DD_FUPbeta_eival(j)
         ...
         !write(molcfg%lupri,*)
         !write(molcfg%lupri,*) 'Alpha orbital energy differences  (occ index, virt index):' ; call ls_flshfo(molcfg%lupri)
         !write(molcfg%lupri,*) '=========================================================='
         !do i = 1, DD_nocca*DD_nvirta
         !   write(molcfg%lupri, '(i5, F18.7, i19, i11)') i, alfa_orb_energy_diff(i), alfaFUQFUPindex(i,1), alfaFUQFUPindex(i,2)
         !enddo

         !write(molcfg%lupri,*)
         !write(molcfg%lupri,*) 'Beta orbital energy differences  (occ index, virt index):' ; call ls_flshfo(molcfg%lupri)
         !write(molcfg%lupri,*) '========================================================='
         !do i = 1, DD_noccb*DD_nvirtb
         !   write(molcfg%lupri, '(i5, F18.7, i19, i11)') i, beta_orb_energy_diff(i), betaFUQFUPindex(i,1), betaFUQFUPindex(i,2)
         !enddo

         !!Sort orbital energy differences obtained from FUP/FUP eigenvectors to
         !!obtain initial guesses corresponding to nroots lowest orbital energy differences:

         !write(molcfg%lupri,*)
         !write(molcfg%lupri,*) '    Sorted orbital energy differences:'
         !write(molcfg%lupri,*) '=========================================='
         !do i = 1, nroots
         !   alfa = .false.
         ...
         !   if (alfamin < betamin .and. molcfg%solver%cfg_nocca > 0) then
         !      alfa = .true.
         !   endif
         !   if (alfa) then
         !      posmin = MINLOC(alfa_orb_energy_diff)
         !      posmax = MAXLOC(alfa_orb_energy_diff)
         !      write (molcfg%lupri, "('Root no.', i4, F12.7, '   Position:', i4, ' (ALPHA)')")&
         !            & i, alfa_orb_energy_diff(posmin(1)),  posmin(1)
         !      do mu = 1, ndim
         !         do nu = 1, ndim
         !            iniguess_full(mu,nu) = DD_FUQalfa_eivecs(mu,alfaFUQFUPindex(posmin(1),1))* &
         !                                &  DD_FUPalfa_eivecs(nu,alfaFUQFUPindex(posmin(1),2))
         !         enddo
         !      enddo
         !      alfa_orb_energy_diff(posmin(1)) = alfa_orb_energy_diff(posmax(1))

         !      call mat_unres_dense_part_from_full(iniguess_full,'a',iniguess)
         !      !call mat_set_from_full(iniguess_full,1.0d0, iniguess, 'iniguess')
         !   else
         !      posmin = MINLOC(beta_orb_energy_diff)
         !      posmax = MAXLOC(beta_orb_energy_diff)
         !      write (molcfg%lupri, "('Root no.', i4, F12.7, '   Position:', i4, ' (BETA)')")&
         !             & i, beta_orb_energy_diff(posmin(1)),  posmin(1)
         !      do mu = 1, ndim
         !         do nu = 1, ndim
         !            iniguess_full(mu,nu) = DD_FUQbeta_eivecs(mu,betaFUQFUPindex(posmin(1),1))* &
         !                                &  DD_FUPbeta_eivecs(nu,betaFUQFUPindex(posmin(1),2))
         !         enddo
         !      enddo
         !      beta_orb_energy_diff(posmin(1)) = beta_orb_energy_diff(posmax(1))

         !      call mat_unres_dense_part_from_full(iniguess_full,'b',iniguess)
         !      !call mat_set_from_full(iniguess_full,1.0d0, iniguess, 'iniguess')
         !   endif
         !   !Normalize X*S*X=1
         !   call chol_rsp_lintrans(decomp,iniguess,scr2,scr) !scr = rho
         !   fac = abs(mat_dotproduct(scr,iniguess))
         !   call mat_scal(1.0d0/sqrt(fac),iniguess)
         !   call x_from_oao_basis(decomp,iniguess,bvec_ao(i))
...
!  type(decompItem),intent(inout) :: decomp
  ...
  freqi=10000
  j=1
  THRESHOLD = molcfg%solver%rsp_conv_thr*1.d-2
  ...
  do KK=1,degeneratestates
     ...
     NEWXSOL(1)=eivecs(state1(KK))
     NEWXSOL(2)=eivecs(state2(KK))
     !ONLY NEED RHO
     ...
     TT = 1.D0/sqrt(ABS(mat_dotproduct(newXSOL(1),rho1)))
     call mat_scal(TT,NEWXSOL(1))
     
     eivecs(state1(KK)) = NEWXSOL(1)
     eivecs(state2(KK)) = NEWXSOL(2)         
