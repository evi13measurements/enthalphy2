...
!   call MAT_ADD(1.0d0, grad1, 1.0d0, grad2, gradtest)  !The gradients have opposite signs
...
!
!   call MAT_ADD(1.0d0, hes1, -1.0d0, hes2, hestest)
...
!   call util_diag(hes2,.false.,0.25d0,'ARH lowest hes eigenval: ')
!
!   call MAT_FREE(grad1)
!   call MAT_FREE(grad2)
   ...
   implicit none
      ...
      if (decomp%cfg_unres) then
         ...
         do m = 1, hesdim
            ...
            call arh_lintrans(solver,decomp,x_trial_mat,antisymmetric,0.0d0,scr)
         ...
         call util_diag(decomp%lupri,hessian,.false.,1.0d0,'Lowest Hessian eigenvalue: ')
...
!>
   ...
   implicit none
           ...
           TYPE(matrix)            :: hessian, x_trial, x_trial_mat, column, scr
           logical                 :: temp, alpha
           real(realk),allocatable :: fullhes(:,:) , parthes(:,:)
      ...
      call MAT_INIT(x_trial_mat,ndim,ndim)
      call MAT_INIT(column,hesdim,1)
      allocate(fullhes(2*hesdim,2*hesdim))

      do m = 1, 2*hesdim
         call MAT_ZERO(x_trial)
         call MAT_ZERO(x_trial_mat)
         if (m <= hesdim) then
            call mat_unres_dense_create_elm_alpha(m, 1, 1.0d0, x_trial)
         else
            call mat_unres_dense_create_elm_beta(m-hesdim, 1, 1.0d0, x_trial)
         endif
...
!            call mat_create_elm(i, j, 1.0d0, delta_i)
!            call mat_create_elm(j, i, -1.0d0, delta_i)
...
!            call MAT_ADD(beta, x, 1.0d0, delta_i, testmat)
...
!
!            lintra_elm = (e1-e2+e3-e4)/(4.0d0*alfa*beta)
!            lintra_elm = lintra_elm/4.d0 !linear transformation is divided by 4
...
!   
!   call mat_scal(0.25d0,g1) ; call mat_scal(0.25d0,g2)
!   call mat_add(1.d0,g1,-1.0d0, g2, scr1)
!
...
!
!   call mat_add(1.d0,scr1,-1.0d0, lintra, scr2)
!   sec_norm = sqrt(mat_sqnorm2(scr2))
!   call mat_add(1.d0,scr1,-1.0d0, arhnormal, scr2)
!   arhnormal_norm = sqrt(mat_sqnorm2(scr2))
!
!   !call mat_add(1.d0,scr1,1.0d0, arhlintra, scr2)
!   !arh_norm = sqrt(mat_sqnorm2(scr2))
!
!   !call mat_add(1.d0,lintra, 1.0d0, arhlintra, scr2)
!   !lintradiff = sqrt(mat_sqnorm2(scr2))
!
!   xnorm = sqrt(mat_sqnorm2(x))
!
!   write(lupri, "(4D13.5, '  :::')") sec_norm, arhnormal_norm, xnorm, graddiff
!
!   call mat_free(scr1)
!   call mat_free(scr2)
!                                                                                                           
...
!
!      !ARH lintrans:
!      call arh_lintrans(x,symm,0.0d0,lintraARH,fifoqueue)
!
!      !Turn off ARH terms and get RH lintrans:
!      dummy = cfg_arhterms 
!      cfg_arhterms = .false.
!      call arh_lintrans(x,symm,0.0d0,lintraRH,fifoqueue)
!      cfg_arhterms = dummy
!
!      !Turn on sec. order terms and get sec. order lintrans:
!      dummy = cfg_do_2nd_order
!      cfg_do_2nd_order = .true.
!      call chol_hessian_times_vector(x,symm,0.0d0,lintraExact) !lintra is E[2](x)
!      cfg_do_2nd_order = dummy
!
!      xnorm = SQRT(mat_sqnorm2(x))
!
!      CALL mat_add(1.d0,lintraExact,-1.0d0,lintraARH,scr1)
!      hesnormARH = SQRT(mat_sqnorm2(scr1))
!      ratioARH = hesnormARH/xnorm
!
!      CALL mat_add(1.d0,lintraExact,-1.0d0,lintraRH,scr1)
