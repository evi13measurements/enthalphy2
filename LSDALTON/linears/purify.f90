...
use matrix_operations
  ...
  real(realk), parameter :: thr=1d-12,dev_factor=20d0
  ...
  call mat_mul(Z,   F,'N','N',1d0,0d0,Dnew)
  call mat_mul(Dnew,Z,'N','T',1d0,0d0,Ft)
  ...
  if(method.eq.1.or.method.eq.2) then
     call mat_add(emax/(emax-emin),Id,-1d0/(emax-emin),Ft,D)
  ...
  do iter=1,maxiter
     ...
     call mat_mul(D,D,'N','N',1d0,0d0,D2)
     if(method.eq.1) then
        if(Ne_curr.lt.nocc) then
           call mat_add(2d0,D,-1d0,D2,D)
        else
           call mat_copy(1d0,D2,D)
        endif
     elseif(method.eq.2) then
        call mat_add(4d0,D,-3d0,D2,Dp)
        call mat_mul(D2,Dp,'N','N',1d0,0d0,Df)
        trf=mat_tr(Df)
        call mat_add(1d0,Id,-1d0,D,Dp)
        call mat_mul(Dp,Dp,'N','N',1d0,0d0,Dp2)
        call mat_mul(D2,Dp2,'N','N',1d0,0d0,Dg)
        trg=mat_tr(Dg)
        gamma=dble(nocc-trf)/trg
        if(gamma.gt.2d0) then
           call mat_add(2d0,D,-1d0,D2,D)
        elseif(gamma.lt.0d0) then
           call mat_copy(1d0,D2,D)
        else
           call mat_add(1d0,Df,-gamma,Dg,D)
        endif
     elseif(method.eq.3) then
        if(trace_error.gt.0.5d0) then
           ...
           call mat_mul(D,D2,'N','N',1d0,0d0,D3)
           call mat_add(3d0,D2,-2d0,D3,D)
        endif
     elseif(method.eq.4) then
        call mat_mul(D,D2,'N','N',1d0,0d0,D3)
        call mat_add(1d0,D2,-1d0,D3,T)
        cn=mat_tr(T)
        call mat_add(1d0,D,-1d0,D2,T)
        cn=cn/mat_tr(T)
        if(cn.lt.0.5d0) then
           call mat_add(1d0-2d0*cn,D,1d0+cn,D2,D)
           call mat_add(1d0,D,-1d0,D3,D)
           call mat_scal(1d0/(1d0-cn),D)
        else
           call mat_add((1d0+cn)/cn,D2,-1d0/cn,D3,D)
        endif
        Dsum=mat_sum(D)
  ...
  call mat_mul(Z,D,'T','N',1d0,0d0,L)
  call mat_mul(L,Z,'N','N',1d0,0d0,Dnew)
...
!----------------------------------------------------------------------
  ...
  real(realk), parameter :: tol=1d-7,beta=0.5d0
  integer :: ndim,i
  ...
  enddo
  alpha=min(beta/(emax-efermi),(1d0-beta)/(efermi-emin))
  call mat_add(alpha*efermi,Id,-alpha,F,D)
  call mat_add(1d0,D,beta,Id,D)
