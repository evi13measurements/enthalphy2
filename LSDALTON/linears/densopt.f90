...
CONTAINS
   ...
   !>
   SUBROUTINE DOPT_get_density(config,fifoqueue, queue, F, H1, D, SCF_it)
      IMPLICIT NONE
...
!** Initializations
      ...
      if (config%opt%cfg_density_method == config%opt%cfg_f2d_direct_dens) then
         Dnew = queue%D(queue%current_position)
      endif

      !** If virtual part of F should be scaled
      if (config%opt%cfg_scale_virt) call diag_scale_virt_fock(config%decomp%S,H1,F,D)
!
!** Level-shift ??
!
      IF(config%opt%cfg_density_method == config%opt%cfg_f2d_roothaan .and. &
       & config%diag%cfg_lshift /= config%diag%cfg_lshift_none) THEN
         !
         ! Level shifted mode - always shift F by some scalar * SDS
         !
         call mat_init(SDS,ndim,ndim)
         !** Find SDS once and for all :-)
         call mat_mul(D,config%decomp%S,'n','n',1.d0,0.d0,Dnew)
         call mat_mul(config%decomp%S,Dnew,'n','n',1.d0,0.d0,SDS)
         SDSexists = .true.
...
!
      if (.not. found_density .and. config%diag%cfg_no_confs_checked_in_rh) then 
        ...
        if (config%opt%CFG_density_method == config%opt%CFG_F2D_arh) then !For arh/DD it is not necessary to save the previous density
           call get_density(config,F,mu,D,fifoqueue)
        else
           call get_density(config,F,mu,Dnew,fifoqueue)
        endif
...
!
      ...
      rewind dens_lun
      call mat_write_to_disk(dens_lun,D)
...
!** Finalize      
   ...
   !>
   subroutine get_density(config,F,mu,Dnew,fifoqueue)
     implicit none
     !> Contains all info about configuration/settings for SCF calculation
     type(configItem), intent(inout)     :: config
     !> Fock/KS matrix
...
!test
     ...
     ndim = config%decomp%S%nrow
     shiftedF = .false.
     if (config%diag%cfg_lshift /= config%diag%cfg_lshift_none .and. &
     ...
     if (config%opt%cfg_density_method == config%opt%cfg_f2d_roothaan) then
       ...
       call arh_get_density(config%solver,config%decomp,F,fifoqueue,Dnew,SCF_iteration)
