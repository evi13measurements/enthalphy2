...
contains
   ...
   rsp_Ared = 0.0d0 ; rsp_Gred = 0.0d0 ; rsp_Sred = 0.0d0
   ...
   !else
      ...
      !call mat_copy(-1.0d0,res_in,scrmat(1))
      scrmat(1) = res_in
      call mat_scal(-1.0d0, scrmat(1))
   !endif
   !call flshfo(lupri)
   ...
   call mat_write_to_disk(lubrsp,b_current)
   call mat_write_to_disk(lusigmarsp,sigma)
   call mat_write_to_disk(lurhorsp,rho)
   ...
   !t_lintra = 0.0d0 ; t_ortho = 0.0d0 ; t_setup = 0.0d0
   !t_solve = 0.0d0  ; t_write = 0.0d0 ; t_precond = 0.0d0 ; t_testforsym = 0.0d0
   !call CPU_TIME(t2)
   !t_init = t2 -t1
   ...
   if (decomp%debug_rsp_linsca) then !Normalize X*S*X=1
      ...
      call mat_scal(1.0d0/sqrt(fac),x)
   ...
   do i = 1, iter-1  !Setup lower half of reduced E2 and S2
      call mat_read_from_disk(lusigmarsp,vec_i)
      ...
      call mat_read_from_disk(lurhorsp,vec_i)
      call mat_trans(vec_i, vec_i_trans)
   ...
   do i = 1, iter-1  !Setup upper half of reduced E2 and S2
      call mat_read_from_disk(lubrsp,vec_i)
      call mat_trans(vec_i, vec_i_trans)
   ...
   if (abs(error) > 1.0d-10) write (decomp%lupri,*) 'WARNING: Error on non-diag elms of reduced E2 ', error

   error = abs(rsp_Sred(1,2*iter) - rsp_Sred(2*iter,1))
   if (abs(error) > 1.0d-10) write (decomp%lupri,*) 'WARNING: Error on non-diag elms of reduced S2 ', error
  ...
  !TEST: remaining elms of reduced residual should be zero:
   ...
   do i = 1, iter
      call mat_read_from_disk(lusigmarsp,sigma_i)
      call mat_read_from_disk(lubrsp,b_i)
      call mat_read_from_disk(lurhorsp,rho_i)
   ...
   !    - res_in
      call MAT_ADD(1.0d0, sigma_i, omega, rho_i, scrmat)  !Stinne: SIGN CHANGED ON OMEGA!!! 10/7-2009
      ...
      call MAT_ADD(1.0d0, vec_i_trans, -omega, vec_i_trans2, scrmat)  !Stinne: SIGN CHANGED ON OMEGA!!! 10/7-2009
      call mat_daxpy(xred(2*i), scrmat, res)                          !NB: comments not changed!
   enddo
   call mat_daxpy(-1.0d0, res_in, res) !Subtract right-hand-side from residual
   ...
   omega = 1.0d6
   do i = 1, 2*iter
      if (ALFR(i) > 0.0d0 .and. ALFR(i) < omega) then
         omega = ALFR(i)
         chosen_eival = i
         eigenval_found = .true.
      endif
   ...
   !    + xred(3)*(sigma2 - omega*rho2) + xred(4)*(sigma2_T + omega*rho2_T)
   !    - res_in
      call MAT_ADD(1.0d0, sigma_i, -omega, rho_i, scrmat) 
      call mat_daxpy(xred(2*i-1), scrmat, res)

      call mat_trans(sigma_i, vec_i_trans)
      call mat_trans(rho_i, vec_i_trans2)

      call MAT_ADD(1.0d0, vec_i_trans, omega, vec_i_trans2, scrmat) 
   ...
   implicit none  
     ...
     call MAT_ADD(1.0d0,decomp%FUQ,-1.0d0,decomp%FUP,FQminusFP)
     !Sigma part of linear transformation, (FUQ-FUP)*X + X*(FUQ-FUP):
     call MAT_MUL(FQminusFP,x_in,'n','n',1.0d0,0.0d0,FX)
     call MAT_MUL(x_in,FQminusFP,'n','n',1.0d0,0.0d0,XF)
     call MAT_ADD(1.0d0,FX,1.0d0,XF,sigma)
     !Rho part, -omega*(DX - XD)
     call MAT_MUL(decomp%DU,x_in,'n','n',1.0d0,0.0d0,DX)
     call MAT_MUL(x_in,decomp%DU,'n','n',1.0d0,0.0d0,XD)
  ...
  !BE THE SAME AS THE CONVERGENCE WITH MO PRECONDITIONING)!!!!! 16/8 - 2006
     call MAT_ADD(1.0d0,DX,-1.0d0,XD,rho) !Sign changed 10/8-06 Stinne !Changed back 16/8-06!!!!
