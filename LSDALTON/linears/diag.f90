...
MODULE diagonalization
   ...
   use matrix_util
   PUBLIC :: dopt_get_density_INIT, DOPT_get_density_SHUTDOWN, get_didem, Dinp
...
implicit none
   ...
   diag%CFG_lshift_MOchange  = 1
   diag%CFG_lshift_search    = 2
   diag%cfg_lshift_dorth     = 3
   diag%CFG_lshift_none      = 4
   diag%CFG_lshift_vanlenthe = 5

   diag%CFG_lshift = diag%CFG_lshift_none !Default = no level shift
   print *, 'Number of chosen level shift:', diag%CFG_lshift
   diag%cfg_min_lshift        = 0.0d0
   diag%cfg_fixed_shift       = .false.
   diag%cfg_fixed_shift_param = 0.0d0
...
!> \param diag Used to store info about diagonalization
   subroutine dopt_get_density_INIT(Ndim,param1,param2,diag)
      ...
      if (diag%CFG_lshift .ne. diag%cfg_lshift_none) then
      ...
      if (diag%cfg_lshift == diag%cfg_lshift_MOchange .or. diag%debug_rh_mu_E) then
...
!>
   ...
   use opttype
      ...
      if (diag%cfg_lshift == diag%cfg_lshift_none) then
         !do no lshift, so return 0 for mu
         mu=0d0 
      else if (diag%cfg_lshift == diag%cfg_lshift_search) then
      ...
      else if (diag%cfg_lshift == diag%cfg_lshift_dorth) then
      ...
      else if (diag%cfg_lshift == diag%cfg_lshift_MOchange) then
         ...
         if (diag%info_rh_mu) WRITE(diag%LUPRI,*) 'using mu_mochange or mu_mindamp = ',&
              & mu,'  grep'
      else if (diag%cfg_lshift == diag%cfg_lshift_vanlenthe) then
...
!> \param Dnew Density matrix
   subroutine diag_f_to_get_d(diag,F,S,Dnew)
     ...
     allocate(eival(sz)) ! allow for unrestricted

     call mat_diag_f(F,S,eival,Cmo)

     call density_from_orbs(diag%cfg_unres,diag%nocc,diag%nocca,diag%noccb,Cmo,Dnew) 
     ...
     if (diag%cfg_save_CMO) then
       ...
       rewind cmo_lun
       call mat_write_to_disk(cmo_lun,Cmo)
       write(cmo_lun) Ggap
...
!
      do
         ...
         if (ratio_r > limit_ratio*0.9d0 .or. (mu_r - mu_l) < 0.3d0) then 
!         if (ratio_r > limit_ratio*0.9d0 .or. (mu_r - mu_l) < 0.05d0) then !Stinne - didn't use this
            ...
            if (diag%cfg_no_conf_shift) then
               ...
               if (ratio_r < limit_ratio*0.1d0) then
               !if (ratio_r < limit_ratio*0.05d0) then !Stinne - didn't use this after all. cfg_no_conf_shift
                                                       !instead
                  diag%cfg_no_confs_checked_in_rh = .false.
...
!> \param ratio ||Dorth||/||D||
   subroutine ratio_Dorth_D(av,queue,S,Dmu,ratio)
      ...
      call mat_add(1.d0,Dmu,-1.d0,Dpara,Dorth)
...
!>
   subroutine check_conf(av,diag,F,S,mu_r,queue,Dnew,mu,mochange)
      ...
      !For mu = 0:
      call diag_Get_density(diag,F,S,0.0d0,Dnew)
      mochange_0 = Gmochange
...
!
      n = 0
      nfock = 0
      mu0 = MAX(stat_tab(stat_current_iteration,7) ,0.0d0) !mu in last iteration
      mus = 0.0d0
      call mat_init(Dnew,S%nrow,S%ncol)
...
!> \param ndia Number of diagonalizations in this SCF iteration
   subroutine print_and_stat(av,diag,F,D,S,H1,Dnew,queue,mu,ndia)
      ...
      !    Epred_0 = 0.0d0
      !  endif
      !  Eorb_idem = 2.d0*mat_dotproduct(F,Didem)
      !endif
...
!
       ...
       if (diag%cfg_lshift == diag%cfg_lshift_MOchange) then
         stat_tab(stat_current_iteration+1,8) = Gmochange
       elseif (diag%cfg_lshift == diag%cfg_lshift_dorth) then
         call ratio_Dorth_D(av,queue,S,Dnew,ratio)
         stat_tab(stat_current_iteration+1,8) = ratio
       else
         stat_tab(stat_current_iteration+1,8) = 0.0d0
       endif
 
       !** col4 for RH E-model change
       if (diag%DEBUG_RH_DSM_ECHANGE .or. diag%DEBUG_EMODEL_CHANGE .or. diag%DEBUG_RH_MU_E) then
         orbE = 2.d0*mat_dotproduct(F,Didem)
...
!> \param Cmo MO coefficients
   subroutine GET_Didem(diag,Dmu,S,Didem,Cmo)
      ...
      call mat_mul(Dmu,S,'n','n',1.d0,0.d0,DS)
      call mat_mul(S,DS,'n','n',-1.d0,0.d0,SDmuS)
      if(PRESENT(Cmo)) THEN
         ALLOCATE(eig(S%nrow*2)) ! allow for unrestricted
         call mat_diag_f(SDmuS,S,eig,Cmo)
         call density_from_orbs(diag%cfg_unres,diag%nocc,diag%nocca,diag%noccb,Cmo,Didem)
         if (diag%debug_idempotency) then
         ...
         DEALLOCATE(eig)
      ELSE
         call diag_get_density(diag,SDmuS,S,0.0d0,Didem)
      END IF
      call mat_free(DS)
      call mat_free(SDmuS)
...
!            Eorb_slut = 2.d0*mat_dotproduct(F,Dnew)
!            if (queue%used_entries > 0) then
!              !Find dEpred
...
!            dmuold = dmu
!            n = n + 1
!            if (n*dmu >= mumax/4.d0) then
!              n = 0
!              dmu = MAX(dmu/2.0d0,0.15d0)
!            endif
!         enddo
...
!         do 
!            CFG_lshift = CFG_lshift_none
!            CFG_density_method = CFG_F2D_DIRECT_DENS
!            if (debug_dd_limit_found) exit
!            !if (mu < 0.6d0*abs(cfg_mu_dd)) exit !cfg_mu_dd is the necessary damping found, 
...
!            mochange = Gmochange
!            !Find ERH_mu to use for dERH = ERH_mu - ERH_0
!            Eorb_slut_diag = 2.d0*mat_dotproduct(F,Dnew)
!            !if (queue%used_entries > 0) then
!            !  !Find dEpred
!            !  dEpred = roothan_Epred(queue,H1,S,Dnew) - Epred_0
!            !  !Find ratio = ||Dorth||/||D||
...
!> \param minpr = TrDoldSDnewS/sqrt(TrDoldSDoldS*TrDnewSDnewS)
   SUBROUTINE Cchange(Didem, Dnonidem, S, minpr)
     ...
     call mat_mul(S,Dnonidem,'n','n',1.d0,0.d0,SDnew)
     call mat_mul(SDnew,S,'n','n',1.d0,0.d0,SDnewS)
     !TrDnewSDoldS
     minpr = mat_dotproduct(Didem,SDnewS)
     !TrDoldSDoldS
     nrm1  = mat_dotproduct(Dnonidem,SDnewS)
     call mat_mul(S,Didem,'n','n',1.d0,0.d0,SDnew)
     call mat_mul(SDnew,S,'n','n',1.d0,0.d0,SDnewS)
     !TrDnewSDnewS
...
!           & + 2.0d0*mat_dotproduct(Fpara,Dmu)  &
!           & - 2.0d0*mat_dotproduct(Fpara,Dpara) &
!           & + POTNUC  
!      call mat_init(Dchange,dmu%nrow,dmu%ncol)
!      call mat_add(1.0d0,Dmu,-1.0d0,Dpara,Dchange)
...
!      call DGEMM('T','n',ndim,ndim,ndim,1.0d0,CMO,ndim,Sfull,ndim,0.0d0,CTS,ndim)
!      call DGEMM('n','n',ndim,ndim,ndim,1.0d0,CTS,ndim,CMO,ndim,0.0d0,xnorm,ndim)
...
!test
     ...
     shiftedF = .false.
     if (diag%cfg_lshift /= diag%cfg_lshift_none .and. ABS(mu) > 1.d-8) then
       !Evaluate the shifted Fock matrix
       ! Fdamp = F - muSDS
        call mat_init(Fdamp,S%nrow,S%nrow)
        CALL mat_add(1.d0,F,-mu, SDS, Fdamp)
        shiftedF = .true.
     endif
     ! Diagonalization
     if(shiftedF) then
...
!>
   subroutine diag_scale_virt_fock(S,H1,F,D)
...
!Returns the overlap of the new MO with the smallest overlap to the old MOs
   subroutine util_min_MO_overlap(cfg_nocc,S,Cold,Cnew,noverlap,overlap)
      ...
      call mat_mul(Cold,S,'t','n',1.d0,0.d0,ColdTS)
      call mat_mul(ColdTS,Cnew,'n','n',1.d0,0.d0,ColdTSCnew)
