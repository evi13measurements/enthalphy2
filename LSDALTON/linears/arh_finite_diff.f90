...
contains
   ...
   !Get [D,X]:
   call MAT_MUL(fifoqueue%D_exp,x,'n','n',1.0d0,0.0d0,wrk1)
   call mat_trans(wrk1,wrk2)
   call mat_scal(-1.0d0,wrk2)
   call MAT_ADD(1.0d0, wrk1, -1.0d0, wrk2, com)
   ...
   do j = 1, ndens
      ...
      TrcomF = mat_trAB(com,Fdiff)
         ...
         do k = 1, ndens
            ...
            call MAT_ADD(1.0d0, Dk, -1.0d0, D0, Ddiffk)
            ...
            do l = 1, ndens
               ...
               call MAT_ADD(1.0d0, Dl, -1.0d0, D0, Ddiffl)
               !TrcomDi = Tr ([D,X]*(Di - D_0))
   ...
   !Calculate gradient:
      !gradient_contrib = (e_plus1-e_minus1)/(2.0d0*small) !gradient to lower order
      gradient_contrib = (8.0d0*(e_plus1-e_minus1) - (e_plus2-e_minus2))/(12.0d0*small)
      call mat_create_elm(i, 1, gradient_contrib, gradient)     

   !Calculate diagonal hessian elements:
      diag_hes = (16.0d0*(e_plus1+e_minus1-2.0d0*e0) - (e_plus2+e_minus2-2.0d0*e0))/(12.0d0*small**2)
   ...
   do i = 1, vecdim
      ...
      !Ep2p2:
         call mat_create_elm(j, 1, 2.0d0*small, X_trial_vec)
         call mat_create_elm(i, 1, 2.0d0*small, X_trial_vec) 
         call mat_VEC_TO_MAT('a', X_trial_vec, X_trial_mat) 
      ...
      non_diag_hes = (16.0d0*G1 - G2)/(48.0d0*small**2)
