module COMPLEXSYMSOLVER
  ...
  !use configuration
  private
  public ::  rsp_sym_complex_solver,sym_complex_solver, rsp_sym_complex_init
...
if ((abs(molcfg%solver%rsp_gamma) .LT. 1d-8) .and. (.not. gd_complex)) then
...
type(Matrix)                        :: Xsolvec(rsp_number_of_rhs),sigmas(rsp_number_of_rhs)
type(Matrix)                        :: Xsolveci(rsp_number_of_rhs),sigmasi(rsp_number_of_rhs)
type(Matrix)                        :: rhos(rsp_number_of_rhs),rhosi(rsp_number_of_rhs)
...
ndim = S%nrow
  ...
  red_Xp_glob = 0.0d0 ; red_Xm_glob = 0.0d0
  red_Xpi_glob = 0.0d0 ; red_Xmi_glob = 0.0d0
  E1=0d0; E2=0d0; S1=0d0; S2=0d0; G1=0d0; G2=0d0
  E3=0d0; E4=0d0; S7=0d0; S8=0d0; G3=0d0; G4=0d0
...
!get start vectors by preconditioning RHS with efficient preconditoner by Villaume et al.
   ...
   nmi_new = nx_new
     ...
     do j=1,max(nx_new,nm_new)
      ...
      enddo
do j=1,nx_new
call mat_add(1d0,xp(j),1d0,xm(j),xsolvecp(j))
call mat_add(1d0,xpi(j),1d0,xmi(j),xsolvecpi(j))
enddo
...
!   call mat_add(1d0,xsolvecp(j),1d0,xsolvecm(j),xsolvec(j))
!   enddo
...
!   call mat_add(1d0,xsolvecpi(j),1d0,xsolvecmi(j),xsolveci(j))
!   enddo
...
!  endif
    ...
    call mat_add(0.5d0,sigmas(j),0.5d0,scrT,sigmasp(j))
    call mat_add(0.5d0,sigmas(j),-0.5d0,scrT,sigmasm(j))
    
    call mat_trans(rhos(j),scrT)  
    
    call mat_add(0.5d0,rhos(j),0.5d0,scrT,rhosp(j))
    call mat_add(0.5d0,rhos(j),-0.5d0,scrT,rhosm(j))

call mat_trans(xsolvec(j),scrT)
      
    call mat_add(0.5d0,xsolvec(j),0.5d0,scrT,xsolvecp(j))
    call mat_add(0.5d0,xsolvec(j),-0.5d0,scrT,xsolvecm(j))
   call mat_free(scrT)
    enddo
!    if (nx_new>nm_new) then
!      do j=nm_new+1,nx_new
...
!   endif 
    ...
    call mat_add(0.5d0,sigmasi(j),0.5d0,scrT,sigmaspi(j))
    call mat_add(0.5d0,sigmasi(j),-0.5d0,scrT,sigmasmi(j))
    
    call mat_trans(rhosi(j),scrT)  
    
    call mat_add(0.5d0,rhosi(j),0.5d0,scrT,rhospi(j))
    call mat_add(0.5d0,rhosi(j),-0.5d0,scrT,rhosmi(j))

call mat_trans(xsolveci(j),scrT)  
        
    call mat_add(0.5d0,xsolveci(j),0.5d0,scrT,xsolvecpi(j))
    call mat_add(0.5d0,xsolveci(j),-0.5d0,scrT,xsolvecmi(j))
   ...
   call mat_free(scrT)
    ...
    if (nxi_new>nmi_new) then
      do j=nmi_new+1,nxi_new
         sigmaspi(j)=sigmasi(j)
         rhosmi(j)=rhosi(j)
      enddo
    elseif (nmi_new>nxi_new) then
      do j=nxi_new+1,nmi_new
         sigmasmi(j)=sigmasi(j)
         rhospi(j)=rhosi(j)
      enddo
   endif 
...
!        !build a reduce space
   ...
   do j = 1,max(nx_new,nm_new)
       call mat_write_to_disk(lu_x_rsp,xsolvec(j))
       call mat_write_to_disk(lu_sigma_rsp,sigmas(j))
       call mat_write_to_disk(lu_rho_rsp,rhos(j))
   enddo
   ...
   do j = 1,max(nxi_new,nmi_new)
       call mat_write_to_disk(lu_xi_rsp,xsolveci(j))
       call mat_write_to_disk(lu_sigmai_rsp,sigmasi(j))
       call mat_write_to_disk(lu_rhoi_rsp,rhosi(j))
   enddo
   ...
   nti_red=nti_red+max(nxi_new,nmi_new)
    ...
    if (conv)  then
        do l=1,ngd
           ...
           call mat_add(1d0,xp(l),1d0,xm(l),xsol(l))
           call mat_add(1d0,xpi(l),1d0,xmi(l),xsolimg(l))
...
if (nxi_new>0) then
   ...
   if (nmi_new>0) then
    ...
    rewind(lu_sigma_rsp) 
    do j = 1, nt_red
       call mat_read_from_disk(lu_sigma_rsp,sigma_j)
       call mat_trans(sigma_j,scrT)
       call mat_add(0.5d0,sigma_j,0.5d0,scrT,sigmap_j)
       call mat_add(0.5d0,sigma_j,-0.5d0,scrT,sigmam_j)
       
       if ((nx_new>0) .and. (sqrt(mat_sqnorm2(sigmap_j))>1d-9)) then
       k = 0
       do i = n_red+1, n_red+nx_new
          k = k + 1
      ...
      if ((nm_new>0) .and. (sqrt(mat_sqnorm2(sigmam_j))>1d-9)) then
           k = 0
           do i = nm_red+1, nm_red+nm_new
              k = k + 1
   ...
   enddo
    ...
    do j = 1, nti_red
       call mat_read_from_disk(lu_sigmai_rsp,sigmai_j)
       call mat_trans(sigmai_j,scrT)
       call mat_add(0.5d0,sigmai_j,0.5d0,scrT,sigmapi_j)
       call mat_add(0.5d0,sigmai_j,-0.5d0,scrT,sigmami_j)
       
       if ((nxi_new>0).and. (sqrt(mat_sqnorm2(sigmapi_j))>1d-9)) then
       k = 0
       do i = ni_red+1, ni_red+nxi_new
          k = k + 1
       ...
       if ((nmi_new>0).and. (sqrt(mat_sqnorm2(sigmami_j))>1d-9)) then
       k = 0
       do i = nmi_red+1, nmi_red+nmi_new
          k = k + 1
    ...
    call mat_zero(rho_j)
    do j = 1, nt_red
       call mat_read_from_disk(lu_rho_rsp,rho_j)
       call mat_trans(rho_j,scrT)
       call mat_add(0.5d0,rho_j,0.5d0,scrT,rhop_j)
       
       if (sqrt(mat_sqnorm2(rhop_j))>1d-9) then
      if (nx_new>0) then 
       k = 0
       do i = n_red+1, n_red+nx_new
          k = k + 1
    ...
    do j = 1, nti_red
       call mat_read_from_disk(lu_rhoi_rsp,rhoi_j)
       call mat_trans(rhoi_j,scrT)
       call mat_add(0.5d0,rhoi_j,0.5d0,scrT,rhopi_j)
       
       if (sqrt(mat_sqnorm2(rhopi_j))>1d-9) then
     if (nx_new>0) then
          k = 0
         do i = n_red+1, n_red+nx_new
          k = k + 1
    ...
    do i = 1, nt_red
       call mat_read_from_disk(lu_x_rsp,x_j)
       call mat_trans(x_j,scrT)
       call mat_add(0.5d0,x_j,0.5d0,scrT,xp_j)
       call mat_add(0.5d0,x_j,-0.5d0,scrT,xm_j)
    
  if (sqrt(mat_sqnorm2(xp_j))>1d-9) then
    if (nx_new>0) then
       k = 0
    ...
    if ((nm_new>0) .and. (sqrt(mat_sqnorm2(xm_j))>1d-9)) then
       k = 0
    ...
    do i = 1, nti_red
       call mat_read_from_disk(lu_xi_rsp,xi_j)
       call mat_trans(xi_j,scrT)
       call mat_add(0.5d0,xi_j,0.5d0,scrT,xpi_j)
       call mat_add(0.5d0,xi_j,-0.5d0,scrT,xmi_j)
    
  if (sqrt(mat_sqnorm2(xpi_j))>1d-9) then
    if (nxi_new>0) then
       k = 0
    ...
    if ((nmi_new>0).and. (sqrt(mat_sqnorm2(xmi_j))>1d-9)) then
       k = 0
    ...
    call mat_add(0.5d0,gd(j),0.5d0,gT,gdp)
    call mat_add(0.5d0,gd(j),-0.5d0,gT,gdm)
    ...
    call mat_add(0.5d0,gdi(j),0.5d0,gT,gdpi)
    call mat_add(0.5d0,gdi(j),-0.5d0,gT,gdmi)
   endif
   call mat_free(gT)
...
type(prop_molcfg), intent(inout)    :: molcfg
    ...
    S1p=0d0; S2p=0d0; S3m=0d0; S4m=0d0
    S5p=0d0; S6p=0d0; S7m=0d0; S8m=0d0
...
call expand_on_basis4(molcfg,nti_red,ndim,red_Xmi_glob(1:nmi_red,ibx),red_Xpi_glob(1:ni_red,ibx),lu_rhoi_rsp,&
       ...
       S2XR(ibx)=residualsm(ibx)
       S2XRi(ibx)=residualsmi(ibx)
       S2XI(ibx)=residualsp(ibx)
       S2XIi(ibx)=residualspi(ibx)
 ...
 call expand_on_basis4(molcfg,nti_red,ndim,red_Xpi_glob(1:ni_red,ibx),red_Xmi_glob(1:nmi_red,ibx),lu_sigmai_rsp,&
       ...
       call mat_scal(-1d0,residualspi(ibx))
       call mat_scal(-1d0,residualsmi(ibx))
      ! subtract gradient
    call mat_init(gdp,ndim,ndim)  
    call mat_init(gdm,ndim,ndim) 
    call mat_init(gT,ndim,ndim)
    ...
    call mat_add(0.5d0,gd(ibx),0.5d0,gT,gdp)
    call mat_add(0.5d0,gd(ibx),-0.5d0,gT,gdm)
    ...
    call mat_add(0.5d0,gdi(ibx),0.5d0,gT,gdpi)
    call mat_add(0.5d0,gdi(ibx),-0.5d0,gT,gdmi)
   endif
   ...
   call mat_free(gT)
      ...
      call mat_daxpy(-1.0d0,gdp,residualsp(ibx))
      call mat_daxpy(-1d0,gdm,residualsm(ibx))
      if (gd_complex) then
      call mat_daxpy(1d0,gdpi,residualspi(ibx))
      call mat_daxpy(1d0,gdmi,residualsmi(ibx))
      endif
...
if (.not. conv) then     
    ...
    call mat_init(gT,ndim,ndim)
     ...
     call mat_scal(-1d0,xpi(nx_new))
     call mat_scal(-1d0,xmi(nx_new))
...
type(prop_molcfg), intent(inout)    :: molcfg
 ...
 gammma=molcfg%solver%rsp_gamma
  ...
  do j=1,ngd
     ...
     call mat_add(0.5d0,RHS_real(j),0.5d0,gT,gdp)
     call mat_add(0.5d0,RHS_real(j),-0.5d0,gT,gdm)
     call mat_trans(RHS_img(j),gT)  
     call mat_add(0.5d0,RHS_img(j),0.5d0,gT,gdpi)
     call mat_add(0.5d0,RHS_img(j),-0.5d0,gT,gdmi)
