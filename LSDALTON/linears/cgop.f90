...
do j=1,ngrad 
    call mat_copy(-1.0d0,Grad(j),x(j))
    ...
    !get res
    call mat_add(1d0,Grad(j),-1d0,sigmai(j),res(j))
...
! 
    if (.not. no_pairing) then
      ...
      call mat_add(1d0,sigmaiT,omega(m),rhoiT,ArP)
    endif
...
!     endif
!else
!    call OUTPUT(alpha,1,2*redsize,1,1,1,1,1,lupri)
!    write(lupri,*) 'Bmat'!, alpha(1:2*redsize); call flshfo(lupri)
!          do i=1,2*redsize
...
call util_scriptPx('N',D,S,p_i) 
    ...
    call mat_trans(rhoi(m),rhoiT)
    !get Ap
    call mat_add(1d0,sigmai(m),-omega(m),rhoi(m),Ap)
    !get ApP
    if (.not. no_pairing) then 
        call mat_add(1d0,sigmaiT,omega(m),rhoiT,ApT)
    endif
    call mat_daxpy(1d0,p_i,x(m))
     call util_scriptPx('N',D,S,x(m)) 
    call add_to_modFIFO4(vectorsubspace,x(m),p_i,Ap,ApT,0d0,.false.)
    enddo
...
!---------------------------------------------------------------------
 ...
 call mat_scal_dia(0.5d0,Bvec_tmp)
        sym_or_antisym = .false.
      endif
