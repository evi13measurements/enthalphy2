  ...
  if (opt%cfg_which_conversion == 1) then !Write density in formatted form in dens.format, 
     ...
     if (matrix_type==mtype_dense) then
        ...
        !Convert the read sparse density to a standard fortran matrix:
           ...
           call mat_to_full(Dsparse,1.0d0,Dfull)
        !Deallocate Dsparse and set matrix type to dense:
           call mat_free(Dsparse)
           CALL mat_select_type(mtype_dense)
     ...
     if (matrix_type==mtype_dense) then
        ...
        !Hack: set matrix type to BSM
           ...
           opt%CFG_prefer_BSM = .TRUE.
           call mat_init(Dsparse,ndim,ndim)
           call mat_read_from_disk(luconvert,Dsparse)
        !Convert the read sparse density to a standard fortran matrix:
           allocate(Dfull(ndim,ndim))
           call mat_to_full(Dsparse,1.0d0,Dfull)
        !Deallocate Dsparse and set matrix type to dense:
...
!     call mat_print(S,1,S%nrow,1,S%ncol,lupri)
     ...
     CALL LSTIMER('NUCREP ',tstart,tend,lupri)
...
!     write (lupri,*) 'Overlap matrix:'
     ...
     call mat_mul(prod2,prod2,'n','n',1.d0,0.d0,prod1)
     print *, "prod1:"
...
!     call mat_mul(S,S,'n','n',1.d0,0.d0,prod1)
!     call MAT_PRINT(prod1, 1, nsize, 1, nsize, lupri)
!     call MAT_PRINT(S, 1, nsize, 1, nsize, lupri)
...
!     call mat_csr_add(S,prod1,'n','n',1.d0,ra)
!     call MAT_PRINT(ra, 1, nsize, 1, nsize, lupri)
     ...
     call mat_mul(ra,S,'n','n',1.d0,0.d0,prod2)
...
!     call mat_mul(S,h,'n','n',1.d0,0.d0,prod1)
     
!     write (lupri,*) 'Test matrix:'
!     call MAT_PRINT(prod1, 1, nsize, 1, nsize, lupri)   
!     
!     !Multiply test matrix by itself:
!    call mat_mul(prod1,prod1,'n','n',1.d0,0.d0,prod2)
!     
