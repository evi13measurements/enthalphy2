module COMPLEXSOLVER
  ...
  use rsp_util
  private
  public ::  rsp_complex_solver,complex_solver_check,real_solver_check,complex_solver, &
          &  rsp_complex_init,rsp_complex_simult
...
type(Matrix)                        :: sig_prec(2*rsp_number_of_rhs),rho_prec(2*rsp_number_of_rhs)
...
real(realk)                         :: absorp, disper, rsp_thresh
 ...
 rsp_thresh=molcfg%solver%rsp_thresh
   ...
   red_GD_glob = 0.0d0
   red_E_glob = 0.0d0 ; red_S_glob = 0.0d0 ; red_X_glob = 0.0d0 ; red_Ximg_glob = 0.0d0
   ...
   call get_start_vectors(molcfg,F,D,S,RHS_real,RHS_img,ngd,eival,nx_new,xsolvector)
    ...
    if (conv)  then
        do l=1,ngd
           ...
           call expand_on_basis(molcfg,n_red,ndim,RED_X_glob(1:n_red*2,l),lu_x_rsp,XSOL(l))
           rewind(lu_x_rsp)
           call mat_zero(XSOLimg(l))
           call expand_on_basis(molcfg,n_red,ndim,RED_Ximg_glob(1:n_red*2,l),lu_x_rsp,XSOLimg(l))
        enddo
        do j=1,n_v
...
call mat_init(x_jT,ndim,ndim)
    ...
    enddo
        ...
        !save on disk
    do i = 1,nx_new
       call mat_write_to_disk(lu_x_rsp,xsolvec(i))
       call mat_write_to_disk(lu_sigma_rsp,sigmas(i))
       call mat_write_to_disk(lu_rho_rsp,rhos(i))
...
! STEP 1: check initial linear dependencies among new vectors
   ...
   !
       ...
       rewind(lu_x_rsp)
       do k = 1,Nb_prev
                  call mat_read_from_disk(lu_x_rsp,b_k)
          do irx = 1,Nb_new
...
call mat_daxpy(1.d0,gd,S2XI)
if (gd_complex) then
call mat_daxpy(1.d0,gdi,S2XR)
endif

call mat_add(1.d0,S2XI,-1.d0,E2XR,res_real)
call mat_add(1.d0,S2XR,-1.d0,E2XI,res_img)
...
!  red_X:    the ngd reduced space real solution vectors
    ...
    do igd = 1,ngd
       if (gd_complex) then
           ...
           KHS(1:2*ndim_red) = CMPLX(red_GD_glob(1:2*ndim_red,igd),0d0)
       endif
       if (molcfg%solver%info_rsp) then
...
! 
    do ibx = 1,ngd
      !get the frequency
       ...
       rewind(lu_rho_rsp)
       rewind(lu_sigma_rsp)
       call expand_on_basis_minus(molcfg,ndim_red,ndim,red_X_glob(1:ndim_red*2,ibx),lu_rho_rsp,residuals(ibx))
       S2XR(ibx)=residuals(ibx)
       call mat_scal(-wibx,residuals(ibx))
       !add  sum_j ^RX_j(ibx) sigma_j
       call expand_on_basis(molcfg,ndim_red,ndim,red_X_glob(1:ndim_red*2,ibx),lu_sigma_rsp,residuals(ibx))
       
       call mat_init(residuals_img(ibx),ndim,ndim)
       call mat_zero(residuals_img(ibx))
       rewind(lu_rho_rsp)
       rewind(lu_sigma_rsp)
       call expand_on_basis_minus(molcfg,ndim_red,ndim,red_Ximg_glob(1:ndim_red*2,ibx),lu_rho_rsp,residuals_img(ibx))
       S2XI(ibx)=residuals_img(ibx)
       call mat_scal(-wibx,residuals_img(ibx))
       !add  sum_j ^RX_j(ibx) sigma_j
       call expand_on_basis(molcfg,ndim_red,ndim,red_Ximg_glob(1:ndim_red*2,ibx),lu_sigma_rsp,residuals_img(ibx))
     
      ! subtract gradient
      rhs(ibx)=gd(ibx)
      call mat_daxpy(-gammma,S2XI(ibx),rhs(ibx))
      call mat_daxpy(-1.0d0,rhs(ibx),residuals(ibx))
      
      if (gd_complex) then
          call mat_init(rhsi(ibx),ndim,ndim)
          rhsi(ibx)=gdi(ibx)
          call mat_daxpy(-1.0d0,rhsi(ibx),residuals_img(ibx))
      endif
   ...
   if (.not. conv) then     
       do j=1,ngd
          if (.not. conv_root_img) then 
             nx_new=nx_new+1
             xsolvector(nx_new)=residuals_img(j)
          endif    
          if (.not. conv_root) then 
             nx_new=nx_new+1
             xsolvector(nx_new)=residuals(j)
          endif
       enddo
   endif
