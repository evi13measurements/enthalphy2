...
MODULE scf_stats
   ...
   subroutine scf_stats_init(opt)
      ...
      maxit = opt%cfg_max_linscf_iterations

      NULLIFY(stat_tab,stat_gradnorm,stat_energy)
      ALLOCATE(stat_tab(maxit,11),stat_gradnorm(maxit,2),stat_energy(maxit))
   ...
   subroutine scf_stats_print_table(opt,iteration)
      ...
      !         &stat_gradnorm(iteration-1,1)
      if (opt%cfg_density_method == opt%cfg_f2d_arh) then
        print'(i3,f18.10,f17.11,f8.2,f13.5,f8.2,2e12.2)', iteration,stat_tab(iteration,1),&
        ...
        WRITE(opt%LUPRI,'(i3,f18.10,f17.11,f8.2,f13.5,f8.2,2e12.2,"  ###")') iteration,stat_tab(iteration,1),&
             &stat_tab(iteration,2),stat_tab(iteration,6),stat_tab(iteration,11),stat_tab(iteration,7),&
             &stat_gradnorm(stat_current_iteration,2),stat_gradnorm(iteration,1)
      else
        print'(i3,f18.10,f17.11,f8.2,f13.5,f8.2,f13.7,e12.2)', iteration,stat_tab(iteration,1),&
             &stat_tab(iteration,2),stat_tab(iteration,6),stat_tab(iteration,11),(stat_tab(iteration,j),j=7,8),&
             &stat_gradnorm(iteration,1)
        WRITE(opt%LUPRI,'(i3,f18.10,f17.11,f8.2,f13.5,f8.2,f13.7,e12.2,"  ###")') iteration,stat_tab(iteration,1),&
             &stat_tab(iteration,2),stat_tab(iteration,6),stat_tab(iteration,11),(stat_tab(iteration,j),j=7,8),&
             &stat_gradnorm(iteration,1)
      endif
