...
contains
      ...
      if (pd==1 .and. allocated(rsp_eq_sol)) then
         ...
         do i=1, size(rsp_eq_sol)
            ...
            if (all(rsp_eq_sol(i)%fld%freq == fld%freq)) then
               ...
               Dp(1) = (-1d0)**nanti * dag(rsp_eq_sol(i)%D)
            end if
   ...
   subroutine rsp_solve_eq(mol, S0, D0, F0, sym, freq, neq, DSDp, FDSp, Dp, Fp)
      ...
      do i=1, neq
         ...
         if (rhs_norm < mol%solver%rsp_thresh / 10) then
            ...
            Xph(1) = 0d0 * S0
         else if (FDSp(i)%complex .or. dabs(mol%solver%rsp_gamma) > 1e-20 &
                 & .or. dabs(dimag(freq)) > 1e-20 ) then
            !on the first run, initialize the solvers
            if (first_complex) then
              if (mol%solver%rsp_new_solver) then
            ...
            mol%solver%rsp_gamma = mol%solver%rsp_gamma + dimag(freq)
            ...
            if (mol%solver%rsp_new_solver) then
            call rsp_sym_complex_solver(mol, F0, D0, S0, 1, reFDSp(1:1), &
                                    freq1, 1, reXph(1:1), imXph(1:1),       &
                                    .true., gdi=imFDSp(1:1))
            else
            call rsp_complex_solver(mol, F0, D0, S0, 1, reFDSp(1:1), &
                                    freq1, 1, reXph(1:1), imXph(1:1),       &
                                    .true., gdi=imFDSp(1:1))
            endif 
            ...
            if (first) then
              if ((mol%solver%rsp_new_solver) .and. (dabs(dreal(freq)) .gt.  1d-6)) then
            ...
            if (.not.allocated(iexci)) then
               if (mol%solver%rsp_new_solver) then
                  if (dabs(dreal(freq)) .gt.  1d-6) then
   ...
   subroutine scf_eq_find_resonances(mol, freq, iexci)
      ...
      thr = 1d-5 !ajt FIXME mol%solver%degenerateTHR
      do iter = 1,2    !iter=1 counts degeneracy, then allocates iexci
         ...
         !ajt FIXME if (iter==1 .and. deg/=0 .and. &
         !ajt FIXME      .not.mol%solver%degeneratestates) then
         !ajt FIXME    allocate(iexci(1))
         !ajt FIXME    iexci(1) = imin
         !ajt FIXME    exit
         !ajt FIXME end if
