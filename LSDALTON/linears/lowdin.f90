...
#define MAX_ITER 50
     ...
     real(realk)                  :: ab(2)
   ! wake slaves
     call mpixbcast(11,1,'INTEGER',0)
     call mpixbcast(0, 1,'INTEGER',0)
   ! communicate matrix type and size
     call mpixsend(matrix_type,1,'INTEGER',1,0)
     call mpixsend(S%nrow,     1,'INTEGER',1,1)
#endif
   ...
   ! T1 should converge to this parameter
     t1_converged=sqrt(1.0d0*S%ncol)
   ...
   ! do iterations
     do iter=1, MAX_ITER
       ...
       !update emax, emin
       if(iter.ne.1) then
          emax=1; emin=(1.0D0/4.0D0)*emin*(3-emin)*(3-emin)
       end if
       ...
       alpha = -0.5d0*l2*sqrt(l2)
       beta  =  1.5d0*sqrt(l2)

#ifdef VAR_MPI
       ab(1)=alpha; ab(2)=beta
       call mpixsend(1,1,'INTEGER',1,2)
       call mpixsend(ab,2,'DOUBLE',1,3)
       call mat_mpixsend(T1,1,4)
       call mat_mpixsend(S_sqrt,1,5)
#else
       call mat_mul(S_sqrt,T1,'n','n',alpha,0.0d0,T2)
       call mat_scal(beta,S_sqrt)
       call mat_daxpy(1.0d0,T2, S_sqrt)
#endif

       call mat_mul(T1,S_minus_sqrt,'n','n',alpha,0.0d0,T2)
       call mat_scal(beta,S_minus_sqrt)
       call mat_daxpy(1.00d0,T2, S_minus_sqrt)

#ifdef VAR_MPI
       call mat_mpixrecv(S_sqrt,1,0)
#endif

       call mat_mul(S_minus_sqrt,S_sqrt,'n','n',1.0d0,0.0d0, T1)

       !check convergence
       testx = abs(sqrt(mat_sqnorm2(T1))-t1_converged)
       converged = testx .le.  1.0d-7 
...
#ifdef VAR_MPI
     !send slave quit signal
      call mpixsend(0,1,'INTEGER',1,2)
#endif
...
!     convert back
      ...
      call mat_set_from_full(dY,1.0D0,S_sqrt)

      call q2dconv(n2,dY,Z)
      call mat_set_from_full(dY,1.0D0,S_minus_sqrt)
...
!       &"Residual norm         :", test
   ...
   subroutine lowdin_diag(n, S,S_sqrt, S_minus_sqrt, lupri)
     ...
     do i=1,n
        ...
        eigen_minus_sqrt(i) = 1.0d0/eigen_sqrt(i)
     enddo
     ...
     allocate(t3(n,n))
     ! S^1/2
     call dgemm('n','t', n,n,n, 1.0d0, t2,n, t1,n, 0.0d0,S_sqrt,n)
     ...
     ! S^-1/2
     call dgemm('n','t', n,n,n, 1.0d0, t2,n, t1,n, 0.0d0,S_minus_sqrt,n)
     ...
     call mat_mul(S_sqrt,S_minus_sqrt,'n','n',1.0d0,0.0d0,T3)
     print *, 'T norm:', abs(sqrt(mat_sqnorm2(T3)) - sqrt(1.0d0*S%ncol))
     call mat_free(T3)
#endif
...
#ifdef VAR_MPI
!public routines
   subroutine lowdin_schulz_slave
     use matrix_operations
     use infpar_module

     type(Matrix) :: S_sqrt, T1, T2
     real(realk)  :: ab(2)

     !only one slave is used
     if (infpar%mynum.ne.1) return
 
     !communicate matrix type and size
     call mpixrecv(matrix_type,1,'INTEGER',0,0)
     call mpixrecv(ncr,1,'INTEGER',0,1)

     !init
     call mat_init(S_sqrt,ncr,ncr)
     call mat_init(T1,    ncr,ncr)
     call mat_init(T2,    ncr,ncr)

     !do job, quit if job=0
     do
       call mpixrecv(job,1,'INTEGER',0,2)
       if (job.eq.0) exit

       call mpixrecv(ab,2,'DOUBLE',0,3)
       call mat_mpixrecv(T1,0,4)
       call mat_mpixrecv(S_sqrt,0,5)

       call mat_mul(S_sqrt,T1,'n','n',ab(1),0.0d0,T2)
       call mat_scal(ab(2),S_sqrt)
       call mat_daxpy(1.0d0,T2, S_sqrt)

       call mat_mpixsend(S_sqrt,0,0)
     enddo
