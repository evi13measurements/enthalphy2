...
!>
   ...
   call DD_Fock_eigenvalue(DD,decomp,'h',0.0d0)
   ...
   if (DD%nvirt > 0) then !Fix for atoms
      call DD_Fock_eigenvalue(DD,decomp,'l',0.0d0)
      if (.not. decomp%cfg_hlgap_converged) then
         write (decomp%lupri,*)
   ...
   if (decomp%cfg_check_converged_solution .or. do_rsp_iniguess) then
      ...
      do i = 1, nguesses
         ...
         do mu = 1, fulldim
            do nu = 1, fulldim
               if (do_rsp_iniguess) then
                  iniguess_full(mu,nu) = DD%FUQ_eivecs(mu,FUQFUPindex(posmin(1),1))* &
                                      &  DD%FUP_eivecs(nu,FUQFUPindex(posmin(1),2))
               else
                  iniguess_full(mu,nu) = DD%FUQ_eivecs(mu,FUQFUPindex(posmin(1),1))* &
                                      &  DD%FUP_eivecs(nu,FUQFUPindex(posmin(1),2))- &
                                      &  DD%FUP_eivecs(mu,FUQFUPindex(posmin(1),1))* &
                                      &  DD%FUQ_eivecs(nu,FUQFUPindex(posmin(1),2))
               endif
            enddo
         enddo
         ...
         call mat_set_from_full(iniguess_full,1.0d0, iniguess(i), 'iniguess')
         if (do_rsp_iniguess) then
            !Normalize X*S*X=1
            ...
            call mat_scal(1.0d0/sqrt(fac),iniguess(i))
   ...
   implicit none
      ...
      M_full = 0.0d0; P_full = 0.0d0
      if (desired_eigenval == 'l') then
         call mat_to_full(decomp%FUQ, 1.0d0, M_full)
         call mat_to_full(decomp%QU, 1.0d0, P_full)
      else if (desired_eigenval == 'h') then
         call mat_to_full(decomp%FUP, 1.0d0, M_full)
         call mat_to_full(decomp%DU, 1.0d0, P_full)
      endif
   ...
   implicit none
      ...
      iniguess_full = 0.0d0
      do i = 1, nroots
         iniguess_full(positions(i),i) = 1.0d0
   ...
   !> \date March 2005.
    ...
    do i = 1, nstart
       call mat_init(b_current(i),ndim,ndim)
       call mat_init(sigma_cur(i),ndim,ndim)
    enddo
    do i = 1, decomp%cfg_hessian_nvec
       call mat_init(res(i),ndim,ndim)
       call mat_init(x(i),ndim,ndim)
       call mat_init(scrvec(i),ndim,ndim)
    enddo
    ...
    !   call DD_lintrans(0.0d0,scrvec,b_current,scrvec)
    ...
    do i = 1, nb_curr !linear transformation of start vectors
       call DD_lintrans(DD,decomp,0.0d0,b_current(i),sigma_cur(i),fifoqueue)
       call mat_write_to_disk(DD%lusigma,sigma_cur(i))
    enddo
    ...
    do 
       ...
       if (iter == max_it) then
             ...
             write(decomp%lupri,*) 'Increase .CHECKMAXIT in input. Currently set to', decomp%cfg_check_maxit
       ...
       do i = 1, decomp%cfg_hessian_nvec !Precond each residual individually
          if (.not. conv(i)) then        !Only do precond if residual not converged
             call DD_precond(decomp, 0.0d0,res(i),scrvec(i))
          endif
       enddo
   ...
   call mat_init(sigma_n,ndim,ndim)
   call mat_init(bn,ndim,ndim)
   call mat_init(scrmat,ndim,ndim)
   ...
   do j = 1, nb_curr
      do i = 1, n_on_disk  !Setup lower half of reduced matrix
         call mat_read_from_disk(DD%lusigma,sigma_n)
   ...
   do j = 1, nb_curr 
      do i = 1, n_on_disk   !Explicitly calculate upper half:
         call mat_read_from_disk(DD%lub,bn)
   ...
   do j = 1, decomp%cfg_hessian_nvec
      do i = 1, ndim_red
         ...
         call mat_daxpy(1.0d0, scrmat, res(j))
   ...
   implicit none
          ...
          real(realk), intent(out) :: xproj(ndim)
...
!local
    integer :: ndim,irx,irow,i,iround,iturn,k,ibvec,jbvec,jrx,iB,no_of_new
    integer,allocatable :: lin_depend(:) !linear dependency index array
    type(matrix) :: B_scr
    real(realk) :: TT,T1,T2, norm

    ndim = Bvec_tmp(1)%nrow
    ALLOCATE(lin_depend(Nb_new))
...
!
    ...
    rewind(lub)
    do k = 1,Nb_prev
      call mat_read_from_disk(lub,b_scr)
      do irx = 1,Nb_new
        if (.not. conv(irx)) then
           !if lin_depend(irx) == 0, the vector is skipped
           !because of linear dependencies
    ...
    do irx = 1,Nb_new
      if (.not. conv(irx)) then
         if (lin_depend(irx) /= 0) then
           iB = iB + 1
           no_of_new = no_of_new + 1
           b_current(no_of_new) = Bvec_tmp(irx)
           call mat_write_to_disk(lub,Bvec_tmp(irx))
           if (decomp%info_dd_normalize) write(decomp%lupri,*) 'DD_orthonormalize: new vector is added'
         endif
      endif
    enddo
...
!
  ...
  subroutine DD_Hes_normalize(decomp,iturn,ibvec,iround,lin_depend_i,Bvec_tmp_i)
   ...
   !>
   subroutine DD_mat_eigenvalues_to_aux(unres,A)
   implicit none
   ...
   integer                  :: NULLI
      ...
      if (.not.associated(A%raux)) allocate(A%raux(2))
       ...
       call mat_to_full(A,1.0D0,T)
       call dsyevx('N','I','U',n,T,n,NULLR,NULLR,n,n,0.0D0,m,TMPE,&
     ...
     & 'See man dsyevx for more information.'
        call lsquit('DSYEVX failed in DD_mat_eigenvalues_to_aux')
       endif

       !compute min eigenvalue
       call mat_to_full(A,1.0D0,T)

       call dsyevx('N','I','U',n,T,n,NULLR,NULLR,1,1,0.0D0,m,TMPE,&
                 & Z,1,wrk,lwrk,iwrk,NULLI,info)
       A%raux(2) = TMPE(1)
...
!      print *, 'emax: ', A%raux(1), ' emin: ', A%raux(2)
  end subroutine DD_mat_eigenvalues_to_aux
   ...
   implicit none
        ...
        integer                  :: ndim
        real(realk), allocatable :: FUQfull(:,:), FUPfull(:,:), eigenval(:), eigenvec(:,:)
        real(realk), allocatable :: choltemp(:)
        integer, allocatable     :: icholtemp(:), IFAIL(:)
        real(realk)              :: VL, VU, HOMO, LUMO
        integer                  :: IL, IU, neig, lcholtemp, INFO, m 

   ndim = decomp%FUP%nrow
   Lcholtemp = 8*ndim 

   allocate(FUQfull(ndim,ndim),FUPfull(ndim,ndim),eigenval(ndim))
   allocate(eigenvec(ndim,ndim),choltemp(Lcholtemp),Icholtemp(5*ndim),IFAIL(ndim))
   if (decomp%cfg_unres) then
     call dd_debug_homolumo_unres(decomp)
     diag_hlgap = 0.0d0
     write(decomp%lupri,*) 'WARNING: HOMO-LUMO gap not calculated for unrestricted!'
   else
     call mat_to_full(decomp%FUP,1.0d0,FUPfull)
     call mat_to_full(decomp%FUQ,1.0d0,FUQfull)

     call DSYEVX('V', 'A', 'U', ndim, FUPfull, ndim, VL, VU, IL, IU, &
       &  0.0d0, neig, eigenval, eigenvec, ndim, choltemp, Lcholtemp, Icholtemp, &
       &  IFAIL, INFO )

     if (info /= 0) STOP 'Problem in DSYEVX (dd_debug_homolumo)'

     !write(decomp%lupri,*) "Dim of FUP and eigenvalues of FUP, # found:", ndim, neig
     write(decomp%lupri,*) "Occupied orbital energies:"
     do m = 1, neig
        write(decomp%lupri,'(i6,F15.7)') m, eigenval(m)
     enddo
     do m = neig, 1, -1
        if (abs(eigenval(m)) > 1.0d-7) then
           HOMO = eigenval(m)
           exit
        endif
     enddo
     !write(lupri,*) 'Highest occupied eigenvalue:', HOMO
     ...
     write(decomp%lupri,*) "Virtual orbital energies:"
     do m = 1, neig
        write(decomp%lupri,'(i6,F15.7)') m, eigenval(m)
     enddo
     do m = 1, neig
   ...
   call mat_to_full(decomp%FUP,1.0d0,fullmat)
   ...
   call mat_to_full(decomp%FUQ,1.0d0,fullmat)
