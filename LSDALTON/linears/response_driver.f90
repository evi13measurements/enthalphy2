...
!             MyRspFunc%freq(k) = Rfreq(k)*(1.d0,0.d0) + Ifreq(k)*(0.d0,1.d0)
!          enddo
!
!
!
...
!                   ! Freq1 = -Freq2 - Freq3
!                   MyRspFunc%freq(2) = (1.d0,0.d0)*Rfreq(1) + (0.d0,1.d0)*Ifreq(1)
...
!                   MyRspFunc%freq(3) = (1.d0,0.d0)*Rfreq(3) + (0.d0,1.d0)*Ifreq(3)
!                   MyRspFunc%freq(2) = (1.d0,0.d0)*Rfreq(2) + (0.d0,1.d0)*Ifreq(2)
!                   MyRspFunc%freq(1) = - sum(MyRspFunc%freq(2:MyRspFunc%order))
!
!                   ! Finally, all information about the quadratic single 
!                   ! transition moment for excitation number (j) is now stored in
...
!  End subroutine response_input
  ...
  subroutine rspfunc_or_transmoment(molcfg,F,D,S,RspFunc_stack,n_rspfunc)
    ...
    complex(8), allocatable, target     :: rsp_results(:,:)
...
!  end subroutine determine_number_of_rspfuncs_and_exenergies
  ...
  subroutine transition_moment_density_matrix(molcfg,F,D,S,nexci_max)
    ...
    Do i=1,nexci_max
       ...
       rsp_eq_sol(j)%fld(1)%freq=ExEnergies(i) * (1.d0,0.d0)
  ...
  subroutine Get_first_order_property(molcfg,F,D,S,property,dim_property,property_type)
    ...
    complex(8), intent(inout) :: property(dim_property)
  ...
  subroutine linear_response(molcfg,F,D,S,MyRspFunc, rsp_results_vec,response_size)
    ...
    complex(8), intent(inout) :: rsp_results_vec(response_size)
    !> Local parameters
    type(matrix)                      :: Db(MyRspFunc%dims(2)), &
         & Fb(MyRspFunc%dims(2)), Wb(MyRspFunc%dims(2)), W(1)
    complex(8)              :: lin_RspFunc(MyRspFunc%dims(1),MyRspFunc%dims(2))
    logical                        :: pdbs(2)
    integer                        :: i,j, k
    character*4                    :: NOOP(0)
    integer, pointer               :: lupri
    ...
    subroutine quadratic_response_2nplus1(molcfg,F,D,S,MyRspFunc, rsp_results_vec,response_size)
      ...
      type(matrix)                      :: Da(MyRspFunc%dims(1)), &
           ...
           & zeta(MyRspFunc%dims(1)), W(1)
      complex(8)                     :: quad_RspFunc(MyRspFunc%dims(1),MyRspFunc%dims(2), MyRspFunc%dims(3))
      logical                        :: pdbs(3)
      integer                        :: i,j, k, l, b,c
      character*4                    :: NOOP(0)
      ...
      write(lupri, *) 'Starting quadratic response function calculation for: ', &
        ...
        if(MyRspFunc%trans_moment .and. MyRspFunc%trans_moment_order ==2) then ! double residue
           ...
           Db(1) = -dag(rsp_eq_sol(MyRspFunc%ExNumber2)%D)
           Fb(1)=0.d0*F
           call prop_twoint(molcfg, NOOP, (/D,Db(1)/), (/1/), Fb(1))
        ...
        if(.not. MyRspFunc%trans_moment) then
           call prop_twoave(molcfg, (/ MyRspFunc%code(3) /), (/D, Da, Db, &
                & ( 0.d0*D, i=1,MyRspFunc%dims(1)*MyRspFunc%dims(2) ) /), &
        ...
        if(MyRspFunc%trans_moment_order /= 2) then
           call prop_twoave(molcfg, (/ MyRspFunc%code(2) /), (/D, Da, Dc, &
                & ( 0.d0*D, i=1,MyRspFunc%dims(1)*MyRspFunc%dims(3) ) /), &
        ...
        call prop_twoave(molcfg, (/ MyRspFunc%code(1) /), (/D, Db, Dc, &
             & ( 0.d0*D, i=1,MyRspFunc%dims(2)*MyRspFunc%dims(3) ) /), &
        ...
        call prop_twoave(molcfg, (/NOOP/), (/D, Da, Db, Dc, &
             ...
             & ( 0.d0*D, i=1,MyRspFunc%dims(1)*MyRspFunc%dims(3) ), &
             & ( 0.d0*D, i=1,MyRspFunc%dims(2)*MyRspFunc%dims(3) ), &
             & ( 0.d0*D, i=1,MyRspFunc%dims(1)*MyRspFunc%dims(2)*MyRspFunc%dims(3) ) /), &
             & shape(quad_RspFunc), quad_RspFunc )
        ...
        do j=1,MyRspFunc%dims(2)
           ...
           do k=1,MyRspFunc%dims(3)
              ...
              do i=1,MyRspFunc%dims(1)
                 ...
                 quad_RspFunc(i,j,k) = quad_RspFunc(i,j,k) -tr(lambda(i),Ybc(1))
                 quad_RspFunc(i,j,k) = quad_RspFunc(i,j,k) -tr(zeta(i),Zbc(1))
                 ! Only Sa Wbc contribution if perturbation A uses PDBS.
                 if(pdbs(1)) then
                    quad_RspFunc(i,j,k) = quad_RspFunc(i,j,k) -tr(Sa(i),Wbc(1))
                 endif
      ...
      subroutine quadratic_response_nplus1(molcfg,F,D,S,MyRspFunc, rsp_results_vec,response_size)
        ...
        subroutine cubic_response(molcfg,F,D,S,MyRspFunc, rsp_results_vec,response_size)
          ...
          type(matrix)  :: Da(MyRspFunc%dims(1)), &
               ...
               & zetaAD(MyRspFunc%dims(1),MyRspFunc%dims(4)), W(1)
          complex(8)                     :: cubic_RspFunc(MyRspFunc%dims(1),MyRspFunc%dims(2), &
               & MyRspFunc%dims(3), MyRspFunc%dims(4))
          logical                        :: pdbs(4), AeqB, AeqC, AeqD, BeqC, BeqD, CeqD
          integer                        :: i,j, k, l, m
          character*4                    :: NOOP(0)
          complex(8) :: freqA,freqB,freqC,freqD
          ...
          complex(8), allocatable :: hack_bcad(:,:,:), hack_bdac(:,:,:), &
               & hack_cbad(:,:,:), hack_cdab(:,:,:), &
               & hack_dbac(:,:,:), hack_dcab(:,:,:)
          ...
          write(lupri, *) 'Starting cubic response function calculation for: ', &
            ...
            if(MyRspFunc%code(3) == 'EXCI') then 
               ...
               Dc(1) = -dag(rsp_eq_sol(MyRspFunc%ExNumber2)%D)
            ...
            call get_W_matrix(F,D,S,MyRspFunc,2,pdbs,Wcd,ops=(/3,4/),Dx=Dc,Fx=Fc,Sx=Sc,&
                 Dy=Dd,Fy=Fd,Sy=Sd, &
                 Dxy= (/ ( 0.d0*D, i=1,nC*nD ) /), &
                 Fxy=(/ ( 0.d0*F, i=1,nC*nD ) /) )
            ...
            if(MyRspFunc%trans_moment_order /= 2) then
               ...
               call get_W_matrix(F,D,S,MyRspFunc,2,pdbs,Wbd,ops=(/2,4/),Dx=Db,Fx=Fb,Sx=Sb,&
                    Dy=Dd,Fy=Fd,Sy=Sd, &
                    Dxy= (/ ( 0.d0*D, i=1,nB*nD ) /), &
                    Fxy=(/ ( 0.d0*F, i=1,nB*nD ) /) )
            ...
            if(MyRspFunc%code(4) /= 'EXCI') then
               ...
               call get_W_matrix(F,D,S,MyRspFunc,2,pdbs,Wbc,ops=(/2,3/),Dx=Db,Fx=Fb,Sx=Sb,&
                    Dy=Dc,Fy=Fc,Sy=Sc, &
                    Dxy= (/ ( 0.d0*D, i=1,nB*nC ) /), &
                    Fxy=(/ ( 0.d0*F, i=1,nB*nC ) /) )
...
!!$             & perm = (/1,2,3,4/) )
            ...
            MultiplierLoopA: do i=1,nA
               ...
               MultiplierLoopB: do j=1,nB
                  ...
                  zetaAB(i,j) = Fab(i,j)*D*S + S*D*Fab(i,j) + Fa(i)*Db(j)*S + S*Db(j)*Fa(i) &
                       ...
                       + 0.5d0*freqB*(Sb(j)*D*Sa(i) - Sa(i)*D*Sb(j)) &
               ...
               MultiplierLoopC: do k=1,nC
                  ...
                  if(BeqC) then
                     ...
                     zetaAC(i,k) = Fac(i,k)*D*S + S*D*Fac(i,k) + Fa(i)*Dc(k)*S + S*Dc(k)*Fa(i) &
                          ...
                          + 0.5d0*freqC*(Sc(k)*D*Sa(i) - Sa(i)*D*Sc(k)) &
               ...
               MultiplierLoopD: do l=1,nD
                  ...
                  elseif(CeqD) then
                     ...
                     zetaAD(i,l) = Fad(i,l)*D*S + S*D*Fad(i,l) + Fa(i)*Dd(l)*S + S*Dd(l)*Fa(i) &
                          ...
                          + 0.5d0*freqD*(Sd(l)*D*Sa(i) - Sa(i)*D*Sd(l)) &
                  ...
                  l_loop: do l=1,nD
                     ...
                     IfYbcdZbcd: if(any(pdbs(2:4))) then
                        Ybcd = Fb(j)*(Dc(k)*Sd(l)) - (Sd(l)*Dc(k))*Fb(j) + &
                             ...
                             - (freqC + 0.5d0*(freqB+freqD)) * &
                             ( (Sb(j)*Dc(k))*Sd(l) + (Sd(l)*Dc(k))*Sb(j) ) &
                             - (freqB + 0.5d0*(freqC+freqD)) * &
                             ( (Sc(k)*Db(j))*Sd(l) + (Sd(l)*Db(j))*Sc(k) ) &
                             - (freqD + 0.5d0*(freqB+freqC)) * &
                     ...
                     i_loop: do i=1,nA
                        ...
                        cubic_RspFunc(i,j,k,l) = cubic_RspFunc(i,j,k,l) &
                             -tr(lambdaAB(i,j),Ycd(k,l)) &
                             -tr(lambdaAC(i,k),Ybd(j,l)) &
                             -tr(lambdaAD(i,l),Ybc(j,k))

                        ! First order zeta contributions
                        cubic_RspFunc(i,j,k,l) = cubic_RspFunc(i,j,k,l) &
                             -tr(zetaAB(i,j),Zcd(k,l)) &
                             -tr(zetaAC(i,k),Zbd(j,l)) &
                             -tr(zetaAD(i,l),Zbc(j,k))
            ...
            case(1) ! Wx
               ...
               do i=1,MyRspFunc%dims(ops(1))
                  if(all(pdbs_tmp)) then
                     W(i) = Dx(i)*(F+(0.5d0*freqX)*S)*D + &
                          & D*(F-(0.5d0*freqX)*S)*Dx(i) + D*Fx(i)*D
                  else
                     W(i)=0.d0*D
                  endif
               enddo
                  ...
                  do i=1,MyRspFunc%dims(ops(1))
                     ...
                     if(all(pdbs_tmp)) then
                        ...
                        W(k) = Dxy(k)*F*D + D*F*Dxy(k) + &
                             & D*Fxy(k)*D +&
                             & 0.5d0*(freqX+freqY)*(Dxy(k)*S*D - D*S*Dxy(k)) +&
                             & Dx(i)*F*Dy(j) + Dy(j)*F*Dx(i) +&
                             & Dx(i)*Fy(j)*D + D*Fy(j)*Dx(i) +&
                             & Dy(j)*Fx(i)*D + D*Fx(i)*Dy(j) +&
                             & 0.5d0*freqX*(Dx(i)*Sy(j)*D - D*Sy(j)*Dx(i)) +&
                             & 0.5d0*freqY*(Dy(j)*Sx(i)*D - D*Sx(i)*Dy(j)) +&
                             & 0.5d0*(freqX-freqY)*(Dx(i)*S*Dy(j) - Dy(j)*S*Dx(i))
                     else
                        W(k)=0.d0*D
                     endif
                  enddo
          ...
          !> \author Kasper Kristensen
          !> \date 2010-01 
!          subroutine print_response_function(RspFunc_stack,n_RspFunc,print_results,lupri)
...
!                        write(lupri,'(X,I7,F21.7)') jx,real(RspFunc_stack(i)%result(j))
!                     enddo
!
...
!                           counter=counter+1
!                           write(lupri,'(X,2I7,F21.7)') jx,kx,real(RspFunc_stack(i)%result(counter))
!                        enddo
!                     enddo
!
...
!                              write(lupri,'(X,3I7,F21.7)') jx,kx,lx,real(RspFunc_stack(i)%result(counter))
!                           enddo
!                        enddo
!                     enddo
!
