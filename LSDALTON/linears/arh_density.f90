...
contains
   ...
   implicit none
      ...
      !Trust-region update configuration:
      !----------------------------------
      arh%cfg_arh_expand        = 1.2d0
      arh%cfg_arh_contract      = 0.7d0
      arh%cfg_arh_contract_crit = 0.25d0
      arh%cfg_arh_expand_crit   = 0.75d0
      !Levelshift parameters:
      !----------------------
      arh%cfg_arh_newdamp       = .false.
      arh%cfg_nodamp            = .false.
      arh%cfg_min_lshift        = 0.0d0
      arh%cfg_fixed_shift       = .false.
      arh%cfg_fixed_shift_param = 0.0d0
      arh%cfg_max_step          = 0.6d0
      arh%cfg_max_element       = 0.35d0
      !Thresholds:
      !-----------
      arh%cfg_micro_thresh  = 1.0d-2
   !INFO VARIABLES:
   !=========================
      arh%info_crop           = .false.
   ...
   !=========
      ...
      arh%set_max_step    = 0.6d0
      arh%set_max_element = 0.35d0
      arh%set_local       = .false.
   ...
   if (arh%info_lineq) then
      ...
      write (arh%lupri, '("  Error of red space iteration", i3, ":  ", F16.10, " (mu = ", F6.2, ", xmax = ", &
            & E12.6, ", xnorm = ", E12.6, ", THR = ", E12.6, ")")') it, err, mu, maxelm, xnorm, thresh
   endif

   if (err < thresh .or. err < 1.0d-9) then
      call mat_max_elm(x, maxelm)
      if (j == 0 .AND. ABS(mu) > 1.0d-2 .and. err > 1.0d-9 .and. .not. arh%set_local &
         ...
         call mat_write_to_disk(xsave_lu,x)
      ...
      else if (maxelm > 1.1d0*arh%set_max_element) then
         ...
         if (arh%info_lineq) then
            if (j==0 .and. .not. arh%set_local .and. maxelm/arh%set_max_element > 0.85d0 .and. &
            & abs(mu) > 1.0d-2) then
               write (arh%lupri,*) 'No search in xmax attempted - size of X already at TR limit...'
            endif
            write (arh%lupri,'("Newton equations converged in", i3, " iterations!")') it
   ...
   if (it == k + 15) then
      k = it
      if (arh%info_lineq) then
         write (arh%lupri, '("Error went down by", F8.2, "%")') 100.0d0 - (err/errsave)*100d0
      endif
      if (err > 0.85d0*errsave) then
          if (arh%set_optxelm) then
             write (arh%lupri,*) 'Residual is not going down, decrease trust radius (xmax)'
             call mat_max_elm(x, maxelm)
             arh%set_max_element = maxelm*0.9d0
             write (arh%lupri,*) 'New trust radius =', arh%set_max_element
             arh%trustradius_decreased = .true.
          else
             write (arh%lupri,*) 'Residual is not going down, decrease trust radius (xnorm)'
             arh%trustradius_decreased = .true.
             xnorm = sqrt(mat_sqnorm2(x))
             arh%set_max_step = xnorm*0.9d0
             write (arh%lupri,*) 'New trust radius =', arh%set_max_step
             call mat_max_elm(x, maxelm)
             arh%set_max_element = maxelm*0.9d0
          endif
          do_LS = .true.
   ...
   implicit none
        ...
        real(realk),allocatable                 :: scr1(:), scr2(:), vec(:), testmetric(:,:)
   ...
   if (.not. arh%set_arhterms) then
      ...
      !END DEBUG 
      allocate(scr1(ndens*ndens),scr2(ndens*ndens),vec(ndens))
      ...
      do i = 1, ndens
         ...
         call MAT_ADD(1.0d0, Di, -1.0d0, D0, idiff) !Di = Di - D0
         do j = 1, ndens
            call get_from_modFIFO(fifoqueue, j, dummy, Dj)
            call MAT_ADD(1.0d0, Dj, -1.0d0, D0, jdiff) !Dj = Dj - D0
      ...
      if (.false.) then
         ...
         allocate(testmetric(ndens,ndens))
         do i = 1, ndens
            do j = 1, ndens
               testmetric(i,j) = (1.0d0/sqrt(arh%fifometric(i,i))) * arh%fifometric(i,j) * (1.0d0/sqrt(arh%fifometric(j,j)))
            enddo
         enddo
         k = ndens
         ...
         deallocate(testmetric)
      endif

      deallocate(scr1,scr2,vec)
      call mat_free(idiff)
      call mat_free(jdiff)
   endif
   ...
   implicit none
        ...
        real(realk), intent(out)            :: weights(:)
        type(Matrix)                        :: wrk1, wrk2, com
        type(Matrix),pointer                :: D0, Dj, dummy 
        integer                             :: j, ndens, ndim
        real(realk)                         :: last_weight
        real(realk), allocatable            :: vec(:)
        real(realk), allocatable            :: T(:,:), Thalf(:,:), Tminushalf(:,:), scr(:)
   ...
   if (.not. arh%set_arhterms) then
      ...
      if (arh%info_arh_weights) then
         ...
         call mat_mul(fifoqueue%D_exp,x,'n','n',1.d0,0.d0,wrk1)
         call mat_trans(wrk1,wrk2)
         call MAT_ADD(1.0d0, wrk1, 1.0d0, wrk2, com) !com = DX - XD 

         call get_from_modFIFO(fifoqueue, 0, dummy, D0)
         do j = 1, ndens
            call get_from_modFIFO(fifoqueue, j, dummy, Dj)
            call MAT_ADD(1.0d0, Dj, -1.0d0, D0, wrk2)
         ...
         if (ndens > 0) then !Multiply weights by T^-1/2
            allocate(scr(2*ndens**2 + ndens*(ndens+1)/2))
            allocate(T(ndens,ndens))
            allocate(Thalf(ndens,ndens))
            allocate(Tminushalf(ndens,ndens))
            ...
            deallocate(T)
            deallocate(Thalf)
            deallocate(Tminushalf)
         endif
      endif

      call get_from_modFIFO(fifoqueue, 0, dummy, D0)
      do j = 1, ndens
         call MAT_ADD(1.0d0, Dnew, -1.0d0, D0, wrk1)
         call get_from_modFIFO(fifoqueue, j, dummy, Dj)
         call MAT_ADD(1.0d0, Dj, -1.0d0, D0, wrk2)
   ...
   implicit none
     ...
     do i = 1, ndens
        ...
        call MAT_ADD(1.0d0, Di, -1.0d0, D0, DiminusD0) !Di = Di - D0
        call MAT_ADD(1.0d0, Fi, -1.0d0, F0, FiminusF0) !Fi = Fi - F0
        do j = 1, ndens
           call get_from_modFIFO(fifoqueue, j, Fj, Dj)
           call MAT_ADD(1.0d0, Fj, -1.0d0, F0, FjminusF0) !Dj = Dj - D0
           call MAT_ADD(1.0d0, Dj, -1.0d0, D0, DjminusD0) !Fj = Fj - D0
   ...
   implicit none
        ...
        real(realk), intent(out) :: Vout(:)
        real(realk), allocatable :: A(:,:)
        integer, allocatable     :: IPIV(:)
        integer                  :: INFO

   if (.true.) then
      allocate(A(ndens,ndens))
      allocate(IPIV(ndens))
      A = arh%fifometric
   ...
   do
      ...
      if (N == 0) then
         ...
         call MAT_ADD(1.0d0, b, -1.0d0, scr, res)    !res = b - A*x
         ...
         call mat_daxpy(1.0d0, Hr_new, p) !p = Hr_new + beta*p
         Hr = Hr_new
      endif
      N = N + 1 
   enddo
   ...
   implicit none  
      ...
      call MAT_ADD(1.0d0,decomp%FUQ,-1.0d0,decomp%FUP,scr1) !scr1= FUQ - FUP

      call MAT_MUL(scr1,x_in,'n','n',1.0d0,0.0d0,AX) !AX = (FUQ-FUP)*X
      call mat_trans(AX,scr1)
      if (symm == antisymmetric) then
         call mat_scal(-1.0d0,scr1)
      endif                                          !scr1 = X*(FUQ-FUP)

      call mat_daxpy(1.0d0,scr1,AX)    !AX = (FUQ-FUP)*X + X*(FUQ-FUP)
      ...
      if (arh%set_do_2nd_order) then !Add G(D) type contribution
         ...
         call MAT_MUL(x_in,decomp%DU,'n','n',1.0d0,0.0d0,XF) !XF is XD here!
         call mat_trans(XF,FX)
         if (symm == 2) then
            call mat_scal(-1.0d0,FX)
         endif
         call MAT_ADD(1.0d0,XF,-1.0d0,FX,scr1)
         if (decomp%cfg_do_in_oao) then
           XF=scr1
         else
           call x_from_oao_basis(decomp, scr1, XF) !XF is XD_AO
         endif
         call di_GET_GbDs(XF,scr1,decomp%nocca,decomp%noccb) !scr1 is G_AO without DFT
         if (arh%do_dft) THEN
            call lsquit('Second order DFT not implemented',arh%lupri)
            !Add extra G contributions
            !call mat_init(D_AO, X_in%nrow, X_in%nrow)
            !call x_from_oao_basis(decomp,decomp%DU, D_AO)
            !call di_get_sigma_xc_cont(XF,D_AO,decomp%S,scr2)
            !call mat_daxpy(1.d0,scr2,scr1) !scr1 is now G_AO + DFT
            !call mat_free(D_AO)
         endif
         if (decomp%cfg_do_in_oao) then
           scr2 = scr1
         ...
         call MAT_MUL(scr2,decomp%DU,'n','n',1.0d0,0.0d0,XF) !XF is GU*DU here!
         call mat_trans(XF,FX) !FX is DU*GU here (both D and G are always symm)
         call MAT_ADD(1.0d0,XF,-1.0d0,FX,scr1)
         call mat_daxpy(1.0d0,scr1,AX)
   ...
   implicit none
        ...
        real(realk), allocatable, dimension(:) :: vec2, V2
    ...
    allocate(vec2(ndens), V2(ndens))

    call mat_init(wrk1,ndim,ndim)
    call mat_init(wrk2,ndim,ndim)

    !Get projected input matrix = DX - XD:
    call MAT_MUL(decomp%DU,x,'n','n',1.0d0,0.0d0,wrk1)
    call mat_trans(wrk1,wrk2)
    if (symm == antisymmetric) then
       call mat_scal(-1.0d0,wrk2)
    endif
    call MAT_ADD(1.0d0, wrk1, -1.0d0, wrk2, Ax) !Ax = x projected, DX - XD
    ...
    do i = 1, ndens
       call get_from_modFIFO(fifoqueue, i, Fi, Di)
       call MAT_ADD(1.0d0, Di, -1.0d0, D0, wrk1) !wrk1 = Di - D0
       !call MAT_ADD(1.0d0, Fi, -1.0d0, F0, wrk2) !wrk2 = Fi - F0
    ...
    do i = 1, ndens
       call get_from_modFIFO(fifoqueue, i, Fi, Di) 
       call MAT_ADD(1.0d0, Fi, -1.0d0, F0, wrk1) !wrk1 = Fi - F0
       call mat_daxpy(vec2(i),wrk1,Ax)           !Ax = SUM_i (Fi -F0)*[T^-1*V2]_i
    enddo

    !Project:
    call MAT_MUL(decomp%DU,Ax,'n','n',1.0d0,0.0d0,wrk1)   !wrk1 = DU*Ax
    call MAT_MUL(wrk1,decomp%QU,'n','n',1.0d0,0.0d0,wrk2) !wrk2 = DU*Ax*QU
    call mat_trans(wrk2,wrk1)
    call MAT_ADD(1.0d0, wrk2, -1.0d0, wrk1, Ax)           !Ax = DU*Ax*QU - QU*Ax* DU 
...
!###### Debug section ###############################
   ...
   implicit none
      ...
      do m = 1, vecdim
         ...
         call mat_create_elm(m, 1, 1.0d0, x_trial)
      ...
      !write(lupri,*) 'antisymmetric:', antisymmetric
         call arh_lintrans(arh,decomp,x_trial_mat,antisymmetric,0.0d0,scr,fifoqueue)
...
!####################################################################################
   ...
   if (arh%cfg_arh_crop_safe) then !Otherwise use incoming residual
      ...
      call mat_add(-1.d0,sigma_n,-1.d0, G, res) !scr = r_n par
   ...
   call mat_daxpy(1.0d0,res_new,Xn) !Xn is now x_n+1

   !3. Linear transformation of new solution vector:
   call arh_lintrans(arh,decomp,Xn,symm,0.0d0,sigma_n,fifoqueue) !sigma_n is now sigma_n+1

   !4. Calculate residual r_n+1 = G - (H(Xn) + mu*Xn) 
   call mat_add(-1.d0,G,-1.d0, sigma_n, res)
   call mat_daxpy(mu,Xn,res)
   ...
   implicit none
         ...
         real(realk), allocatable    :: ResRed(:,:), RHS(:)
         TYPE(matrix)                :: scr, scr2, r_newP, xvec
         type(matrix), pointer       :: xpointer, sigmapointer
         integer, allocatable        :: IPIV(:)
         integer                     :: i, ndim, IERR, redsize, qsize, printsize, Asize, newsize
         real(realk), allocatable    :: tempA(:,:), tempS(:,:)
         logical                     :: space_expanded

   ndim = G%nrow
   if (.not. arh%cfg_arh_truncate) then
      call mat_init(xvec, ndim,ndim)
      call mat_init(scr2, ndim,ndim)
   endif
   call mat_init(scr, ndim,ndim)
   call mat_init(r_newP, ndim,ndim)
   if (arh%cfg_arh_truncate) then
   ...
   allocate(ResRed(redsize+2,redsize+2))
   allocate(RHS(redsize+2))
   allocate(IPIV(redsize+2))
   ...
   ResRed(redsize+2,redsize+2) = 0.0d0
   if (.not. arh%cfg_arh_truncate) then
   ...
   do i = 1, redsize  !Setup n+1'th dimension
      if (arh%cfg_arh_truncate) then
         ...
         call mat_add(-1.d0,G,-1.d0, sigmapointer, scr) !scr = i'th residual
         call mat_daxpy(mu,xpointer,scr)
      else
         call mat_read_from_disk(lub,xvec)
         call mat_read_from_disk(lusigma,scr2)
         !call get_sigma(lusigma,sigma_n)
         call mat_add(-1.d0,G,-1.d0, scr2, scr) !scr = i'th residual 
   ...
   do i = 1, redsize
      if (arh%cfg_arh_truncate) then
         ...
         call mat_read_from_disk(lub,scr)
         call mat_read_from_disk(lusigma,scr2)
   ...
   implicit none
         ...
         integer                     :: i, ndim, redsize, qsize, newsize, Asize, printsize, min_eival_index, nsize
         type(matrix), pointer       :: xpointer, sigmapointer
         real(realk), allocatable    :: tempvec(:), tempmat(:,:), tempA(:,:), tempS(:,:)
         real(realk)                 :: min_eival
         logical                     :: new_scheme, space_expanded !Temporary!!
   ...
   call mat_add(-1.d0,G,-1.d0, sigma, scr) !scr = n+1'th optimal residual, r_n+1 par 
   ...
   do i = 1, redsize  
      if (arh%cfg_arh_truncate) then
         ...
         call mat_add(-1.d0,G,-1.d0, sigmapointer, scr) !scr = i'th optimal residual, r_i par
         call mat_daxpy(mu,xpointer,scr) 
         ...
         call mat_add(-1.d0,G,-1.d0, scr2, scr) !scr = i'th optimal residual, r_i par
         call mat_daxpy(mu,xvec,scr) 
   ...
   if (arh%debug_diag_redspace .or. arh%debug_dd) then
      ...
      allocate(tempA(nsize,nsize),tempS(nsize,nsize))
      allocate(tempmat(nsize,nsize),tempvec(nsize))
   ...
   !Diagonalize Sred to test for linear dependencies:
   if (.false.) then
      allocate(tempmat(redsize,redsize))
      tempmat(1:redsize,1:redsize) = arh%Sred(1:redsize,1:redsize)
   ...
   implicit none
         ...
         type(matrix), pointer       :: xpointer, sigmapointer
         real(realk), allocatable    :: eival(:), eivec(:,:)
         real(realk)                 :: norm

   allocate(eival(reddim), eivec(reddim,reddim))
   ...
   call mat_scal(1.0d0/norm,xF)
   call mat_scal(1.0d0/norm,sigmaF)

   deallocate(eival,eivec)
   ...
   implicit none
         ...
         type(matrix), pointer       :: xpointer, sigmapointer
         real(realk), allocatable    :: tempmat(:,:)
   ...
   if (.not. unres) then !For closed shell, the matrices have been scaled by
      ...
      arh%denom = 2.0d0*arh%denom
   endif

   if (arh%denom > 0.0d0) then
   ...
   epred = - mat_dotproduct(x,b) - 0.5d0*mat_dotproduct(x,Ax) -0.5d0*mu*mat_dotproduct(x,x)
   !epred = 0.5d0*mat_dotproduct(x,Ax) - mat_dotproduct(x,b) 
   end function epred
