...
type(Matrix)                        :: Xsolvec(rsp_bvec_dim),sigmas(rsp_bvec_dim),rhos(rsp_bvec_dim)
  ...
  &     ' AND TRANSITION MOMENT CALCULATION (LSTDHF) >>>'
   ...
   if (lineq) then
     do j=1,ngd
         ...
         call mat_add(0.5d0,gdb(j),0.5d0,gT,RHS_real(j))
         call mat_add(0.5d0,gdb(j),-0.5d0,gT,RHS_img(j))
         call mat_free(gT)
      enddo
    endif
       ...
       do j=1,nm_new
        ...
        enddo
         if (lineq) then
            ...
            do j=1,nx_new
               call mat_add(0.5d0,xp(j),0.5d0,xm(j),xsolvec(j))
            enddo
               ...
               if (nx_new>0) then
                 do j=1,nx_new
                   ...
                   call mat_add(0.5d0,xsolvecp(j),0.5d0,gT,xp(j))
                   call mat_add(0.5d0,xsolvecp(j),-0.5d0,gT,xm(j))
                   call mat_free(gT)
                 enddo
                 call orthonormalize21(molcfg,D,S,xp,xm,nx_new,nm_new,nt_red,xsolvecp,xsolvecm,lu_x_rsp,lineq)
              !   nm_new=nx_new
        ...
        !  else
         ...
         !  endif
           ...
           do j=1,min(nx_new,nm_new)
              ...
              call mat_add(0.5d0,sigmas(j),0.5d0,gT,sigmasp(j))
              call mat_add(0.5d0,sigmas(j),-0.5d0,gT,sigmasm(j))
    
              call mat_trans(rhos(j),gT)  
              call mat_add(0.5d0,rhos(j),0.5d0,gT,rhosp(j))
              call mat_add(0.5d0,rhos(j),-0.5d0,gT,rhosm(j))
              call mat_free(gT)
           enddo
          ! if (lineq) then
          if ((nx_new>0) .and. (nm_new)>0) then    
...
!Setup lower half of E2 and S2:
 ...
 do j = 1, nt_red
    ...
    if ((nx_new>0) .and. (sqrt(mat_sqnorm2(rhop_j))>1d-9)) then
        k = 0
        do i = n_red+1, n_red+nx_new
           k = k + 1
 ...
 do i = 1, nt_red
    ...
    if ((nx_new>0) .and. (sqrt(mat_sqnorm2(xp_j))>1d-9))then
       k = 0
    ...
    if ((nm_new>0) .and. (sqrt(mat_sqnorm2(xp_j))>1d-9)) then
       k = 0
       do j = nm_red+1, nm_red+nm_new
          k = k + 1
 ...
 if (lineq) then
    do j = 1, ngd  !Loop over number of gradients
       ...
       do i = n_red+1, n_red+nx_new !Loop in steps of 2 because of pairing
          k = k+1
            if (sqrt(mat_sqnorm2(gd(j)))>1d-9) then    
       ...
       do i = nm_red+1, nm_red+nm_new !Loop in steps of 2 because of pairing
          k = k+1
            if (sqrt(mat_sqnorm2(gdi(j)))>1d-9) then    
...
call mat_zero(scr)
    do ibx = 1,ngd
      ...
      if (molcfg%solver%rsp_olsen) then
       S2Xp(ibx)=residualsm(ibx)
       S2Xm(ibx)=residualsp(ibx)
       endif
       ...
       ! subtract gradient
      if (lineq) then
         call mat_daxpy(-1.0d0,gd(ibx),residualsp(ibx))
         call mat_daxpy(-1d0,gdi(ibx),residualsm(ibx))
      endif
      call util_scriptPx('T',D,S,residualsp(ibx))
      call util_scriptPx('T',D,S,residualsm(ibx))
    enddo

 call mat_add(1d0,residualsp(1),1d0,residualsm(1),scr)
...
! STEP 1: check initial linear dependencies among new vectors
   ...
   !
       ...
       rewind(lu_basis)
       do k = 1,Nb_prev
                  call mat_read_from_disk(lu_basis,b_k)
          do irx = 1,Nb_new
...
!
    ...
    do i=1,nb_new
       ...
       call mat_add(0.5d0,Bvec_tmp(i),0.5d0,bT,bvecs(i))
       call mat_add(0.5d0,Bvec_tmp(i),-0.5d0,bT,bvecsm(i))
    enddo
...
type(prop_molcfg), intent(inout)    :: molcfg
    ...
    do k = 1,nt_red !length of red. solution for each grad
     ...
     call mat_add(0.5d0,basisvec_k,0.5d0,bT,bp) 
     call mat_add(0.5d0,basisvec_k,-0.5d0,bT,bm) 
    
     if (sqrt(mat_sqnorm2(bp))>1d-9) then
      i=i+1
      fac1 = red_eivec_i(i)
      call mat_daxpy(fac1,bp,expanded_vec)
      endif
     if (sqrt(mat_sqnorm2(bm))>1d-9) then
      j=j+1
      fac2 = red_eivec_m(j)
  ...
  !  lin_depend = 1 !Initialize to non-dependent
   ...
   !
       ...
       do k = 1,Nb_prev
                  ...
                  call mat_add(0.5d0,b_k,0.5d0,bT,bp)
                  call mat_add(0.5d0,b_k,-0.5d0,bT,bm)
         if (sqrt(mat_sqnorm2(bp)) >1d-9) then
          do irx = 1,Nb_new
             !if lin_depend(irx) == 0, the vector is skipped
             !because of linear dependencies
            ...
            if (symm) then
             do j = 1,Bvec_tmp_i%ncol
                do k = 1,Bvec_tmp_i%nrow
                   if (k == j) then
                       Bvec_tmp_i%elms((j-1)*Bvec_tmp_i%nrow+k) = 0d0
                   else 
                       if (k>j) then
                        Bvec_tmp_i%elms((j-1)*Bvec_tmp_i%nrow+k) = 1.0d0
...
!              & ' is < cfg_rsp_thr_lin_depend '!, molcfg%rsp_thr_lin_depend
 ...
 !        run_ortho_again = .true.
 !    else
 !   call mat_scal(1d0/sqrt(TT),Bvec_tmp_i)
 !        run_ortho_again = .true.
        !if (molcfg%solver%info_rsp) then
  !        write(molcfg%lupri,*) 'bvector number ', i, ' is removed because its norm**2 :', TT, &
