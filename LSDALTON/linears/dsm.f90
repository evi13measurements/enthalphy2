...
module LINSCA_DSM
   ...
   real(realk), save :: dsm_hes_zero = 0.0d0
   !> Size limit on accepted step
   real(realk), save :: changelimit = 1.2d0
...
!> \param restart True is DSM should be restarted, e.g. after removal of one or more vectors
   subroutine DSM_WEIGHTS(av,queue,msize,S,H1,weights,restart)
      ...
      weights = 0.0d0
      coef = 0.0d0
      trustr = 0.2d0   !normally it works well with 0.2
      steprej = 0
      posdef = .false.
      deltac_tot = 0.0d0
      outproj = .false.
...
!      pos = queue%current_position
      ...
      do
         ...
         elseif (posdef .and..not. outproj) then
            ...
            stat_tab(stat_current_iteration+1,6) = 4.d0
            naccstep = naccstep + 1
            posdef = .false.
      ...
      call mat_mul(S,Dav,'n','n',1.d0,0.d0,SD)
      call mat_mul(SD,S,'n','n',1.d0,0.d0,SDS)
   ...
   subroutine DSMstep(av,naccstep,msize,queue,trustr, S2, DSMgd,DSMhes,mineival,&
      ...
      if (av%info_dsm_step) then
         ...
         call SOLVE_LINEQ(av,msize,queue,S2,DSMgd,DSMhes,n_dir,&
              &           remove_dir,0.0d0,trustr,stepminustrustr,coefcor)
...
!> \brief Returns the solution vector delta c (coefcor) for a certain damping of the hessian (xlambda).
   subroutine SOLVE_LINEQ(av,msize,queue,S2,DSMgd,DSMhes,n_dir,remove_dir,&
      ...
      if (n_dir /= 0) then
         ...
         !alpha = some large number = 1.0d6*E[1]_diagbase_i;
         !x_i = spectral decomposition of the bad direction
         do i=1,n_dir
            if (av%info_dsm_proj) WRITE(av%LUPRI,*)&
                 & 'removing direction, 1.0d6*',ABS(remove_dir(i,msize))
            call DGEMM('N','N',msize-1,1,msize-1,1.0d0,&
               & S2,msize-1,&
               & remove_dir(i,1:msize-1),msize-1,0.0d0,S2x,msize-1)
            call DGEMM('N','T',msize-1,msize-1,1,1.0d0,&
               &S2x,msize-1,&
               &S2x,msize-1,0.0d0,S2xS2xT,msize-1)
            Amat = Amat - 1.0d6*ABS(remove_dir(i,msize))*S2xS2xT
         enddo
      endif
   ...
   subroutine ANALYZE_G_H(av,nqdsm,msize,queue, S2,naccstep,DSMgrad,DSMhes,&
      ...
      if (av%debug_dsm_metric) then
         ...
         do i = 1,msize-1
            Id(i,i) = 1.0d0
         enddo
         call my_DSYGV(msize-1,eivec,Id,eival,wrk1,size(wrk1),"ANALYZE_G_H:1       ")
      ...
      !S2 is the metric
      Id = S2
      call my_DSYGV(msize-1,eivec,Id,eival,wrk1,size(wrk1),"ANALYZE_G_H         ")
      !find the lowest eigenvalue
      ...
      if (outproj) then
         ...
         call DGEMM('T','N',msize-1,1,msize-1,-1.0d0,eivec,msize-1,DSMgrad,msize-1,0.0d0,DSMgrad_diagbase,msize-1)
         do i = 1,msize-1
...
!> Evaluate DSM step.
   subroutine STEP_EVAL(av,nqdsm,msize,his_start,n_dir,remove_dir,minstart,queue,DSMgradnorm_0,&
      ...
      if (n_dir > 0) then
        ...
        do i = 1,n_dir
          call DGEMM('N','N',msize-1,1,msize-1,1.0d0,&
               & S2,msize-1,remove_dir(i,1:msize-1),msize-1,0.0d0,S2x,msize-1)
      ...
      if ((posdef .or. n_dir > 0) .and. DSMgradnorm < DSMgradnorm_0*1.d-3) then
         naccstep = naccstep + 1
         ...
         if (av%DEBUG_DSM_DCHANGE) then
           ...
           !||Dbar - Do||
           call mat_add(1.d0,Dav,-1.d0,queue%D(his_start),Dchange)
           write(av%lupri,*) '||Dbar - Do|| = ',util_Snorm(Dchange,S),'      grepdchange'
           call get_Dtilde(S,Dav,Dtilde)
           call mat_add(1.d0,Dtilde,-1.d0,Dav,Dchange)
           write(av%lupri,*) '||Dtilde - Dbar|| = ',util_Snorm(Dchange,S),'          grepdchange'
           call mat_add(1.d0,Dtilde,-1.d0,queue%D(his_start),Dchange)
...
!> \brief Should the trust-radius be changed and should the step be rejected?
   subroutine update_trustr(av,msize,coefcor,DSMener,DSMenerold,DSMgd,DSMhes,trustr,steprej)
     implicit none
      ...
      if (ABS(DSMenerpred - DSMenerold) > 1.0d-20) then
      ...
      !        &ratio,DSMener-DSMenerold,DSMenerpred-DSMenerold
      if ((ratio > 0.0d0 .and. ratio < 0.40d0) .or. 1.60d0 < ratio) then
         !** STEP REJECTED  Too bold a step
         trustr = trustr * 0.5d0
         ...
         !** STEP TAKEN
         if (ratio > 0.75d0 .and. ratio < 1.25d0) then
            trustr = trustr * 1.3d0
...
!> \brief DSM is ended or restartet depending on the conditions. In particular, neg. hes. eigenvalues are not accepted as ending condition.
   subroutine EXIT_OR_RESTART(av,msize,queue,S,maxstep, &
      ...
      real(realk), intent(inout) :: coef(msize-1),deltac_tot(msize-1),&
           &mineival(maxstep),trustr,Newtonstep(maxstep)
      type(Matrix)  :: Fav, Dav
      integer :: qreduced
...
!
         if (steprej > 5) stat_tab(stat_current_iteration+1,6) = 5.d0
         if (naccstep == maxstep) then
           stat_tab(stat_current_iteration+1,6) = 6.d0 
         endif
!
...
!  
         if (stat_tab(stat_current_iteration+1,6) > 1.d-3) then
...
!
         if (naccstep > 1 .and. mineival(1) < dsm_hes_zero .and. mineival(naccstep) < dsm_hes_zero) then
           if ((mineival(naccstep) - dsm_hes_zero) < (mineival(1) - dsm_hes_zero)*1.2d0) then
             ! lowest Hessian eigenval getting more negative
             restart = .true.
           endif
           if (naccstep > 3 .and. ABS(mineival(naccstep)/mineival(1)) > 0.7d0) then
         ...
         case (3) 
           ...
           changelimit = changelimit*1.5d0
           stat_tab(stat_current_iteration+1,6) = 0.0d0
         case (4) 
...
!                      
          restart = .false.
          end_dsm = .true.
          stat_tab(stat_current_iteration+1,6) = 7.d0
          coef = 0.0d0
...
!             outproj = .true.            
 ...
 !            deltac_tot = 0.0d0
  !           Newtonstep = 0.0d0
   !          stat_tab(stat_current_iteration+1,6) = 0.d0
    !         Fav = queue%F(his_start)
     !        Dav = queue%D(his_start)
      !       DSMener = DSMener_start
        endif
      ...
      if (end_dsm) then 
         ...
         if (outproj) stat_tab(stat_current_iteration+1,6) = &
                    &stat_tab(stat_current_iteration+1,6) + 20.0d0
...
!> \brief Find the DSM energy Edsm = EHF(Do)+TrFoDav-TrFavDo-TrFoDo-TrFavDav+6TrFavDavSDav-4TrFavDavSDavSDav
   subroutine DSMenergy(av,msize,his_start,queue,S,H1,fav,dav,Delta,DSMener)
      ...
      !call cpu_time(timing1)
      call mat_mul(Dav,S,'n','n',1.d0,0.d0,DavS)
      call mat_mul(DavS,Dav,'n','n',1.d0,0.d0,DavSDav)
      call mat_mul(Fav,DavS,'n','n',1.d0,0.d0,FavDavS)
      !call cpu_time(timing2)
      !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
      TrFavDavSDav = mat_dotproduct(Fav,DavSDav)
      trfavdavsdavsdav = mat_dotproduct(FavDavS,DavSDav)
      DSMener = DSMener - TrFavDav + &
           &    6.d0*TrFavDavSDav - 4.d0*TrFavDavSDavSDav
      if (av%cfg_dsm_app == av%cfg_dsm_xtra_term) then
...
!> \brief Find the predicted DSM energy. 
   subroutine DSMenergy_pred(msize,coef,DSMenerold,DSMgd,DSMhes,DSMenerpred)
      ...
      do i = 1,msize-1
         ...
         do j = 1,msize-1
            DSMenerpred = DSMenerpred + 0.5d0*coef(i)*DSMhes(i,j)*coef(j)
         enddo
      enddo
...
!> \brief Find d_tilde = 3dsd - 2dsdsd, where d = d_bar
   subroutine get_Dtilde(S,Dbar,Dtilde)
      ...
      !call cpu_time(timing1)
      call mat_mul(S,Dbar,'n','n',1.d0,0.d0,SD)
      call mat_mul(Dbar,SD,'n','n',1.d0,0.d0,DSD)
      call mat_mul(DSD,SD,'n','n',1.d0,0.d0,DSDSD)
      !call cpu_time(timing2)
      !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
      call mat_add(3.0d0,dsd, -2.D0,dsdsd, dtilde)
...
!> \brief Get DSM gradient and Hessian, improved routine!
   subroutine DSMgrad_and_hes_better(msize,his_start,minstart,queue,Delta,&
      ...
      Ndim = S%nrow
      grad = 0.0d0
      hes = 2.0d0*mat_dotproduct(queue%D(his_start),queue%F(his_start))
      call mat_init(wrk,ndim,ndim)
      call mat_init(wrk2,ndim,ndim)
      call mat_init(DavSDav,ndim,ndim)
      call mat_init(FavDavS,ndim,ndim)
      !call cpu_time(timing1)
      call mat_mul(Dav,S,'n','n',1.d0,0.d0,wrk)
      call mat_mul(wrk,Dav,'n','n',1.d0,0.d0,DavSDav)
      call mat_mul(Fav,wrk,'n','n',1.d0,0.d0,FavDavS)
      !call cpu_time(timing2)
      !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
      ...
      do i=1,msize
         ...
         !call cpu_time(timing1)
         call mat_mul(queue%F(his_p(i)),wrk,'n','n',1.d0,0.d0,FDavS(i))
         call mat_mul(S,queue%D(his_p(i)),'n','n',1.d0,0.d0,wrk2)
         !call cpu_time(timing2)
         !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
         call mat_init(DavSD(i),ndim,ndim)
         !call cpu_time(timing1)
         call mat_mul(Dav,wrk2,'n','n',1.d0,0.d0,DavSD(i))
         !call cpu_time(timing2)
         !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
         call mat_init(FavDS(i),ndim,ndim)
         !call cpu_time(timing1)
         call mat_mul(Fav,wrk2,'n','t',1.d0,0.d0,FavDS(i))
         !call cpu_time(timing2)
         !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
         call mat_init(FavD(i),ndim,ndim)
         !call cpu_time(timing1)
         call mat_mul(Fav,queue%D(his_p(i)),'n','n',1.d0,0.d0,FavD(i))
         !call cpu_time(timing2)
         !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
      enddo   
      ...
      do i = 1,msize
         ...
         grad(i) = grad(i) + mat_dotproduct(queue%D(his_p(i)),queue%F(his_start)) &
                 ...
                 &         + 6.0d0*mat_dotproduct(Fav,DavSD(i))                   &
                 &         + 6.0d0*mat_dotproduct(queue%F(his_p(i)),DavSDav)      &
                 &         + 6.0d0*mat_dotproduct(Fav,wrk)                        &
                 &         - 4.0d0*mat_TrAB(FavDavS,DavSD(i))                     &
                 &         - 4.0d0*mat_dotproduct(FavDS(i),DavSDav)               &
                 &         - 4.0d0*mat_dotproduct(FDavS(i),DavSDav)               &
                 &         - 4.0d0*mat_TrAB(FavDavS,wrk)
         do j = 1,i
           hes(i,j) = hes(i,j) - mat_dotproduct(queue%D(his_p(i)),queue%F(his_p(j)))  &
                    ...
                    &          + 6.0d0*mat_dotproduct(queue%F(his_p(i)),DavSD(j))     &
                    &          + 6.0d0*mat_dotproduct(queue%F(his_p(j)),DavSD(i))     &
                    &          + 6.0d0*mat_dotproduct(FavDS(j),queue%D(his_p(i)))     &
                    &          + 6.0d0*mat_dotproduct(FavDS(i),queue%D(his_p(j)))     &
                    &          - 4.0d0*mat_TrAB(FDavS(i),DavSD(j))                    &
                    &          - 4.0d0*mat_TrAB(FDavS(j),DavSD(i))                    &
                    &          - 4.0d0*mat_TrAB(FavDS(j),DavSD(i))                    &
                    &          - 4.0d0*mat_TrAB(FavDS(i),DavSD(j))                                     
           call mat_trans(DavSD(j),wrk)
           hes(i,j) = hes(i,j) + 6.0d0*mat_dotproduct(queue%F(his_p(i)),wrk)          &
                    &          - 4.0d0*mat_TrAB(FDavS(i),wrk)                         
           call mat_trans(FDavS(i),wrk2)
           hes(i,j) = hes(i,j) - 4.0d0*mat_TrAB(wrk2,wrk)                             &
                    &          - 4.0d0*mat_TrAB(FavDS(i),wrk)
           call mat_trans(DavSD(i),wrk)
           hes(i,j) = hes(i,j) - 4.0d0*mat_TrAB(FDavS(j),wrk)                         &
                    &          - 4.0d0*mat_TrAB(FavDS(j),wrk)                         &
                    &          + 6.0d0*mat_dotproduct(queue%F(his_p(j)),wrk)
           call mat_trans(FDavS(j),wrk2)
           hes(i,j) = hes(i,j) - 4.0d0*mat_TrAB(wrk2,wrk)                             
           call mat_trans(FavDS(i),wrk)
           hes(i,j) = hes(i,j) - 4.0d0*mat_TrAB(wrk,DavSD(j))
           call mat_trans(FavDS(j),wrk)
           hes(i,j) = hes(i,j) - 4.0d0*mat_TrAB(wrk,DavSD(i)) 
         enddo
      enddo
...
!      hes = 0
!      !call cpu_time(timing1)
!      call mat_mul(S,Dav,'n','n',1.d0,0.d0,SDav)
!      do i = 1,msize
!        call mat_mul(S,queue%D(his_p(i)),'n','n',1.d0,0.d0,SD(i))
!      enddo
!      !call cpu_time(timing2)
!      !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!!
...
!        !call mat_scal(-1.d0,dDelta(i))  !- Di
!        !call cpu_time(timing1)
!        call mat_mul(Dav,SD(i),'n','n',1.d0,0.d0,DavSDi)
!        call mat_mul(SDav,DavSDi,'t','n',1.d0,0.d0,wrk1)
!        !call cpu_time(timing2)
!        !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!        call mat_add(3.d0,DavSDi,-2.d0,wrk1,wrk2)
!        call util_get_symm_part(wrk2)
!        !call mat_daxpy(2.d0,wrk2,dDelta(i)) !2[3DavSDi - 2DavSDavSDi]^S
!        call mat_add(2.d0,wrk2, -1d0,queue%D(his_p(i)), dDelta(i))
!        !call cpu_time(timing1)
!        call mat_mul(DavSDi,SDav,'n','n',1.d0,0.d0,wrk2)
!        !call cpu_time(timing2)
!        !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!        call mat_daxpy(-2.d0,wrk2,dDelta(i)) !- 2DavSDiSDav
!!
!! Hessian stuff
!!
!        do j = 1,i
!          !call cpu_time(timing1)
!          call mat_mul(queue%D(his_p(j)),SD(i),'n','n',1.d0,0.d0,wrk2) !wrk2 = DjSDi
!          !call cpu_time(timing2)
!          !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!          ddDelta = wrk2
!          !call mat_scal(6.d0,ddDelta)                  !2*3DjSDi
!          !call cpu_time(timing1)
!          call mat_mul(wrk2,SDav,'n','n',1.d0,0.d0,wrk1)
!          !call cpu_time(timing2)
!          !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!          !call mat_daxpy(-4.d0,wrk1,ddDelta)           !2*- 2DjSDiSDav
!          call mat_add(-4.d0,wrk1,6d0,wrk2,ddDelta)
!          !call cpu_time(timing1)
!          call mat_mul(SDav,wrk2,'t','n',1.d0,0.d0,wrk1)
!          !call cpu_time(timing2)
!          !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!          call mat_daxpy(-4.d0,wrk1,ddDelta)           !2*- 2DavSDjSDi
!          !call cpu_time(timing1)
!          call mat_mul(SD(j),DavSDi,'t','n',1.d0,0.d0,wrk1)
!          !call cpu_time(timing2)
!          !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!          call mat_daxpy(-4.d0,wrk1,ddDelta)           !2*- 2DjSDavSDi
!          call util_get_symm_part(ddDelta)             ![]^S
!          !
!          ! d² Delta/d c_i d c_j has now been build
!          !
!          hes(i,j) = hes(i,j) + 2.d0*mat_dotproduct(Fav,ddDelta)  !2Tr Fav ddDelta
...
!      hes = hes + 2.d0*mat_dotproduct(queue%D(his_start),queue%F(his_start)) !2TrDoFo
!      do i = 1,msize
!        TrDoF(i) = mat_dotproduct(queue%D(his_start),queue%F(his_p(i)))
!        TrFoD(i) = mat_dotproduct(queue%D(his_p(i)),queue%F(his_start))        
!        grad(i) = grad(i) &
!                   & + 2.d0*mat_dotproduct(queue%F(his_p(i)),Delta)               & !2Tr Fi Delta
!                   & + 2.d0*mat_dotproduct(Fav,dDelta(i))                         & !2Tr Fav dDeltai 
...
!        do j = 1,i
!          hes(i,j) = hes(i,j) &
!                   & + 2.d0*mat_dotproduct(queue%F(his_p(i)),dDelta(j))           & !2Tr Fi dDeltaj
!                   & + 2.d0*mat_dotproduct(queue%F(his_p(j)),dDelta(i))           & !2Tr Fj dDeltai
...
!        enddo
!        !call cpu_time(timing1)
!        call mat_mul(S,queue%D(j),'n','n',1.d0,0.d0,SD(i))
!        !call cpu_time(timing2)
!        !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
...
!      do i = 1,msize
!        grad_x(i) = grad_x(i) + 2.0d0*TrFoDo - mat_dotproduct(Fav,queue%D(his_start)) &
!                & - mat_dotproduct(queue%F(his_start),Dav)
...
!          else
!            hes_x(i,j) = hes_x(i,j) + 2.0d0*TrFoDo
!          endif
!        enddo
!      enddo
!  !** chose F
...
!            call mat_mul(Fav,Dav,'n','n',1.d0,0.d0,FD)
!          elseif (j1 == 0) then
!            call mat_mul(Fav,queue%D(j2),'n','n',1.d0,0.d0,FD)
!          elseif (j2 == 0) then
!            call mat_mul(queue%F(j1),Dav,'n','n',1.d0,0.d0,FD)
!          else
!            call mat_mul(queue%F(j1),queue%D(j2),'n','n',1.d0,0.d0,FD)
!          endif
!          !call cpu_time(timing2)
!          !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!          nmult = nmult + 1
...
!            endif
!            !call cpu_time(timing1)
!            call mat_mul(FD,X,'n','n',1.d0,0.d0,FDSD)
!            !call cpu_time(timing2)
!            !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
!            nmult = nmult + 1
!            TrFDSD = mat_Tr(FDSD)
...
!              if (i3 == 0) then
!                grad(i1+i2) = grad(i1+i2) + 6.d0*TrFDSD  !+6TrFavDxSDav +6TrFxDavSDav
!!                write(lupri,*) i1+i2,6.0d0*TrFDSD,' +6TrFavDxSDav +6TrFxDavSDav'
!              else
!                !+6TrFavDySDx +6TrFavDxSDy +6TrFyDavSDx + 6TrFxDavSDy
!                hes(MAX(i1,i2,i3),MIN(MAX(i1,i2),i3)) = hes(MAX(i1,i2,i3),MIN(MAX(i1,i2),i3)) + 6.d0*TrFDSD
!                i4end = 0
!              endif 
!            elseif (i1 /= 0 .and. i2 /= 0) then  !i3 = 0
!              !+6TrFyDxSDav +6TrFxDySDav
!              hes(MAX(i1,i2),MIN(i1,i2)) = hes(MAX(i1,i2),MIN(i1,i2)) + 6.d0*TrFDSD
!              i4end = 0
!            else   !both i1 and i2 are zero
!              if (i3 /= 0) then  !6TrFavDavSDx
!                grad(i3) = grad(i3) + 6.d0*TrFDSD
!!                write(lupri,*) i3,6.d0*TrFDSD,' +6TrFavDavSDx'
!              endif
!            endif    
!  !** choose SD2
!            do i4 = 0,i4end
...
!                  grad(i1+i2) = grad(i1+i2) - 4.d0*TrFDSDSD  !-4TrFavDxSDavSDav -4TrFxDavSDavSDav
!!                  write(lupri,*) i1+i2,-4.d0*TrFDSDSD,' -4TrFavDxSDavSDav -4TrFxDavSDavSDav' 
!                else
...
!                  hes(MAX(in1,in2),MIN(in1,in2)) = hes(MAX(in1,in2),MIN(in1,in2)) - 4.d0*TrFDSDSD
!                endif
!              elseif (i1 /= 0 .and. i2 /= 0) then !i3 = 0, i4 = 0
!                !-4TrFyDxSDavSdav -4TrFxDySDavSDav
!                hes(MAX(i1,i2),MIN(i1,i2)) = hes(MAX(i1,i2),MIN(i1,i2)) - 4.d0*TrFDSDSD
!              else  !both i1 and i2 are zero
!                if (MAX(i3,i4) == i3+i4 .and. i3 /= i4) then
!                  grad(i3+i4) = grad(i3+i4) - 4.d0*TrFDSDSD  !-4TrFavDavSDavSDx -4TrFavDavSDxSDav
!!                  write(lupri,*) i3+i4,- 4.d0*TrFDSDSD,' -4TrFavDavSDavSDx -4TrFavDavSDxSDav' 
!                elseif (i3 /= 0 .and. i4 /= 0) then
!                  !-4TrFavDavSDySDx -4TrFavDavSDxSDy
!                  hes(MAX(i3,i4),MIN(i3,i4)) = hes(MAX(i3,i4),MIN(i3,i4)) - 4.d0*TrFDSDSD
!                endif
!              endif
!            enddo
!          enddo
...
!       do i = 1,msize
!         if (i /= minstart) then
!           call mat_add(1.d0,queue%D(his_p(i)),-1.d0,queue%D(his_start),Dchange)
...
!>  \brief Check the densities one by one, comparing it to Do. If the purification doesn't converge, they are removed from the history
   subroutine CLEAN_DHISTORY(av,S,queue)
      ...
      do k = 1,queue%used_entries
        if (k/=his_start) then
          call mat_add(1.d0,queue%D(his_start),0.2d0,queue%D(k),Dbar_k)
          !call cpu_time(timing1)
          call mat_mul(S,Dbar_k,'n','n',1.d0,0.d0,X)
          call mat_mul(Dbar_k,X,'n','n',1.d0,0.d0,DSD)
          call mat_mul(DSD,X,'n','n',1.d0,0.d0,DSDSD)
          !call cpu_time(timing2)
          !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
          call mat_add(3.0d0,DSD,-2.0d0,DSDSD,X)
          call mat_daxpy(-1.0d0,Dbar_k,X)   !X = Dtilde_k - Dbar_k
          call mat_daxpy(-1.0d0,queue%D(his_start),Dbar_k)   !Dbar_k = Dbar_k - Do
...
!      use fock_evaluator
      ...
      da = 0.2d0 !should be smaller than 1 :)
      ...
      if (av%cfg_dsm_app == av%cfg_dsm_search) then
        ...
        !        coef = 0.0d0
        !        WRITE(LUPRI,*) 'chose alpha: 0.0d0',stat_current_iteration+1
        !        exit
        !      endif
        !      da = MAX(alpha/5.d0,0.2d0)
        !    enddo
        ...
        !    !alpha = 1
        !    alpha = 1.d0
        !    WRITE(LUPRI,*) 'chose alpha: 1.d0',stat_current_iteration+1
        !    !coef = coef
        !  endif
        !endif
        !stat_tab(stat_current_iteration+1,6) = 11.d0
      else
        stat_tab(stat_current_iteration+1,6) = 10.d0
      endif
...
!         call mat_to_full(Delta,2.d0,Dfull)
!
!         CALL di_FCK2AO(.TRUE.,Ffull,Dfull)
!         call mat_set_from_full(Ffull,1d0,Gorth)
!        !call fck_get_fock(Delta,Gorth,junk)
!        !call mat_daxpy(-1.d0,H1,Gorth)
...
!        call mat_scal(-1.d0,scr2)   ! - Dpara
!        call mat_daxpy(1.d0,Delta,scr2)   !Dorth = Delta - Dpara
...
!        call mat_daxpy(1.d0,Delta,scr2)   !2 Delta - Dpara
...
!> \brief Delta = 3DavSDav - 2DavSDavSDav - Dav
   subroutine eval_delta(Dav,S,Delta)
     ...
     ndim = S%nrow
      ...
      !call cpu_time(timing1)
      call mat_mul(S,Dav,'n','n',1.d0,0.d0,SD)
      call mat_mul(Dav,SD,'n','n',1.d0,0.d0,DSD)
      call mat_mul(DSD,SD,'n','n',-2.d0,0.d0,Delta)
      !call cpu_time(timing2)
      !cfg_dsmtime = cfg_dsmtime + (timing2 - timing1)
      call mat_daxpy(3.d0,DSD,Delta)
      call mat_daxpy(-1.d0,Dav,Delta)
...
!!** grad
!!        grad(i) = (fd(i) - fmd(i))/(2.0d0*diff)
!        grad(i) = (8.0d0*fd(i)-8.0d0*fmd(i)-f2d(i)+fm2d(i))/(12.0d0*diff)
!!** hes
!!        hes(i,i) = (fd(i) + fmd(i) - 2.0d0*f0)/(diff*diff)
!        hes(i,i) = (-f2d(i) + 16.0d0*fd(i) - 30.0d0*f0 + 16.0d0*fmd(i) - fm2d(i))/(12.0d0*diff*diff)
!        do j = i+1,msize
!!** fdd
!          wrk(i) = weights(i) + diff
...
!      do i = 1,msize
!        do j = i+1,msize
!          hes(i,j) = (2.d0*f0 + fdd(i,j) + fmdmd(i,j) - fd(i) - fmd(i) - fd(j) - fmd(j))/(2.d0*diff*diff)
!!          hes(i,j) = (f2d(i) + fm2d(i) + f2d(j) + fm2d(j) - 2.d0*f0 - f2d2d(i,j) - fm2dm2d(i,j))/(2.d0*diff*diff)
!!          hes(i,j) = hes(i,j) - 4.d0*(fd(i) + fmd(i) + fd(j) + fmd(j) - 2.d0*f0 - fdd(i,j) - fmdmd(i,j))/(2.d0*diff*diff)
!!          hes(i,j) = hes(i,j)/3.d0
!          hes(j,i) = hes(i,j)
!        enddo
!      enddo
!!
...
!!     fd = func(x+delta)
!!     fmd = func(x-delta)
!!     f2d = func(x+2.d0*delta)
!!     fm2d = func(x-2.d0*delta)
! 
!!     grad2 = (fd-fmd)/(2.0d0*delta)
!!     hes2 = (fd+fmd-2.0d0*f0)/delta**2
!!     grad = (8.0d0*fd-8.0d0*fmd-f2d+fm2d)/(12.0d0*delta)
!!     hes = (-1.0d0*f2d+16.0d0*fd-30.0d0*f0+16.0d0*fmd-1.0d0*fm2d)/(12.0d0*delta**2)
! 
