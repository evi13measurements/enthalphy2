...
module dsm_x_term
   ...
   use matrix_util
   private
   public :: xterm_start,xterm_build_Gpara,xterm_build_hes,&
...
!       call mat_init(wrk,ndim,ndim)
!
!       hes = 2.d0*mat_dotproduct(ddDelta,Gpara) !2Trd²Delta/dc_i dc_j G(Delta_para)
!!
...
!       hes = hes - 2.d0*mat_dotproduct(wrk,Gpara) ! - 2Trd²Delta_para/dc_i dc_j G(Delta_para)
!!
...
!       hes = hes + 2.d0*mat_dotproduct(Delta,wrk)  !  +2TrDelta d²G(Delta_para)/dc_i dc_j 
!       call mat_free(wrk)
!   
...
!        grad(i) = grad(i) +2.d0*mat_dotproduct(Delta,dGpara(i))
!      enddo
!!
...
!        call get_average_arr(av,'D',queue, dsm_history_size, do_dc(:,i), wrk)
!        do j = 1,msize
!          hes(i,j) = hes(i,j) + 2.d0*mat_dotproduct(dDelta(i),dGpara(j)) &
!                   &          + 2.d0*mat_dotproduct(dDelta(j),dGpara(i)) &
!                   &          - 2.d0*mat_dotproduct(wrk,dGpara(j))
!        enddo
!      enddo
!  
!      do i = 1,msize
