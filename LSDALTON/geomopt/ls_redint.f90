...
!!!!! Probably, temporary here
      Write(*,*)'deguggg: intcoord',optinfo%NIntCoord
...
!
     ...
     &           MARK,CRDTXT,optinfo%INTCRD(I,2),optinfo%INTCRD(I,3), &
     &           optinfo%INTCRD(I,4),optinfo%INTCRD(I,5),1.8D2*VALINT(I)/PI,UNTTXT
         ELSE
            UNTTXT = ' deg.'
     ...
     &           MARK,CRDTXT,optinfo%INTCRD(I,2),optinfo%INTCRD(I,3), &
     &           optinfo%INTCRD(I,4),180.D0*VALINT(I)/PI,UNTTXT
         END IF
 10   CONTINUE
 ...
 25   CONTINUE
      ...
      IF (VAL .LT. 1.0D6) THEN
         DO 30 I = IFIRST, optinfo%NIntCoord
            IF (ABS(ABS(optinfo%CoordInt(I))-VAL) .LT. 1.0D-8) THEN
               optinfo%CoordInt(I) = 1.1D6
            END IF
 30      CONTINUE
      END IF
...
!
 ...
 35   CONTINUE
      IF (IC .LE. optinfo%NIntCoord) THEN
         IF (optinfo%CoordInt(IC) .GT. 1.0D6) THEN
...
!
     ...
     &                    - ATMARR(optinfo%INTCRD(IC,3),4)
                     ...
                     optinfo%Change = (BNDL2-BNDL1)/1.0D-6
                     ATMARR(II,JJ+1) = ATMARR(II,JJ+1) - 1.0D-6
...
!
     ...
     &                    ANGLE2 = vecang_ls(VEC1,VEC4) + vecang_ls(VEC4,VEC2)
                     optinfo%Change = (ANGLE2-ANGLE)/1.0D-6
                     ATMARR(II,JJ+1) = ATMARR(II,JJ+1) - 1.0D-6
                     WRITE(LUPRI,'(2I5,F16.6)') II,JJ,optinfo%Change
 48               CONTINUE
 47            CONTINUE
...
!
     ...
     &                    ATMARR(optinfo%INTCRD(IC,II),JJ+1) + 1.0D-6
                     VEC1(1)= ATMARR(optinfo%INTCRD(IC,2),2) &
     &                    - ATMARR(optinfo%INTCRD(IC,3),2)
                     VEC1(2)= ATMARR(optinfo%INTCRD(IC,2),3) &
     &                    - ATMARR(optinfo%INTCRD(IC,3),3)
...
!
      IF (.NOT. optinfo%DelInt) THEN
         call ls_GTDWLM(Molecule,MXRCRD,IC,DWILBM,ATMARR,WILBMT,lupri,optinfo)
      ELSE
         call ls_GTDWL1(Molecule,MXRCRD,IC,DWILBM,ATMARR,WILBMT,BMTRAN, &
...
!  /* Deck gtdwl1 */
      SUBROUTINE LS_GTDWL1(Molecule,MXRCRD,IC,DWILBM,ATMARR,WILBMT,BMTRAN,& 
...
!
!hjaaj   WMAT(IDIM2+1,IDIM2+1) = 1.1D6
...
Implicit Real(realk) (A-H,O-Z)
      ...
      Real(realk) GRADQ(MXRCRD), HESSX(MXCOOR,MXCOOR)
      Real(realk) WILBMT(MXRCRD,MXCOOR), BMTRAN(MXRCRD,MXRCRD)
      call ls_DZERO(TMPMAT,MX2CRD*MX2CRD)
      DO 10 IC = 1, optinfo%NIntCoord
         call ls_GTDWL0(Molecule,MXRCRD,IC,HESSX,ATMARR,WILBMT,BMTRAN,& 
         & lupri,optinfo)
 ...
 22      CONTINUE
 20   CONTINUE
      call ls_DZERO(HESSX,MXCOOR*MXCOOR)
      DO 30 I = 1, optinfo%ICartCoord
         DO 32 J = 1, optinfo%ICartCoord
...
Implicit Real(realk) (A-H,O-Z)
      ...
      Real(realk) HESSX(MXCOOR,MXCOOR), GRADQ(MXRCRD)
...
Implicit Real(realk) (A-H,O-Z)
      ...
      Real(realk) WILBMT(MXRCRD,MXCOOR), BMTINV(MXRCRD,MXCOOR)
      Real(realk) BMTRAN(MXRCRD,MXRCRD)
      Real(realk) HESINT(MXRCRD,MXRCRD)
      PARAMETER (IPRMIN = 1, IPRMED = 3, IPRMAX = 5, IPRDBG = 12)
!
      IF (optinfo%IPrint .GE. IPRMIN) THEN
...
!
      call dgemm('N','N',NDIM,1,NDIM,1.0d0,PJINMT,MXRCRD, &
     &           optinfo%GRDINT,NDIM,0.0d0,TMPMT1,NDIM)
...
!     Then the Hessian
!
      call dgemm('N','N',NDIM,NDIM,NDIM,1.0d0,PJINMT,MXRCRD, &
     &           HESINT,MXRCRD,0.0d0,TMPMT1,NDIM)
      call dgemm('N','N',NDIM,NDIM,NDIM,1.0d0,TMPMT1,NDIM, &
     &           PJINMT,MXRCRD,0.0d0,HESINT,MXRCRD)
!
!     The projected Hessian is "stabilized".
!     1.0D4 is an arbitrary (high) value.
...
!      DO 90 I = 1, NDIM
!         DO 95 J = 1, NDIM
!            HESINT(I,J) = HESINT(I,J) - 1.0D4*PJINMT(I,J)
! 95      CONTINUE
!         HESINT(I,I) = HESINT(I,I) + 1.0D4
! 90   CONTINUE
      IF (optinfo%IPrint .GE. IPRMED) THEN
...
!
      optinfo%IndHes = 0
      DO J = 1, NCRDHS
         optinfo%GRDDIA(J) = DDOT(NCRDHS,optinfo%GRDINT,1,TMPHES(1,J),1)
         DO I = 1, NCRDHS
            EVEC(I,J) = TMPHES(I,J)
         ENDDO
         IF (optinfo%Eval(J) .LT. -THRIND) optinfo%IndHes = optinfo%IndHes + 1
...
!
      DO 25 I = 1, NCRDHS
         IF (ABS(optinfo%EVAL(I)) .LT. 1.0D-6) optinfo%EVAL(I) = optinfo%EVAL(I) +  1.0D5
 25   CONTINUE
      DO 30 I = 1, NCRDHS
         JMIN = I
         EMIN = optinfo%EVAL(I)
 ...
 30   CONTINUE
     ...
     &     .AND. optinfo%IndHes .GT. 0) optinfo%IndHes = optinfo%IndHes - 1
      DO 40 I = 1, NCRDHS
         IF (ABS((ABS(optinfo%EVAL(I)) - 1.0D5)) .LT. 1.0D-3) &
     &        optinfo%EVAL(I) = optinfo%EVAL(I) -  1.0D5
 40   CONTINUE
      IF (optinfo%IPrint .GE. IPRMIN) THEN
...
!               IMODE = 1
! 3             CONTINUE
!               IF ((ABS(optinfo%GRDDIA(IMODE)) .LE. 1.0D-10) &
...
!
         ...
         DO 10 I = 1, optinfo%NIntCoord
!
!     The eigenvalue threshold was changed from 1.0D-8 due to
!     problems with optimization in delocalized internals.
!
            IF (ABS(optinfo%EVAL(I)) .LE. 1.0D-6) THEN
               optinfo%STPDIA(I) = D0
            ELSE
...
!     Newton step is larger than 0.5D-3. For saddle points we
...
!
     ...
     &        (optinfo%stepNorm .GE. 0.5D-3)) .OR. (optinfo%Saddle .AND. &
...
!            IF (DOSCAL) THEN
!               CMPLIM = MAX(optinfo%TrustRad*0.67D0, 0.5D0)
...
!
     ...
     &        + 0.5D0*dv3dot_ls(optinfo%NIntCoord,optinfo%STPDIA,optinfo%EVAL,optinfo%STPDIA)
!
...
!
         IF ((.NOT. optinfo%Saddle) .AND. (optinfo%predictedChange .GT. 0.0D0)) THEN
...
!
     ...
     &           optinfo%STPINT(I) = MOD(optinfo%STPINT(I),2.0D0*PI)
            IF (ABS(optinfo%STPINT(I)) .LE. 1.0D-8) optinfo%STPINT(I) = D0
 20      CONTINUE
         IF (optinfo%IPrint .GE. IPRMIN) THEN
...
!
         DO 210 I = 1, optinfo%NIntCoord
            IF (ABS(optinfo%EVAL(I)) .GE. 1.0D-6) THEN
...
!
     ...
     &           ' Predicted energy change', optinfo%predictedChange
         ...
         DO 250 I = 1, optinfo%NIntCoord
            IF (ABS(optinfo%STPDIA(I)) .GE. 1.0D-6) THEN
...
!
 ...
 18      CONTINUE
      ...
      ELSE
         DO 19 I = 1, NRIC
            DIFF(I) = (optinfo%CoordInt(I) - VALNEW(I))*(optinfo%IConstr(I)*1.0D0)
            IF (optinfo%INTCRD(I,1) .GT. 10) DIFF(I) = MOD(DIFF(I),2.0D0*PI)
...
!
 ...
 20      CONTINUE
         ...
         ITRCRD = ITRCRD + 1
         GOTO 123
!      ELSE IF (((ITRCRD .GE. ITRLIM) .AND. (RMS1ST .LE. 1.0D2*RMSINT))
!     &        .OR. ((ITRCRD .LT. ITRLIM) .AND.
!     &        (RMS1ST .LE. 1.0D2*RMSINT) .AND.
!     &        (RMSINT .GE. 1.0D2*RMSLIM))
!     &        .OR. (RMSINT .GE. 1.0D0)) THEN
      ELSE IF ((ITRCRD .GE. ITRLIM) .OR. ((RMS1ST .LE. 1.0D2*RMSINT) &
     &        .AND. (RMSINT .GE. 1.1D0*RMSLIM)) .OR. &
     &        (RMSINT .GE. 1.0D0)) THEN
!      ELSE IF ((ITRCRD .GE. ITRLIM) .OR. (RMSINT .GE. 1.0D2)) THEN
         IF (optinfo%IPrint .GE. IPRMIN) THEN
            WRITE(LUPRI,*) &
 ...
 40      CONTINUE
         ...
         DO 45 I = 1, NRIC
            ...
            IF (optinfo%INTCRD(I,1) .GT. 10) optinfo%STPINT(I) = MOD(optinfo%STPINT(I),2.0D0*PI)
            IF ((optinfo%INTCRD(I,1) .GT. 20) &
     &           .AND. (ABS(optinfo%STPINT(I)) .GT. PI)) THEN
               IF (optinfo%STPINT(I) .GT. 0.0D0) THEN
                  optinfo%STPINT(I) = optinfo%STPINT(I) - 2.0D0*PI
               ELSE
                  optinfo%STPINT(I) = optinfo%STPINT(I) + 2.0D0*PI
               END IF
            END IF
...
!
      IF (CORREC .AND. (optinfo%NFreeze .GT. 0)) THEN
         DO 510 I = 1, optinfo%NFreeze
            DO 515 J = 1, 3
               CSTEP(3*(optinfo%FreezeArray(I)-1)+J) = 0.D0
 515        CONTINUE
 510     CONTINUE
...
!         IMODE = 1
! 50      CONTINUE
!         IF ((ABS(optinfo%GRDDIA(IMODE)) .LT. 1.0D-10) .AND. &
...
!     
!         ELSE IF (ABS(optinfo%GRDDIA(IMODE)) .LT. 1.0D-10) THEN
!            IMODE = 1
!         END IF
!      END IF
!      IF (optinfo%IPrint .GE. IPRMAX) THEN
...
! 420  CONTINUE
!
!     We add 1.0D5 to all eigenvalues that are essentially zero
!     for the sorting.
!
!      DO 427 I = 1, NCRDHS
!         IF (ABS(optinfo%EVAL(I)) .LE. 1.0D-8) optinfo%EVAL(I) = optinfo%EVAL(I) + 1.0D5
! 427  CONTINUE
!      DO 430 I = 1, NCRDHS
!         JMIN = I
!         EMIN = optinfo%EVAL(I)
...
!
!      CMPLIM = MAX(optinfo%TrustRad*0.67D0, 0.30D0)
!      DO 500 I = 1, NCRDHS-1
...
! ::: numbers close to these values
            IF ((ABS(ABS(TEMP)-1.0D0) .LT. ZERTOL) &
     &           .OR. (ABS(TEMP).GT.1.0D0)) THEN
               TEMP = ANINT(TEMP)
            END IF
            vecang_ls = ACOS(TEMP)
         ELSE
