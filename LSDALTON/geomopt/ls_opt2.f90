...
!
      CA = ABS(5.0D0*GRAD0 - 2.0D0*GRAD1 + 3.0D0*optinfo%energyOld - 3.0D0*optinfo%energy)
      CB = GRAD1 + GRAD0 + 2.0D0*optinfo%energyOld - 2.0D0*optinfo%energy - 2.0D0*CA
...
Implicit Real(realk) (A-H,O-Z)
      ...
      PARAMETER (D0 = 0.0D0)
      ICRD = NCART
      IF (INTERN) ICRD = optinfo%NIntCoord
      ...
      IF (optinfo%ItrNmr .EQ. 0) THEN
         ...
         IF (INTERN) THEN
            call ls_GQ2GX(MXRCRD,EVEC(1,optinfo%NSPMod),VECMOD,WILBMT)
         ELSE
 ...
 5       CONTINUE
         ...
         DO 10 I = 1, NVEC
            IF (TMPVC2(I) .LT. 1.0D0) THEN
               REMAIN = .TRUE.
               IF (INTERN) THEN
                  call ls_GQ2GX(MXRCRD,EVEC(1,I),TMPVEC,WILBMT)
               ELSE
...
!     another mode.
!
         IF ((ABS(optinfo%GRDINT(IOVRL)) .LT. 1.0D-10) .AND. REMAIN) THEN
            TMPVC2(IOVRL) = 2.0D0
            GOTO 5
         END IF
         ...
         call ls_dzero(VECMOD,MXCOOR)
         IF (INTERN) THEN
            call ls_GQ2GX(MXRCRD,EVEC(1,IOVRL),VECMOD,WILBMT)
         ELSE
...
Implicit Real(realk) (A-H,O-Z)
      ...
      LOGICAL   LINDEP(6)
!
      Real(realk), PARAMETER :: THRRND=1.D-2, THRTT=1.D-4, D1=1.0D0
!
      LINDEP = .FALSE.
...
!     DATA C1,C2,C3,C4,C5,C6/1.D-12,1.D-12,1.D-20,1.D-14,1.D-9,1.D-5/
...
Implicit Real(realk) (A-H,O-Z)
      ...
      IF ( (FMAX.LT.0.D0) .OR. (FMIN.GT.0.D0) )  RETURN
!
      ITER = 0
 100  CONTINUE
         ITER= ITER+1
...
!
Do i = 1, NCoord/3
   TMatrix(i*3-2:i*3,1)=(/1.D0,0.D0,0.D0/)
   TMatrix(i*3-2:i*3,2)=(/0.D0,1.D0,0.D0/)
   TMatrix(i*3-2:i*3,3)=(/0.D0,0.D0,1.D0/)
Enddo
!
! Lower (rotational) part of T
!
Do i = 1,NCoord/3
   TMatrix(3*i-2:3*i,4) = (/0.D0,-Coord(3*i),Coord(3*i-1)/)
   TMatrix(3*i-2:3*i,5) = (/Coord(3*i),0.D0,-Coord(3*i-2)/)
   TMatrix(3*i-2:3*i,6) = (/-Coord(3*i-1),Coord(3*i-2),0.D0/)      
Enddo
!
...
!
      Real(realk)  DX(*)
      Real(realk),PARAMETER ::  ZERO = 0.0D0 
!
      dnorm2_ls = ZERO
      IF(N.LE.0)RETURN
      DTEMP  = ZERO
