 ...
 !        Enddo
     ...
     &      ' End of optimization            :       ', optinfo%GeConv, MARK
      ...
      IF ((optinfo%ItrNmr .GT. 0) .AND. (optinfo%ItBreak .LT. (optinfo%ItrNmr - 1))) THEN
         ...
         IF (ABS(optinfo%predictedChange) .GT. 1.0D-10) THEN
...
!
 ...
 10   CONTINUE
      ...
      END
!  /* Deck inihes */
      SUBROUTINE LS_INIHES(Molecule,MXRCRD,MX2CRD,GRDOLD,HESOLD, &
...
Implicit Real(realk) (A-H,O-Z)
      ...
      Real(realk) HESOLD(MXCOOR,MXCOOR), TMPMAT(MX2CRD,MX2CRD)
...
!
      call ls_DZERO(HESOLD,MXCOOR*MXCOOR)
...
!
         ...
         IF (optinfo%EvLini .LE. 0.0D0) THEN
...
!
         IF (optinfo%DelInt .AND. (optinfo%EvLini .LE. 0.0D0)) THEN
...
!
     ...
     &           'UPSTPD',lupri,optinfo)
            call ls_INIHES(Molecule,MXRCRD,MX2CRD,GRDOLD,HESOLD,TMPMAT, &
...
!            IF (optinfo%RejIni .AND. (IREJOL .GT. 0)) RESET = .TRUE.
     ...
     &              HESINT,MXRCRD,TMPMAT(1),TMPMAT(MX2CRD+1), &
     ...
     &              RESET,optinfo%GradNorm,.TRUE.,lupri,optinfo)
            END IF
     ...
     &              optinfo%NIntCoord,MXRCRD,TMPMAT,TMPMT2,TMPMT3,TMPMT4,lupri,optinfo)
            ELSE
               call ls_UPBFR1(MXRCRD,optinfo%STPSYM,GAMMA,HESOLD,optinfo%HessMol,optinfo%ICartCoord, &
     &              MXCOOR,TMPMAT,TMPMT2,TMPMT3,TMPMT4,lupri,optinfo)
            END IF
...
!  /* Deck upbfr1 */
     ...
     &     MPVEC,TMPMAT,TMPMT2,TMPMT3,TMPMT4,lupri,optinfo)
use ls_util 
use precision
use files
use optimization_input
...
!     larger than 0.25D0 are marked (later to be removed).
!
      IF (SMART) THEN
         DO 20 II = 1, INUM
            DNRM = D0
...
Implicit Real(realk) (A-H,O-Z)
      ...
      Real(realk), parameter :: THRLDP = 1.0D-28
...
!
      ...
      call dgemm('N','T',NCR,NCR,NPR,-1.D0,TMAT,NCR, &
     &           TMAT,NCR,1.D0,PROJOP,NCR)
      IF (optinfo%IPrint .GT. 5) THEN
...
!
 ...
 90      CONTINUE
     ...
     &        optinfo%IndHes = optinfo%IndHes - 1
         DO 95 I = 1, NCRDHS
            IF (ABS(optinfo%EVAL(I)) .LE. 1.0D-6) optinfo%EVAL(I) = optinfo%EVAL(I) + 1.0D5
 95      CONTINUE
!
...
!
     ...
     &           ' and actual energy change is : ', RATIO
      ...
      ELSE IF (ABS(RATIO-1.0D0) .LE. DP25*(1.0D0-optinfo%RTENGD)) THEN
         WRITE(LUPRI,*) &
...
!hjaug99 IF (ISTATE .GT. 1 .AND. optinfo%TrustRad .GT. 0.30D0) optinfo%TrustRad = 0.30D0
...
!
      IF (optinfo%Saddle) optinfo%TrustRad = MAX(0.025D0, MIN(1.0D0,optinfo%TrustRad))
...
!
 ...
 40      CONTINUE
     ...
     &        + 0.5D0*SNDTRM
         IF (optinfo%predictedChange .GT. 0.0D0) THEN
            DO 50 I = 1, ICRD
               STEP(I) = EVEC(I,1)*FAC
 50         CONTINUE
            FAC = 0.5D0*FAC
...
!
     ...
     &        .AND. (optinfo%GradNorm .GT. 1.0D-4)) THEN
 10      CONTINUE
         IMOD = IMOD + 1
...
!
     ...
     & + 0.5D0*DV3DOT_LS(NVEC,optinfo%STPDIA,optinfo%EVAL, &
     &optinfo%STPDIA)
...
!
     ...
     &           (1.D0*optinfo%NIntCoord))
            GNRM = SQRT(DDOT(optinfo%NIntCoord,optinfo%GRDINT,1,optinfo%GRDINT,1)/ &
     &           (1.D0*optinfo%NIntCoord))
         ELSE
            call ls_MAXELM(optinfo%GradMol,ICOOR,2,GRDMAX,lupri,optinfo)
            call ls_MAXELM(optinfo%STPSYM,ICOOR,1,STPMAX,lupri,optinfo)
            SNRM = optinfo%StepNorm/SQRT(1.D0*optinfo%ICartCoord)
            GNRM = optinfo%GradNorm/SQRT(1.D0*optinfo%ICartCoord)
         END IF
         CNVRGD(1) = .FALSE.
         CNVRGD(2) = .FALSE.
         CNVRGD(3) = .FALSE.
...
!
            IF (optinfo%ItrNmr .EQ. 0) THEN
               optinfo%GradThr = MAX(0.1D0*optinfo%GradThr,1.D-7)
               optinfo%ThGradMax = 5.D0*optinfo%GradThr
               DIFRMS = 0.D0
               DIFMAX = 0.D0
               PRVRMS = GNRM
               PRVMAX = GRDMAX
            ELSE
               DIFRMS = ABS(PRVRMS-GNRM)
     ...
     &           optinfo%GradThr, optinfo%ThGradMax, optinfo%ThrStep, optinfo%ThStepMax
            WRITE(LUPRI,'(A,24X,4(L,10X))') ' Converged?      ', &
     &           (CNVRGD(I), I = 1,4)
            WRITE(LUPRI,'(A)')' -----------------------' // &
     &     '-----------------------------------------------------------'
     ...
     &           optinfo%GradThr, optinfo%ThGradMax, optinfo%ThrStep, optinfo%ThStepMax, '  |'
            WRITE(LUPRI,'(A,3(L,10X),L,6X,A)') ' Converged?      ', &
     &           (CNVRGD(I), I = 1,4), '|'
            WRITE(LUPRI,'(A///)') &
     &     ' ----------------------------------------------------------' &
     &       //  '--+---------------------------'
...
!
         DO 100 I = 1, NONTRO
            IF (ABS(optinfo%EVAL(I) + RNU) .LE. 1.0D-8) THEN
               optinfo%STPDIA(I) = D0
            ELSE
