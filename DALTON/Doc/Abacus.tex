...
See SOSOCS option above.\newline
 \verb|READ (LUCMD, * ) NCSSCC,NCTERM,NCMTHD,THRSLO|\newline
     ...
     NCMTHD: Method of convergence to be used:\newline
              1: Comparison between the last two values.\newline
              2: Comparison between the last and the response value.\newline
              3: Combination of 1 and 2 methods.\newline
              4: Comparison with the extrapolated value from a polynomial method.\newline
              5: Comparison with the extrapolated value from a rational method.\newline
              6: Comparison with the extrapolated value from a splines method.\newline
              7: Comparison with the extrapolated value from a $y = a + b\cdot tanh (c \cdot x)$
                 function \cite{zps-ICP143-244107-2015}.\newline
              8: Debugging-purposes only: Another fitting of method 7\newline
...
Warning: To run calculations with localization, with a razonable basis set, it is necessary to
increase, around twice the default values at least, the following parameters MXCORB, MAXOCC,
MXCORB_CC, MXCENT_QM, MXQM3, MAXST, MAXMAC and MAXVEC in the corresponding files maxorb.h,
mxcent.h, qm3.h, rspc82.F, sirinp.F and mcdrsp.F respectively. Such an increase produces fails of
the following test cases: geoopt_preopt, geoopt_prop, geoopt_freeze, prop_vibg1, prop_vibg2 and
pehf_molgrad and, in concecuenses, all calculation related with these items.
